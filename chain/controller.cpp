...
#include <chain/include/block_log.hpp>
#include <chain/include/fork_database.hpp>
#include <chain/include/exceptions.hpp>

#include <chain/include/account_object.hpp>
#include <chain/include/block_summary_object.hpp>
#include <chain/include/dosio_contract.hpp>
#include <chain/include/global_property_object.hpp>
#include <chain/include/contract_table_objects.hpp>
#include <chain/include/generated_transaction_object.hpp>
#include <chain/include/transaction_object.hpp>
#include <chain/include/reversible_block_object.hpp>

#include <chain/include/authorization_manager.hpp>
#include <chain/include/resource_limits.hpp>
#include <chain/include/chain_snapshot.hpp>
...
struct controller_impl {
   ...
   optional<dp::time_point>       replay_head_time;
   db_read_mode                   read_mode = db_read_mode::SPECULATIVE;
   bool                           in_trx_requiring_checks = false; ///< if true, checks that are normally skipped on replay (e.g. auth checks) cannot be skipped
   optional<dp::microseconds>     subjective_cpu_leeway;
   ...
   void pop_block() {
      ...
      DOS_ASSERT( prev, block_validate_exception, "attempt to pop beyond last irreversible block" );
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         DOS_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( dosio, dosio, newaccount );
   SET_APP_HANDLER( dosio, dosio, setcode );
   SET_APP_HANDLER( dosio, dosio, setabi );
   SET_APP_HANDLER( dosio, dosio, updateauth );
   SET_APP_HANDLER( dosio, dosio, deleteauth );
   SET_APP_HANDLER( dosio, dosio, linkauth );
   SET_APP_HANDLER( dosio, dosio, unlinkauth );
/*
   SET_APP_HANDLER( dosio, dosio, postrecovery );
   SET_APP_HANDLER( dosio, dosio, passrecovery );
   SET_APP_HANDLER( dosio, dosio, vetorecovery );
*/

   SET_APP_HANDLER( dosio, dosio, canceldelay );
   ...
   void on_irreversible( const block_state_ptr& s ) {
      ...
      if (!log_head) {
         if (s->block) {
            DOS_ASSERT(s->block_num == blog.first_block_num(), block_log_exception, "block log has no blocks and is appending the wrong first block.  Expected ${expected}, but received: ${actual}",
                      ("expected", blog.first_block_num())("actual", s->block_num));
            append_to_blog = true;
         } else {
            DOS_ASSERT(s->block_num == blog.first_block_num() - 1, block_log_exception, "block log has no blocks and is not properly set up to start after the snapshot");
         }
      } else {
         auto lh_block_num = log_head->block_num();
         if (s->block_num > lh_block_num) {
            DOS_ASSERT(s->block_num - 1 == lh_block_num, unlinkable_block_exception, "unlinkable block", ("s->block_num", s->block_num)("lh_block_num", lh_block_num));
            DOS_ASSERT(s->block->previous == log_head->id(), unlinkable_block_exception, "irreversible doesn't link to block log head");
            append_to_blog = true;
         }
      }
      ...
      if (s->block) {
         if (read_mode == db_read_mode::IRREVERSIBLE) {
            ...
            if (!head || s->block_num == head->block_num + 1) {
               ...
               DOS_ASSERT(!head || head->block_num == 1, block_validate_exception, "Attempting to re-apply an irreversible block that was not the implied genesis block");
            }
   ...
   void init(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      ...
      if (snapshot) {
         DOS_ASSERT( !head, fork_database_exception, "" );
         snapshot->validate();
         ...
         if( !end ) {
            ...
            DOS_ASSERT( end->block_num() == head->block_num, fork_database_exception,
      ...
      if( objitr != ubi.rend() ) {
         DOS_ASSERT( objitr->blocknum == head->block_num, fork_database_exception,
         ...
         auto end = blog.read_head();
         DOS_ASSERT( !end || end->block_num() == head->block_num, fork_database_exception,
                    "fork database exists but reversible block database does not, replay blockchain",
                    ("blog_head",end->block_num())("head",head->block_num)  );
      }

      DOS_ASSERT( db.revision() >= head->block_num, fork_database_exception, "fork database is inconsistent with shared memory",
   ...
   void clear_all_undo() {
      ...
      db.with_write_lock([&] {
         db.undo_all();
         /*
         DP_ASSERT(db.revision() == self.head_block_num(),
   ...
   void initialize_fork_db() {
      ...
      genheader.pending_schedule_hash = dp::sha256::hash(initial_schedule);
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(dosio_contract_abi(abi_def()));
         }
      });
   ...
   void commit_block( bool add_to_fork_db ) {
      auto reset_pending_on_exit = dp::make_scoped_exit([this]{
         pending.reset();
      });
      ...
      try {
         if (add_to_fork_db) {
            ...
            head = fork_db.head();
            DOS_ASSERT(new_bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
   ...
   dp::scoped_exit<std::function<void()>> make_block_restore_point() {
      ...
      return dp::make_scoped_exit( std::move(callback) );
   }

   transaction_trace_ptr apply_onerror( const generated_transaction& gtrx,
                                        dp::time_point deadline,
                                        dp::time_point start,
                                      ...
                                      )
   {
      ...
      etrx.expiration = self.pending_block_time() + dp::microseconds(999'999); // Round up to avoid appearing expired
      ...
      try {
         ...
         dp::move_append( pending->_actions, move(trx_context.executed) );

         trx_context.squash();
         restore.cancel();
         return trace;
      } catch( const dp::exception& e ) {
         cpu_time_to_bill_us = trx_context.update_billed_cpu_time( dp::time_point::now() );
         trace->except = e;
   ...
   bool failure_is_subjective( const dp::exception& e ) const {
      auto code = e.code();
   ...
   bool scheduled_failure_is_subjective( const dp::exception& e ) const {
      auto code = e.code();
      return    (code == tx_cpu_usage_exceeded::code_value)
             || failure_is_subjective(e);
   }

   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, dp::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      const auto& idx = db.get_index<generated_transaction_multi_index,by_trx_id>();
      auto itr = idx.find( trxid );
      DOS_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
      return push_scheduled_transaction( *itr, deadline, billed_cpu_time_us, explicit_billed_cpu_time );
   }

   transaction_trace_ptr push_scheduled_transaction( const generated_transaction_object& gto, dp::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false )
   { try {
      ...
      dp::datastream<const char*> ds( gtrx.packed_trx.data(), gtrx.packed_trx.size() );

      DOS_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
                 ("gtrx.delay_until",gtrx.delay_until)("pbt",self.pending_block_time())          );

      signed_transaction dtrx;
      dp::raw::unpack(ds,static_cast<transaction&>(dtrx) );
      ...
      auto reset_in_trx_requiring_checks = dp::make_scoped_exit([old_value=in_trx_requiring_checks,this](){
      ...
      trx_context.leeway =  dp::microseconds(0); // avoid stealing cpu resource
      trx_context.deadline = deadline;
      trx_context.explicit_billed_cpu_time = explicit_billed_cpu_time;
      trx_context.billed_cpu_time_us = billed_cpu_time_us;
      ...
      try {
         ...
         trace->except = e;
         trace->except_ptr = std::current_exception();
         trace->elapsed = dp::time_point::now() - trx_context.start;
      }
      trx_context.undo();
      ...
      return trace;
   } DP_CAPTURE_AND_RETHROW() } /// push_scheduled_transaction
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      DOS_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      DOS_ASSERT(deadline != dp::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         try {
            ...
            trx_context.delay = dp::seconds(trx->trx.delay_sec);
            ...
            if (!trx->implicit) {
               transaction_receipt::status_enum s = (trx_context.delay == dp::seconds(0))
                                                    ? transaction_receipt::executed
   ...
   {
      DOS_ASSERT( !pending, block_validate_exception, "pending block already exists" );

      auto guard_pending = dp::make_scoped_exit([this](){
         pending.reset();
      });

      if (!self.skip_db_sessions(s)) {
         DOS_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete ) {
         ...
         try {
            ...
            in_trx_requiring_checks = true;
            push_transaction( onbtrx, dp::time_point::maximum(), self.get_global_properties().configuration.min_transaction_cpu_usage, true );
   ...
   void apply_block( const signed_block_ptr& b, controller::block_status s ) { try {
      try {
         DOS_ASSERT( b->block_extensions.size() == 0, block_validate_exception, "no supported extensions" );
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               trace = push_transaction( packed_transactions.at(packed_idx++), dp::time_point::maximum(), receipt.cpu_usage_us, true );
            } else if( receipt.trx.contains<transaction_id_type>() ) {
               trace = push_scheduled_transaction( receipt.trx.get<transaction_id_type>(), dp::time_point::maximum(), receipt.cpu_usage_us, true );
            } else {
               DOS_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
            ...
            DOS_ASSERT( pending->_pending_block_state->block->transactions.size() > 0,
                        block_validate_exception, "expected a receipt",
                        ("block", *b)("expected_receipt", receipt)
                      );
            DOS_ASSERT( pending->_pending_block_state->block->transactions.size() == num_pending_receipts + 1,
            ...
            DOS_ASSERT( r == static_cast<const transaction_receipt_header&>(receipt),
         ...
         DOS_ASSERT(producer_block_id == pending->_pending_block_state->header.id(),
   ...
   } DP_CAPTURE_AND_RETHROW() } /// apply_block

   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      DOS_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      DOS_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block( b->previous );
      DOS_ASSERT( prev, unlinkable_block_exception, "unlinkable block ${id}", ("id", id)("previous", b->previous) );
   ...
   void push_block( std::future<block_state_ptr>& block_state_future ) {
      ...
      DOS_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");

      auto reset_prod_light_validation = dp::make_scoped_exit([old_value=trusted_producer_light_validation, this]() {
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         DOS_ASSERT( b, block_validate_exception, "trying to push empty block" );
         DOS_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
   ...
   void maybe_switch_forks( controller::block_status s ) {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         DOS_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
                     "loss of sync between fork_db and chainbase during fork switch" ); // _should_ never fail

         for( auto ritr = branches.first.rbegin(); ritr != branches.first.rend(); ++ritr ) {
            optional<dp::exception> except;
            try {
            ...
            catch (const dp::exception& e) { except = e; }
            if (except) {
   ...
   void finalize_block()
   {
      DOS_ASSERT(pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      try {
      ...
      */
      ...
      uint64_t CPU_TARGET = DOS_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {DOS_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, max_virtual_mult, {99, 100}, {1000, 999}}
      );
   ...
   void clear_expired_input_transactions() {
      ...
      while( (!dedupe_index.empty()) && ( now > dp::time_point(dedupe_index.begin()->expiration) ) ) {
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         DOS_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         DOS_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         DOS_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         DOS_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         DOS_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         DOS_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      DOS_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
   ...
   */
   ...
   signed_transaction get_on_block_transaction()
   {
      ...
      on_block_act.data = dp::raw::pack(self.head_block_header());

      signed_transaction trx;
      trx.actions.emplace_back(std::move(on_block_act));
      trx.set_reference_block(self.head_block_id());
      trx.expiration = self.pending_block_time() + dp::microseconds(999'999); // Round up to nearest second to avoid appearing expired
      return trx;
   }
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, dp::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   DOS_ASSERT( get_read_mode() != chain::db_read_mode::READ_ONLY, transaction_type_exception, "push transaction not allowed in read-only mode" );
   DOS_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
   return my->push_transaction(trx, deadline, billed_cpu_time_us, billed_cpu_time_us > 0 );
}

transaction_trace_ptr controller::push_scheduled_transaction( const transaction_id_type& trxid, dp::time_point deadline, uint32_t billed_cpu_time_us )
{
   validate_db_available_size();
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      DOS_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      DOS_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   DOS_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
} DP_CAPTURE_AND_RETHROW( (block_num) ) }
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   DOS_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   DOS_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
vector<transaction_metadata_ptr> controller::get_unapplied_transactions() const {
   ...
   if ( my->read_mode == db_read_mode::SPECULATIVE ) {
      ...
      DOS_ASSERT( my->unapplied_transactions.empty(), transaction_exception, "not empty unapplied_transactions in non-speculative mode" ); //should never happen
   }
   return result;
}
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   DOS_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   DOS_ASSERT( time_point(trx.expiration) <= pending_block_time() + dp::seconds(chain_configuration.max_transaction_lifetime),
               tx_exp_too_far_exception,
               "Transaction expiration is too far in the future relative to the reference time of ${reference_time}, "
               "expiration is ${trx.expiration} and the maximum transaction lifetime is ${max_til_exp} seconds",
               ("trx.expiration",trx.expiration)("reference_time",pending_block_time())
               ("max_til_exp",chain_configuration.max_transaction_lifetime) );
} DP_CAPTURE_AND_RETHROW((trx)) }
...
void controller::validate_db_available_size() const {
   ...
   DOS_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   DOS_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::set_subjective_cpu_leeway(dp::microseconds leeway) {
