#include <chain/include/snapshot.hpp>
#include <chain/include/exceptions.hpp>
#include <dp/scoped_exit.hpp>

namespace dosio { namespace chain {

variant_snapshot_writer::variant_snapshot_writer(dp::mutable_variant_object& snapshot)
: snapshot(snapshot)
{
   snapshot.set("sections", dp::variants());
...
void variant_snapshot_writer::write_end_section( ) {
   snapshot["sections"].get_array().emplace_back(dp::mutable_variant_object()("name", std::move(current_section_name))("rows", std::move(current_rows)));
}
...
variant_snapshot_reader::variant_snapshot_reader(const dp::variant& snapshot)
:snapshot(snapshot)
,cur_row(0)
{
}
...
void variant_snapshot_reader::validate() const {
   DOS_ASSERT(snapshot.is_object(), snapshot_validation_exception,
         "Variant snapshot is not an object");
   const dp::variant_object& o = snapshot.get_object();

   DOS_ASSERT(o.contains("version"), snapshot_validation_exception,
         "Variant snapshot has no version");

   const auto& version = o["version"];
   DOS_ASSERT(version.is_integer(), snapshot_validation_exception,
         "Variant snapshot version is not an integer");

   DOS_ASSERT(version.as_uint64() == (uint64_t)current_snapshot_version, snapshot_validation_exception,
   ...
   DOS_ASSERT(o.contains("sections"), snapshot_validation_exception,
         "Variant snapshot has no sections");

   const auto& sections = o["sections"];
   DOS_ASSERT(sections.is_array(), snapshot_validation_exception, "Variant snapshot sections is not an array");

   const auto& section_array = sections.get_array();
   for( const auto& section: section_array ) {
      DOS_ASSERT(section.is_object(), snapshot_validation_exception, "Variant snapshot section is not an object");

      const auto& so = section.get_object();
      DOS_ASSERT(so.contains("name"), snapshot_validation_exception,
            "Variant snapshot section has no name");

      DOS_ASSERT(so["name"].is_string(), snapshot_validation_exception,
                 "Variant snapshot section name is not a string");

      DOS_ASSERT(so.contains("rows"), snapshot_validation_exception,
                 "Variant snapshot section has no rows");

      DOS_ASSERT(so["rows"].is_array(), snapshot_validation_exception,
...
void variant_snapshot_reader::set_section( const string& section_name ) {
   ...
   DOS_THROW(snapshot_exception, "Variant snapshot has no section named ${n}", ("n", section_name));
}
...
{
   DOS_ASSERT(section_pos == std::streampos(-1), snapshot_exception, "Attempting to write a new section without closing the previous section");
...
void istream_snapshot_reader::validate() const {
   ...
   auto restore_pos = dp::make_scoped_exit([this,pos=snapshot.tellg(),ex=snapshot.exceptions()](){
      snapshot.seekg(pos);
   ...
   try {
      ...
      DOS_ASSERT(actual_totem == expected_totem, snapshot_exception,
      ...
      DOS_ASSERT(actual_version == expected_version, snapshot_exception,
                 "Binary snapshot is an unsuppored version.  Expected : ${expected}, Got: ${actual}",
                 ("expected", expected_version)("actual", actual_version));

      while (validate_section()) {}
   } catch( const std::exception& e ) {  \
      snapshot_exception dpe(DP_LOG_MESSAGE( warn, "Binary snapshot validation threw IO exception (${what})",("what",e.what())));
      throw dpe;
   }
}
...
bool istream_snapshot_reader::has_section( const string& section_name ) {
   auto restore_pos = dp::make_scoped_exit([this,pos=snapshot.tellg()](){
      snapshot.seekg(pos);
   });
...
void istream_snapshot_reader::set_section( const string& section_name ) {
   ...
   DOS_THROW(snapshot_exception, "Binary snapshot has no section named ${n}", ("n", section_name));
}
...
integrity_hash_snapshot_writer::integrity_hash_snapshot_writer(dp::sha256::encoder& enc)
:enc(enc)
{
}
