...
namespace dosio { namespace chain {
   ...
   fork_database::fork_database( const dp::path& data_dir ):my( new fork_database_impl() ) {
      my->datadir = data_dir;

      if (!dp::is_directory(my->datadir))
         dp::create_directories(my->datadir);

      auto fork_db_dat = my->datadir / config::forkdb_filename;
      if( dp::exists( fork_db_dat ) ) {
         string content;
         dp::read_file_contents( fork_db_dat, content );

         dp::datastream<const char*> ds( content.data(), content.size() );
         unsigned_int size; dp::raw::unpack( ds, size );
         ...
         dp::raw::unpack( ds, head_id );

         my->head = get_block( head_id );

         dp::remove( fork_db_dat );
      }
   }
   ...
   void fork_database::close() {
      ...
      dp::raw::pack( out, unsigned_int{num_blocks_in_fork_db} );
      for( const auto& s : my->index ) {
         dp::raw::pack( out, *s );
      }
      if( my->head )
         dp::raw::pack( out, my->head->id );
      else
         dp::raw::pack( out, block_id_type() );
   ...
   void fork_database::set( block_state_ptr s ) {
      ...
      DOS_ASSERT( s->id == s->header.id(), fork_database_exception, 
                  "block state id (${id}) is different from block state header id (${hid})", ("id", string(s->id))("hid", string(s->header.id())) );

         //DP_ASSERT( s->block_num == s->header.block_num() );

      DOS_ASSERT( result.second, fork_database_exception, "unable to insert block state, duplicate state detected" );
      if( !my->head ) {
         my->head =  s;
   ...
   block_state_ptr fork_database::add( const block_state_ptr& n, bool skip_validate_previous ) {
      DOS_ASSERT( n, fork_database_exception, "attempt to add null block state" );
      DOS_ASSERT( my->head, fork_db_block_not_found, "no head block set" );

      if( !skip_validate_previous ) {
         auto prior = my->index.find( n->block->previous );
         DOS_ASSERT( prior != my->index.end(), unlinkable_block_exception,
                     "unlinkable block", ("id", n->block->id())("previous", n->block->previous) );
      }

      auto inserted = my->index.insert(n);
      DOS_ASSERT( inserted.second, fork_database_exception, "duplicate block added?" );
   ...
   block_state_ptr fork_database::add( signed_block_ptr b, bool skip_validate_signee ) {
      DOS_ASSERT( b, fork_database_exception, "attempt to add null block" );
      DOS_ASSERT( my->head, fork_db_block_not_found, "no head block set" );

      const auto& by_id_idx = my->index.get<by_block_id>();
      auto existing = by_id_idx.find( b->id() );
      DOS_ASSERT( existing == by_id_idx.end(), fork_database_exception, "we already know about this block" );

      auto prior = by_id_idx.find( b->previous );
      DOS_ASSERT( prior != by_id_idx.end(), unlinkable_block_exception, "unlinkable block", ("id", string(b->id()))("previous", string(b->previous)) );

      auto result = std::make_shared<block_state>( **prior, move(b), skip_validate_signee );
      DOS_ASSERT( result, fork_database_exception , "fail to add new block state" );
   ...
   pair< branch_type, branch_type >  fork_database::fetch_branch_from( const block_id_type& first,
      ...
      while( first_branch->block_num > second_branch->block_num )
      {
         ...
         DOS_ASSERT( first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(first_branch->header.previous)) );
      }
      ...
      while( second_branch->block_num > first_branch->block_num )
      {
         ...
         DOS_ASSERT( second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(second_branch->header.previous)) );
      }
      ...
      while( first_branch->header.previous != second_branch->header.previous )
      {
         ...
         first_branch = get_block( first_branch->header.previous );
         second_branch = get_block( second_branch->header.previous );
         DOS_ASSERT( first_branch && second_branch, fork_db_block_not_found, 
   ...
   void fork_database::mark_in_current_chain( const block_state_ptr& h, bool in_current_chain ) {
      ...
      DOS_ASSERT( itr != by_id_idx.end(), fork_db_block_not_found, "could not find block in fork database" );
   ...
   block_state_ptr   fork_database::get_block_in_current_chain_by_num( uint32_t n )const {
      ...
      //DP_ASSERT( nitr != numidx.end() && (*nitr)->block_num == n,
      //           "could not find block in fork database with block number ${block_num}", ("block_num", n) );
      //DP_ASSERT( (*nitr)->in_current_chain == true,
   ...
   void fork_database::add( const header_confirmation& c ) {
      ...
      DOS_ASSERT( b, fork_db_block_not_found, "unable to find block id ${id}", ("id",c.block_id));
