...
#include <chain/include/wast_to_wasm.hpp>
...
namespace dosio { namespace chain {
   ...
   { 
      ...
      if(parse_errors.size())
      {
         ...
         DOS_ASSERT( false, wasm_exception, "error parsing wast: ${msg}", ("msg",ss.str()) );
      }
      ...
      catch(const Serialization::FatalSerializationException& exception)
      {
         ...
         DOS_ASSERT( false, wasm_exception, "error converting to wasm: ${msg}", ("msg",ss.get()) );
   ...
   } DP_CAPTURE_AND_RETHROW( (wast) ) }  /// wast_to_wasm
   ...
   } DP_CAPTURE_AND_RETHROW() } /// wasm_to_wast
