#include <chain/include/wasm_interface.hpp>
#include <chain/include/apply_context.hpp>
#include <chain/include/controller.hpp>
#include <chain/include/transaction_context.hpp>
#include <chain/include/producer_schedule.hpp>
#include <chain/include/exceptions.hpp>
#include <boost/core/ignore_unused.hpp>
#include <chain/include/authorization_manager.hpp>
#include <chain/include/resource_limits.hpp>
#include <chain/include/wasm_interface_private.hpp>
#include <chain/include/wasm_dosio_validation.hpp>
#include <chain/include/wasm_dosio_injection.hpp>
#include <chain/include/global_property_object.hpp>
#include <chain/include/account_object.hpp>
...
namespace dosio { namespace chain {
   ...
   void wasm_interface::validate(const controller& control, const bytes& code) {
      ...
      try {
         ...
         DOS_ASSERT(false, wasm_serialization_error, e.message.c_str());
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.context_free )
            DOS_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         DOS_ASSERT( context.context_free, unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         DOS_ASSERT( context.privileged, unaccessible_api, "${code} does not have permission to call this API", ("code",context.receiver) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         DOS_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         DOS_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         DOS_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         DOS_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      int64_t set_proposed_producers( array_ptr<char> packed_producer_schedule, size_t datalen) {
         ...
         dp::raw::unpack(ds, producers);
         DOS_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         // check that producers are unique
         std::set<account_name> unique_producers;
         for (const auto& p: producers) {
            DOS_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            DOS_ASSERT( p.block_signing_key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
            unique_producers.insert(p.producer_name);
         }
         DOS_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
      ...
      uint32_t get_blockchain_parameters_packed( array_ptr<char> packed_blockchain_parameters, size_t buffer_size) {
         ...
         auto s = dp::raw::pack_size( gpo.configuration );
         if( buffer_size == 0 ) return s;

         if ( s <= buffer_size ) {
            datastream<char*> ds( packed_blockchain_parameters, s );
            dp::raw::pack(ds, gpo.configuration);
            return s;
         }
         return 0;
      }
...
class softfloat_api : public context_aware_api {
   public:
      ...
      float _dosio_f32_max( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a)) {
            return af;
      ...
      float _dosio_f32_copysign( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
      ...
      // float unops
      float _dosio_f32_abs( float af ) {
         float32_t a = to_softfloat32(af);
         a.v &= ~(1 << 31);
         return from_softfloat32(a);
      }
      float _dosio_f32_neg( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _dosio_f32_floor( float af ) {
         float32_t a = to_softfloat32(af);
         int e = (int)(a.v >> 23 & 0xFF) - 0X7F;
         uint32_t m;
         if (e >= 23)
      ...
      float _dosio_f32_trunc( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _dosio_f32_nearest( float af ) {
         float32_t a = to_softfloat32(af);
         int e = a.v>>23 & 0xff;
         int s = a.v>>31;
         float32_t y;
      ...
      // float relops
      bool _dosio_f32_eq( float a, float b ) {  return f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _dosio_f32_ne( float a, float b ) { return !f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _dosio_f32_lt( float a, float b ) { return f32_lt( to_softfloat32(a), to_softfloat32(b) ); }
      bool _dosio_f32_le( float a, float b ) { return f32_le( to_softfloat32(a), to_softfloat32(b) ); }
      bool _dosio_f32_gt( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      bool _dosio_f32_ge( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      double _dosio_f64_max( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return af;
      ...
      double _dosio_f64_copysign( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
      ...
      // double unops
      double _dosio_f64_abs( double af ) {
         float64_t a = to_softfloat64(af);
         a.v &= ~(uint64_t(1) << 63);
         return from_softfloat64(a);
      }
      double _dosio_f64_neg( double af ) {
         float64_t a = to_softfloat64(af);
      ...
      double _dosio_f64_sqrt( double a ) {
         float64_t ret = f64_sqrt( to_softfloat64(a) );
         return from_softfloat64(ret);
      }
      // ceil, floor, trunc and nearest are lifted from libc
      double _dosio_f64_ceil( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7ff;
         float64_t y;
      ...
      double _dosio_f64_floor( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7FF;
         float64_t y;
      ...
      double _dosio_f64_trunc( double af ) {
         float64_t a = to_softfloat64( af );
      ...
      double _dosio_f64_nearest( double af ) {
         float64_t a = to_softfloat64( af );
         int e = (a.v >> 52 & 0x7FF);
         int s = a.v >> 63;
         float64_t y;
      ...
      // double relops
      bool _dosio_f64_eq( double a, double b ) { return f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _dosio_f64_ne( double a, double b ) { return !f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _dosio_f64_lt( double a, double b ) { return f64_lt( to_softfloat64(a), to_softfloat64(b) ); }
      bool _dosio_f64_le( double a, double b ) { return f64_le( to_softfloat64(a), to_softfloat64(b) ); }
      bool _dosio_f64_gt( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      bool _dosio_f64_ge( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      double _dosio_f32_promote( float a ) {
         return from_softfloat64(f32_to_f64( to_softfloat32(a)) );
      }
      float _dosio_f64_demote( double a ) {
         return from_softfloat32(f64_to_f32( to_softfloat64(a)) );
      }
      int32_t _dosio_f32_trunc_i32s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_dosio_f32_ge(af, 2147483648.0f) || _dosio_f32_lt(af, -2147483648.0f))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
         return f32_to_i32( to_softfloat32(_dosio_f32_trunc( af )), 0, false );
      }
      int32_t _dosio_f64_trunc_i32s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_dosio_f64_ge(af, 2147483648.0) || _dosio_f64_lt(af, -2147483648.0))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
         return f64_to_i32( to_softfloat64(_dosio_f64_trunc( af )), 0, false );
      }
      uint32_t _dosio_f32_trunc_i32u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_dosio_f32_ge(af, 4294967296.0f) || _dosio_f32_le(af, -1.0f))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
         return f32_to_ui32( to_softfloat32(_dosio_f32_trunc( af )), 0, false );
      }
      uint32_t _dosio_f64_trunc_i32u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_dosio_f64_ge(af, 4294967296.0) || _dosio_f64_le(af, -1.0))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
         return f64_to_ui32( to_softfloat64(_dosio_f64_trunc( af )), 0, false );
      }
      int64_t _dosio_f32_trunc_i64s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_dosio_f32_ge(af, 9223372036854775808.0f) || _dosio_f32_lt(af, -9223372036854775808.0f))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
         return f32_to_i64( to_softfloat32(_dosio_f32_trunc( af )), 0, false );
      }
      int64_t _dosio_f64_trunc_i64s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_dosio_f64_ge(af, 9223372036854775808.0) || _dosio_f64_lt(af, -9223372036854775808.0))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");

         return f64_to_i64( to_softfloat64(_dosio_f64_trunc( af )), 0, false );
      }
      uint64_t _dosio_f32_trunc_i64u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_dosio_f32_ge(af, 18446744073709551616.0f) || _dosio_f32_le(af, -1.0f))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
         return f32_to_ui64( to_softfloat32(_dosio_f32_trunc( af )), 0, false );
      }
      uint64_t _dosio_f64_trunc_i64u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_dosio_f64_ge(af, 18446744073709551616.0) || _dosio_f64_le(af, -1.0))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            DP_THROW_EXCEPTION( dosio::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
         return f64_to_ui64( to_softfloat64(_dosio_f64_trunc( af )), 0, false );
      }
      float _dosio_i32_to_f32( int32_t a )  {
         return from_softfloat32(i32_to_f32( a ));
      }
      float _dosio_i64_to_f32( int64_t a ) {
         return from_softfloat32(i64_to_f32( a ));
      }
      float _dosio_ui32_to_f32( uint32_t a ) {
         return from_softfloat32(ui32_to_f32( a ));
      }
      float _dosio_ui64_to_f32( uint64_t a ) {
         return from_softfloat32(ui64_to_f32( a ));
      }
      double _dosio_i32_to_f64( int32_t a ) {
         return from_softfloat64(i32_to_f64( a ));
      }
      double _dosio_i64_to_f64( int64_t a ) {
         return from_softfloat64(i64_to_f64( a ));
      }
      double _dosio_ui32_to_f64( uint32_t a ) {
         return from_softfloat64(ui32_to_f64( a ));
      }
      double _dosio_ui64_to_f64( uint64_t a ) {
...
class crypto_api : public context_aware_api {
   public:
       ...
       */
      void assert_recover_key( const dp::sha256& digest,
                        array_ptr<char> sig, size_t siglen,
                        array_ptr<char> pub, size_t publen ) {
         dp::crypto::signature s;
         dp::crypto::public_key p;
         datastream<const char*> ds( sig, siglen );
         datastream<const char*> pubds( pub, publen );

         dp::raw::unpack(ds, s);
         dp::raw::unpack(pubds, p);

         auto check = dp::crypto::public_key( s, digest, false );
         DOS_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
      }

      int recover_key( const dp::sha256& digest,
                        array_ptr<char> sig, size_t siglen,
                        array_ptr<char> pub, size_t publen ) {
         dp::crypto::signature s;
         datastream<const char*> ds( sig, siglen );
         ...
         dp::raw::unpack(ds, s);
         dp::raw::pack( pubds, dp::crypto::public_key( s, digest, false ) );
         return pubds.tellp();
      }

      template<class Encoder> auto encode(char* data, size_t datalen) {
         Encoder e;
         const size_t bs = dosio::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
      ...
      void assert_sha256(array_ptr<char> data, size_t datalen, const dp::sha256& hash_val) {
         auto result = encode<dp::sha256::encoder>( data, datalen );
         DOS_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }

      void assert_sha1(array_ptr<char> data, size_t datalen, const dp::sha1& hash_val) {
         auto result = encode<dp::sha1::encoder>( data, datalen );
         DOS_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }

      void assert_sha512(array_ptr<char> data, size_t datalen, const dp::sha512& hash_val) {
         auto result = encode<dp::sha512::encoder>( data, datalen );
         DOS_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }

      void assert_ripemd160(array_ptr<char> data, size_t datalen, const dp::ripemd160& hash_val) {
         auto result = encode<dp::ripemd160::encoder>( data, datalen );
         DOS_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }

      void sha1(array_ptr<char> data, size_t datalen, dp::sha1& hash_val) {
         hash_val = encode<dp::sha1::encoder>( data, datalen );
      }

      void sha256(array_ptr<char> data, size_t datalen, dp::sha256& hash_val) {
         hash_val = encode<dp::sha256::encoder>( data, datalen );
      }

      void sha512(array_ptr<char> data, size_t datalen, dp::sha512& hash_val) {
         hash_val = encode<dp::sha512::encoder>( data, datalen );
      }

      void ripemd160(array_ptr<char> data, size_t datalen, dp::ripemd160& hash_val) {
         hash_val = encode<dp::ripemd160::encoder>( data, datalen );
      }
};
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         transaction trx = dp::raw::unpack<transaction>( trx_data, trx_size );
      ...
      {
         DOS_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
      ...
      int64_t get_account_creation_time( account_name account ) {
         ...
         DOS_ASSERT( acct != nullptr, action_validate_exception,
   ...
   private:
      void unpack_provided_keys( flat_set<public_key_type>& keys, const char* pubkeys_data, size_t pubkeys_size ) {
         ...
         keys = dp::raw::unpack<flat_set<public_key_type>>( pubkeys_data, pubkeys_size );
      }
      ...
      void unpack_provided_permissions( flat_set<permission_level>& permissions, const char* perms_data, size_t perms_size ) {
         ...
         permissions = dp::raw::unpack<flat_set<permission_level>>( perms_data, perms_size );
      }
...
public:
   ...
   void abort() {
      DOS_ASSERT( false, abort_called, "abort() called");
   }

   // Kept as intrinsic rather than implementing on WASM side (using dosio_assert_message and strlen) because strlen is faster on native side.
   void dosio_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         std::string message( msg );
         DOS_THROW( dosio_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void dosio_assert_message( bool condition, array_ptr<const char> msg, size_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void dosio_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         DOS_THROW( dosio_assert_code_exception,
                    "assertion failure with error code: ${error_code}", ("error_code", error_code) );
      }
   }

   void dosio_exit(int32_t code) {
   ...
   public:
      ...
      void printi128(const __int128& val) {
         if ( !ignore ) {
            ...
            dp::uint128_t v(val_magnitude>>64, static_cast<uint64_t>(val_magnitude) );

            if( is_negative ) {
               context.console_append("-");
            }

            context.console_append(dp::variant(v).get_string());
         }
      }

      void printui128(const unsigned __int128& val) {
         if ( !ignore ) {
            dp::uint128_t v(val>>64, static_cast<uint64_t>(val) );
            context.console_append(dp::variant(v).get_string());
         }
      }
      ...
      void printhex(array_ptr<const char> data, size_t data_len ) {
         if ( !ignore ) {
            context.console_append(dp::to_hex(data, data_len));
         }
      }
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         DOS_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, payer, id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         DOS_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, size_t length) {
         DOS_ASSERT((std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, size_t data_len ) {
         ...
         DOS_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
                    "inline action too big" );

         action act;
         dp::raw::unpack<action>(data, data_len, act);
      ...
      void send_deferred( const uint128_t& sender_id, account_name payer, array_ptr<char> data, size_t data_len, uint32_t replace_existing) {
         transaction trx;
         dp::raw::unpack<transaction>(data, data_len, trx);
         context.schedule_deferred_transaction(sender_id, payer, std::move(trx), replace_existing);
      }

      bool cancel_deferred( const unsigned __int128& val ) {
         dp::uint128_t sender_id(val>>64, uint64_t(val) );
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         DOS_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
...
/*
 * This api will be removed with fix for `dos #2561`
 */
class call_depth_api : public context_aware_api {
   public:
      ...
      void call_depth_assert() {
         DP_THROW_EXCEPTION(wasm_execution_error, "Exceeded call depth maximum");
      }
};
...
REGISTER_INTRINSICS(context_free_system_api,
   (abort,                void()              )
   (dosio_assert,         void(int, int)      )
   (dosio_assert_message, void(int, int, int) )
   (dosio_assert_code,    void(int, int64_t)  )
   (dosio_exit,           void(int)           )
);
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      (_dosio_f32_add,       float(float, float)    )
      (_dosio_f32_sub,       float(float, float)    )
      (_dosio_f32_mul,       float(float, float)    )
      (_dosio_f32_div,       float(float, float)    )
      (_dosio_f32_min,       float(float, float)    )
      (_dosio_f32_max,       float(float, float)    )
      (_dosio_f32_copysign,  float(float, float)    )
      (_dosio_f32_abs,       float(float)           )
      (_dosio_f32_neg,       float(float)           )
      (_dosio_f32_sqrt,      float(float)           )
      (_dosio_f32_ceil,      float(float)           )
      (_dosio_f32_floor,     float(float)           )
      (_dosio_f32_trunc,     float(float)           )
      (_dosio_f32_nearest,   float(float)           )
      (_dosio_f32_eq,        int(float, float)      )
      (_dosio_f32_ne,        int(float, float)      )
      (_dosio_f32_lt,        int(float, float)      )
      (_dosio_f32_le,        int(float, float)      )
      (_dosio_f32_gt,        int(float, float)      )
      (_dosio_f32_ge,        int(float, float)      )
      (_dosio_f64_add,       double(double, double) )
      (_dosio_f64_sub,       double(double, double) )
      (_dosio_f64_mul,       double(double, double) )
      (_dosio_f64_div,       double(double, double) )
      (_dosio_f64_min,       double(double, double) )
      (_dosio_f64_max,       double(double, double) )
      (_dosio_f64_copysign,  double(double, double) )
      (_dosio_f64_abs,       double(double)         )
      (_dosio_f64_neg,       double(double)         )
      (_dosio_f64_sqrt,      double(double)         )
      (_dosio_f64_ceil,      double(double)         )
      (_dosio_f64_floor,     double(double)         )
      (_dosio_f64_trunc,     double(double)         )
      (_dosio_f64_nearest,   double(double)         )
      (_dosio_f64_eq,        int(double, double)    )
      (_dosio_f64_ne,        int(double, double)    )
      (_dosio_f64_lt,        int(double, double)    )
      (_dosio_f64_le,        int(double, double)    )
      (_dosio_f64_gt,        int(double, double)    )
      (_dosio_f64_ge,        int(double, double)    )
      (_dosio_f32_promote,    double(float)         )
      (_dosio_f64_demote,     float(double)         )
      (_dosio_f32_trunc_i32s, int(float)            )
      (_dosio_f64_trunc_i32s, int(double)           )
      (_dosio_f32_trunc_i32u, int(float)            )
      (_dosio_f64_trunc_i32u, int(double)           )
      (_dosio_f32_trunc_i64s, int64_t(float)        )
      (_dosio_f64_trunc_i64s, int64_t(double)       )
      (_dosio_f32_trunc_i64u, int64_t(float)        )
      (_dosio_f64_trunc_i64u, int64_t(double)       )
      (_dosio_i32_to_f32,     float(int32_t)        )
      (_dosio_i64_to_f32,     float(int64_t)        )
      (_dosio_ui32_to_f32,    float(int32_t)        )
      (_dosio_ui64_to_f32,    float(int64_t)        )
      (_dosio_i32_to_f64,     double(int32_t)       )
      (_dosio_i64_to_f64,     double(int64_t)       )
      (_dosio_ui32_to_f64,    double(int32_t)       )
      (_dosio_ui64_to_f64,    double(int64_t)       )
);
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wavm")
      runtime = chain::include::wasm_interface::vm_type::wavm;
   else if (s == "wabt")
      runtime = chain::include::wasm_interface::vm_type::wabt;
   else
