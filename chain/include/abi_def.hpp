...
datastream<ST>& operator << (datastream<ST>& s, const dosio::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
datastream<ST>& operator >> (datastream<ST>& s, dosio::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const dosio::chain::may_not_exist<T>& e, dp::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const dp::variant& v, dosio::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}

} // namespace dp

DP_REFLECT( dosio::chain::type_def                         , (new_type_name)(type) )
DP_REFLECT( dosio::chain::field_def                        , (name)(type) )
DP_REFLECT( dosio::chain::struct_def                       , (name)(base)(fields) )
DP_REFLECT( dosio::chain::action_def                       , (name)(type)(ricardian_contract) )
DP_REFLECT( dosio::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
DP_REFLECT( dosio::chain::clause_pair                      , (id)(body) )
DP_REFLECT( dosio::chain::error_message                    , (error_code)(error_msg) )
DP_REFLECT( dosio::chain::variant_def                      , (name)(types) )
DP_REFLECT( dosio::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
