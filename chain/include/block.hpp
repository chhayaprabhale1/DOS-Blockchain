#pragma once
#include <chain/include/block_header.hpp>
...
namespace dosio { namespace chain {
   ...
   struct transaction_receipt_header {
      ...
      dp::enum_type<uint8_t,status_enum>   status;
      uint32_t                             cpu_usage_us; ///< total billed CPU usage (microseconds)
      dp::unsigned_int                     net_usage_words; ///<  total billed NET usage, so we can reconstruct resource state when skipping context free data... hard failures...
   };
   ...
   struct transaction_receipt : public transaction_receipt_header {
      ...
      dp::static_variant<transaction_id_type, packed_transaction> trx;

      digest_type digest()const {
         digest_type::encoder enc;
         dp::raw::pack( enc, status );
         dp::raw::pack( enc, cpu_usage_us );
         dp::raw::pack( enc, net_usage_words );
         if( trx.contains<transaction_id_type>() )
            dp::raw::pack( enc, trx.get<transaction_id_type>() );
         else
            dp::raw::pack( enc, trx.get<packed_transaction>().packed_digest() );
         return enc.result();
      }
   };
...
DP_REFLECT_ENUM(chain::transaction_receipt::status_enum,
