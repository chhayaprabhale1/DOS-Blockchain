...
namespace dosio { namespace chain {
    ...
    *  In the context of dosio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         DOS_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         DOS_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
         return dp::raw::unpack<T>(data);
      }
   };
...
DP_REFLECT( dosio::chain::permission_level, (actor)(permission) )
DP_REFLECT( dosio::chain::action, (account)(name)(authorization)(data) )
