...
#include <dosio/chain/webassembly/wavm.hpp>
#include <dosio/chain/webassembly/wabt.hpp>
#include <dosio/chain/webassembly/runtime_interface.hpp>
#include <dosio/chain/wasm_dosio_injection.hpp>
#include <dosio/chain/transaction_context.hpp>
...
using namespace dp;
using namespace dosio::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
      wasm_interface_impl(wasm_interface::vm_type vm) {
         if(vm == wasm_interface::vm_type::wavm)
            ...
            DOS_THROW(wasm_exception, "wasm_interface_impl fall through");
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            DOS_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            DOS_ASSERT(module.memories.defs.size(), wasm_exception, "");
            ...
            if(base_offset >= memory_size || base_offset + data_segment.data.size() > memory_size)
               DP_THROW_EXCEPTION(wasm_execution_error, "WASM data segment outside of valid memory range");
                                                                                    ...
                                                                                    transaction_context& trx_context )
      {
         ...
         if(it == instantiation_cache.end()) {
            auto timer_pause = dp::make_scoped_exit([&](){
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, DOSIO_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
