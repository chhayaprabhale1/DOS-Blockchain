#pragma once
#include <chain/include/block_state.hpp>
#include <chain/include/trace.hpp>
...
namespace dosio { namespace chain {
   ...
   class controller {
      public:
         ...
         transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx, dp::time_point deadline, uint32_t billed_cpu_time_us = 0 );

         /**
          * Attempt to execute a specific transaction in our deferred trx database
          *
          */
         transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& scheduled, dp::time_point deadline, uint32_t billed_cpu_time_us = 0 );
         ...
         void set_subjective_cpu_leeway(dp::microseconds leeway);
         ...
         */
         ...
         optional<abi_serializer> get_abi_serializer( account_name n, const dp::microseconds& max_serialization_time )const {
            if( n.good() ) {
               try {
         ...
         template<typename T>
         dp::variant to_variant_with_abi( const T& obj, const dp::microseconds& max_serialization_time ) {
...
DP_REFLECT( dosio::chain::controller::config,
            (actor_whitelist)
            (actor_blacklist)
            (contract_whitelist)
            (contract_blacklist)
