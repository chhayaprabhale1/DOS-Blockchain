...
#include <chain/include/parallel_markers.hpp>
...
namespace detail {
   ...
   class authority_checker {
      ...
      public:
         ...
         {
            DOS_ASSERT( static_cast<bool>(checktime), authorization_exception, "checktime cannot be empty" );
         }
         ...
         bool satisfied( const permission_level& permission,
                         dp::microseconds override_provided_delay,
                         permission_cache_type* cached_perms = nullptr
                       )
         {
            auto delay_reverter = dp::make_scoped_exit( [this, delay = provided_delay] () mutable {
      ...
      private:
         ...
         bool satisfied( const AuthorityType& authority, permission_cache_type& cached_permissions, uint16_t depth ) {
            ...
            auto KeyReverter = dp::make_scoped_exit([this, keys = _used_keys] () mutable {
               _used_keys = keys;
            });
         ...
         struct weight_tally_visitor {
            ...
            uint32_t operator()(const wait_weight& permission) {
               if( checker.provided_delay >= dp::seconds(permission.wait_sec) ) {
   ...
   auto make_auth_checker( PermissionToAuthorityFunc&&          pta,
                           ...
                           dp::microseconds                     provided_delay = dp::microseconds(0),
