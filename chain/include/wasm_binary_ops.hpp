...
namespace dosio { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return dosio::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       dosio::chain::wasm_ops::to_string(f); }
};
template <>
...
template <class Op_Types>
struct DOSIO_OperatorDecoderStream
{
   DOSIO_OperatorDecoderStream(const std::vector<U8>& codeBytes)
   ...
   instr* decodeOp() {
      DOS_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      ...
      #define VISIT_OPCODE(opcode,name,nameString,Imm,...) \
         case IR::Opcode::name: \
         { \
            DOS_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_exception, ""); \
...
template <class Op_Types>
const std::vector<instr*>* DOSIO_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace dosio, chain, wasm_ops

DP_REFLECT_TEMPLATE( (typename T), dosio::chain::wasm_ops::block< T >, (code)(rt) )
DP_REFLECT( dosio::chain::wasm_ops::memarg, (a)(o) )
DP_REFLECT( dosio::chain::wasm_ops::blocktype, (result) )
DP_REFLECT( dosio::chain::wasm_ops::memoryoptype, (end) )
