...
namespace dosio { namespace chain {
   ...
   namespace detail {
      ...
      struct abstract_snapshot_row_writer {
         ...
         virtual void write(dp::sha256::encoder& out) const = 0;
      ...
      struct snapshot_row_writer : abstract_snapshot_row_writer {
         ...
         void write(dp::sha256::encoder& out) const override {
            write_stream(out);
         }

         dp::variant to_variant() const override {
            variant var;
            dp::to_variant(data, var);
            return var;
         }
   ...
   namespace detail {
      struct abstract_snapshot_row_reader {
         ...
         virtual void provide(const dp::variant&) const = 0;
         virtual std::string row_type_name() const = 0;
      };
      ...
      struct row_validation_helper {
         ...
         static auto apply(const T& data, F f) -> std::enable_if_t<is_chainbase_object_v<T>> {
            auto orig = data.id;
            f();
            DOS_ASSERT(orig == data.id, snapshot_exception,
      ...
      struct snapshot_row_reader : abstract_snapshot_row_reader {
         ...
         void provide(std::istream& in) const override {
            row_validation_helper::apply(data, [&in,this](){
               dp::raw::unpack(in, data);
            });
         }

         void provide(const dp::variant& var) const override {
            row_validation_helper::apply(data, [&var,this]() {
               dp::from_variant(var, data);
            });
         }
   ...
   class variant_snapshot_writer : public snapshot_writer {
      public:
         variant_snapshot_writer(dp::mutable_variant_object& snapshot);
      ...
      private:
         dp::mutable_variant_object& snapshot;
         std::string current_section_name;
         dp::variants current_rows;
   };

   class variant_snapshot_reader : public snapshot_reader {
      public:
         explicit variant_snapshot_reader(const dp::variant& snapshot);
      ...
      private:
         const dp::variant& snapshot;
         const dp::variant_object* cur_section;
         uint64_t cur_row;
   };
   ...
   class integrity_hash_snapshot_writer : public snapshot_writer {
      public:
         explicit integrity_hash_snapshot_writer(dp::sha256::encoder&  enc);
