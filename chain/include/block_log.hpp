...
namespace dosio { namespace chain {
   ...
   class block_log {
      public:
         block_log(const dp::path& data_dir);
         ...
         static dp::path repair_log( const dp::path& data_dir, uint32_t truncate_at_block = 0 );

         static genesis_state extract_genesis_state( const dp::path& data_dir );

      private:
         void open(const dp::path& data_dir);
