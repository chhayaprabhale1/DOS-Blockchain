...
struct abi_serializer {
   ...
   abi_serializer( const abi_def& abi, const dp::microseconds& max_serialization_time );
   void set_abi(const abi_def& abi, const dp::microseconds& max_serialization_time);
   ...
   bool      is_type(const type_name& type, const dp::microseconds& max_serialization_time)const;
   ...
   dp::variant binary_to_variant( const type_name& type, const bytes& binary, const dp::microseconds& max_serialization_time, bool short_path = false )const;
   dp::variant binary_to_variant( const type_name& type, dp::datastream<const char*>& binary, const dp::microseconds& max_serialization_time, bool short_path = false )const;

   bytes       variant_to_binary( const type_name& type, const dp::variant& var, const dp::microseconds& max_serialization_time, bool short_path = false )const;
   void        variant_to_binary( const type_name& type, const dp::variant& var, dp::datastream<char*>& ds, const dp::microseconds& max_serialization_time, bool short_path = false )const;

   template<typename T, typename Resolver>
   static void to_variant( const T& o, dp::variant& vo, Resolver resolver, const dp::microseconds& max_serialization_time );

   template<typename T, typename Resolver>
   static void from_variant( const dp::variant& v, T& o, Resolver resolver, const dp::microseconds& max_serialization_time );
   ...
   static bool to_abi(const Vec& abi_vec, abi_def& abi)
   {
      if( !is_empty_abi(abi_vec) ) { /// 4 == packsize of empty Abi
         dp::datastream<const char*> ds( abi_vec.data(), abi_vec.size() );
         dp::raw::unpack( ds, abi );
         return true;
      }
      return false;
   }

   typedef std::function<dp::variant(dp::datastream<const char*>&, bool, bool)>  unpack_function;
   typedef std::function<void(const dp::variant&, dp::datastream<char*>&, bool, bool)>  pack_function;
...
private:
   ...
   dp::variant _binary_to_variant( const type_name& type, const bytes& binary, impl::binary_to_variant_context& ctx )const;
   dp::variant _binary_to_variant( const type_name& type, dp::datastream<const char*>& binary, impl::binary_to_variant_context& ctx )const;
   void        _binary_to_variant( const type_name& type, dp::datastream<const char*>& stream,
                                   dp::mutable_variant_object& obj, impl::binary_to_variant_context& ctx )const;

   bytes       _variant_to_binary( const type_name& type, const dp::variant& var, impl::variant_to_binary_context& ctx )const;
   void        _variant_to_binary( const type_name& type, const dp::variant& var,
                                   dp::datastream<char*>& ds, impl::variant_to_binary_context& ctx )const;
...
namespace impl {
   ...
   struct abi_traverse_context {
      abi_traverse_context( dp::microseconds max_serialization_time )
      : max_serialization_time( max_serialization_time ), deadline( dp::time_point::now() + max_serialization_time ), recursion_depth(0)
      {}

      abi_traverse_context( dp::microseconds max_serialization_time, dp::time_point deadline )
      : max_serialization_time( max_serialization_time ), deadline( deadline ), recursion_depth(0)
      {}

      void check_deadline()const;

      dp::scoped_exit<std::function<void()>> enter_scope();

   protected:
      dp::microseconds max_serialization_time;
   ...
   struct abi_traverse_context_with_path : public abi_traverse_context {
      abi_traverse_context_with_path( const abi_serializer& abis, dp::microseconds max_serialization_time, const type_name& type )
      : abi_traverse_context( max_serialization_time ), abis(abis)
      {
         set_path_root(type);
      }

      abi_traverse_context_with_path( const abi_serializer& abis, dp::microseconds max_serialization_time, dp::time_point deadline, const type_name& type )
      ...
      dp::scoped_exit<std::function<void()>> push_to_path( const path_item& item );
   ...
   struct variant_to_binary_context : public abi_traverse_context_with_path {
      ...
      dp::scoped_exit<std::function<void()>> disallow_extensions_unless( bool condition );
   ...
   struct abi_to_variant {
      ...
      static void add( mutable_variant_object &mvo, const char* name, const dp::static_variant<Args...>& v, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         mutable_variant_object obj_mvo;
    ...
    * this will degrade to the common dp::to_variant as soon as the type no longer contains
    * ABI related info
    *
   ...
   struct abi_from_variant {
      ...
      static void extract( const variant& v, action& act, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         DOS_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         DOS_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         DOS_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
      ...
      static void extract( const variant& v, packed_transaction& ptrx, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         DOS_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         DOS_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            DOS_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            transaction trx;
            vector<bytes> context_free_data;
   ...
   /**
    * Reflection visitor that uses a resolver to resolve ABIs for nested types
    * this will degrade to the common dp::from_variant as soon as the type no longer contains
    * ABI related info
    *
    * @tparam Reslover - callable with the signature (const name& code_account) -> optional<abi_def>
    */
   ...
   void abi_to_variant::add( mutable_variant_object &mvo, const char* name, const M& v, Resolver resolver, abi_traverse_context& ctx )
   {
      ...
      dp::reflector<M>::visit( impl::abi_to_variant_visitor<M, Resolver>( member_mvo, v, resolver, ctx) );
   ...
   void abi_from_variant::extract( const variant& v, M& o, Resolver resolver, abi_traverse_context& ctx )
   {
      auto h = ctx.enter_scope();
      const variant_object& vo = v.get_object();
      dp::reflector<M>::visit( abi_from_variant_visitor<M, decltype(resolver)>( vo, o, resolver, ctx ) );
   }
} /// namespace dosio::chain::impl

template<typename T, typename Resolver>
void abi_serializer::to_variant( const T& o, variant& vo, Resolver resolver, const dp::microseconds& max_serialization_time ) try {
   mutable_variant_object mvo;
   impl::abi_traverse_context ctx(max_serialization_time);
   impl::abi_to_variant::add(mvo, "_", o, resolver, ctx);
   vo = std::move(mvo["_"]);
} DP_RETHROW_EXCEPTIONS(error, "Failed to serialize type", ("object",o))

template<typename T, typename Resolver>
void abi_serializer::from_variant( const variant& v, T& o, Resolver resolver, const dp::microseconds& max_serialization_time ) try {
   impl::abi_traverse_context ctx(max_serialization_time);
   impl::abi_from_variant::extract(v, o, resolver, ctx);
} DP_RETHROW_EXCEPTIONS(error, "Failed to deserialize variant", ("variant",v))
