...
namespace dosio { namespace chain { namespace webassembly { namespace wavm {
...
class wavm_runtime : public dosio::chain::wasm_runtime_interface {
   public:
      wavm_runtime();
      ~wavm_runtime();
...
template<>
struct native_to_wasm<const dp::time_point_sec &> {
   using type = I32;
};

template<>
struct native_to_wasm<dp::time_point_sec> {
   using type = I32;
};
...
inline auto convert_native_to_wasm(const running_instance_context& ctx, const dp::time_point_sec& val) {
   return native_to_wasm_t<const dp::time_point_sec &>(val.sec_since_epoch());
}
...
template<>
struct wasm_to_rvalue_type<dp::time_point_sec> {
   static constexpr auto value = ResultType::i32;
};
...
struct intrinsic_invoker_impl<Ret, std::tuple<T &, Inputs...>, std::tuple<Translated...>> {
   ...
   static auto translate_one(running_instance_context& ctx, Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      DOS_ASSERT((U32)ptr != 0, wasm_exception, "references cannot be created for null pointers");
      MemoryInstance* mem = ctx.memory;
   ...
   static auto translate_one(running_instance_context& ctx, Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<!std::is_const<U>::value, Ret> {
      // references cannot be created for null pointers
      DOS_ASSERT((U32)ptr != 0, wasm_exception, "reference cannot be created for null pointers");
      MemoryInstance* mem = ctx.memory;
      if(!mem || (U32)ptr+sizeof(T) >= IR::numBytesPerPage*Runtime::getMemoryNumPages(mem))
         Runtime::causeException(Exception::Cause::accessViolation);
      T &base = *(T*)(getMemoryBaseAddress(mem)+(U32)ptr);
...
#define _REGISTER_WAVM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static Intrinsics::Function _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
      dosio::chain::webassembly::wavm::wasm_function_type_provider<WASM_SIG>::type(),\
      (void *)dosio::chain::webassembly::wavm::intrinsic_function_invoker_wrapper<WASM_SIG, SIG>::type::fn<&CLS::METHOD>()\
   );\


} } } }// dosio::chain::webassembly::wavm
