...
#include <dosio/chain/webassembly/runtime_interface.hpp>
#include <dosio/chain/exceptions.hpp>
#include <dosio/chain/apply_context.hpp>
...
namespace dosio { namespace chain { namespace webassembly { namespace wabt_runtime {

using namespace dp;
using namespace wabt;
using namespace wabt::interp;
using namespace dosio::chain::webassembly::common;
...
struct wabt_apply_instance_vars {
   ...
   char* get_validated_pointer(uint32_t offset, uint32_t size) {
      DOS_ASSERT(memory, wasm_execution_error, "access violation");
      DOS_ASSERT(offset + size <= memory->data.size() && offset + size >= offset, wasm_execution_error, "access violation");
...
class wabt_runtime : public dosio::chain::wasm_runtime_interface {
   public:
      wabt_runtime();
...
{
   DOS_ASSERT( length < INT_MAX/(uint32_t)sizeof(T), binaryen_exception, "length will overflow" );
...
inline null_terminated_ptr null_terminated_ptr_impl(wabt_apply_instance_vars& vars, uint32_t ptr)
{
   ...
   DP_THROW_EXCEPTION(wasm_execution_error, "unterminated string");
}
...
template<>
struct is_reference_from_value<dp::time_point_sec> {
   static constexpr bool value = true;
};
...
inline auto convert_native_to_literal(const wabt_apply_instance_vars& vars, char* ptr) {
   ...
   DOS_ASSERT(ptr >= base && ptr < top_of_memory, wasm_execution_error, "returning pointer not in linear memory");
   Value v;
   v.i32 = (int)(ptr - base);
...
/**
 * Specialization for transcribing a reference to a dp::time_point_sec which can be passed as a native value
 *    This type transcribes into a native type which is loaded by value into a
 *    variable on the stack and then passed by reference to the intrinsic.
 *
 * @tparam Ret - the return type of the native method
 ...
 */
template<typename Ret, typename... Inputs>
struct intrinsic_invoker_impl<Ret, std::tuple<const dp::time_point_sec&, Inputs...>> {
   using next_step = intrinsic_invoker_impl<Ret, std::tuple<Inputs...>>;
   using then_type = Ret (*)(wabt_apply_instance_vars&, const dp::time_point_sec&, Inputs..., const TypedValues&, int);

   template<then_type Then>
   static Ret translate_one(wabt_apply_instance_vars& vars, Inputs... rest, const TypedValues& args, int offset) {
      uint32_t wasm_value = args.at((uint32_t)offset).get_i32();
      auto value = dp::time_point_sec(wasm_value);
      return Then(vars, value, rest..., args, (uint32_t)offset - 1);
   }
...
struct intrinsic_invoker_impl<Ret, std::tuple<T &, Inputs...>> {
   ...
   static auto translate_one(wabt_apply_instance_vars& vars, Inputs... rest, const TypedValues& args, int offset) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      uint32_t ptr = args.at((uint32_t)offset).get_i32();
      DOS_ASSERT(ptr != 0, binaryen_exception, "references cannot be created for null pointers");
      T* base = array_ptr_impl<T>(vars, ptr, 1);
      if ( reinterpret_cast<uintptr_t>(base) % alignof(T) != 0 ) {
         if(vars.ctx.control.contracts_console())
...
#define _REGISTER_WABT_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static dosio::chain::webassembly::wabt_runtime::intrinsic_registrator _INTRINSIC_NAME(__wabt_intrinsic_fn, __COUNTER__) (\
      MOD,\
      NAME,\
      dosio::chain::webassembly::wabt_runtime::wabt_function_type_provider<WASM_SIG>::type(),\
      dosio::chain::webassembly::wabt_runtime::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\

} } } }// dosio::chain::webassembly::wabt_runtime
