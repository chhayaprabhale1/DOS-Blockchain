...
namespace dosio { namespace chain { namespace wasm_validations {
   ...
   struct large_offset_validator {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         ...
         if(memarg_instr->field.o >= wasm_constraints::maximum_linear_memory)
            DP_THROW_EXCEPTION(wasm_execution_error, "Smart contract used an invalid large memory store/load offset");
      }
   ...
   struct blacklist_validator {
      ...
      static constexpr bool post = false;
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         DP_THROW_EXCEPTION(wasm_execution_error, "Error, blacklisted opcode ${op} ", 
            ("op", inst->to_string()));
      }
   };
   ...
   struct nested_validator {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         if (!disabled) {
            ...
            depth++;
            DOS_ASSERT(depth < 1024, wasm_execution_error, "Nested depth exceeded");
         }
      }
   };
   ...
   class wasm_binary_validation {
      ...
      public:
         wasm_binary_validation( const dosio::chain::controller& control, IR::Module& mod ) : _module( &mod ) {
         ...
         void validate() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::DOSIO_OperatorDecoderStream<op_constrainers> decoder(fd.code);
               while ( decoder ) {
