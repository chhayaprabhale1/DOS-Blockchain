...
#include <chain/include/producer_object.hpp>
...
uint128_t transaction_id_to_sender_id( const transaction_id_type& tid ) {
   dp::uint128_t _id(tid._hash[3], tid._hash[2]);
...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::dosio_code_name ) // virtual dosio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         DOS_THROW( action_validate_exception,
 ...
 */
void apply_dosio_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::dosio_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   DOS_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   DOS_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   DOS_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   DOS_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_object, by_name>(create.creator);
   if( !creator.privileged ) {
      DOS_ASSERT( name_str.find( "dosio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'dosio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   DOS_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
} DP_CAPTURE_AND_RETHROW( (create) ) }

void apply_dosio_setcode(apply_context& context) {
   ...
   DOS_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   DOS_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );

   dp::sha256 code_id; /// default ID == 0

   if( act.code.size() > 0 ) {
     code_id = dp::sha256::hash( act.code.data(), (uint32_t)act.code.size() );
   ...
   DOS_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
...
void apply_dosio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_dosio_updateauth(apply_context& context) {
   ...
   DOS_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   DOS_ASSERT( update.permission.to_string().find( "dosio." ) != 0, action_validate_exception,
               "Permission names that start with 'dosio.' are reserved" );
   DOS_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   DOS_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      DOS_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      DOS_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      DOS_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      DOS_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      DOS_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_dosio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::dosio_auth_scope );
   ...
   DOS_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   DOS_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
...
void apply_dosio_linkauth(apply_context& context) {
//   context.require_write_lock( config::dosio_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      DOS_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      DOS_ASSERT(account != nullptr, account_query_exception,
      ...
      DOS_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::dosio_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         DOS_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         DOS_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
  ...
  } DP_CAPTURE_AND_RETHROW((requirement))
}

void apply_dosio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::dosio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   DOS_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_dosio_canceldelay(apply_context& context) {
