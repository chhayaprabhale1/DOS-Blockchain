...
#include <chain/include/permission_object.hpp>
#include <chain/include/permission_link_object.hpp>
#include <chain/include/authority_checker.hpp>
...
#include <chain/include/database_utils.hpp>
...
namespace dosio { namespace chain {
   ...
   namespace detail {
      ...
      struct snapshot_row_traits<permission_object> {
         ...
         static void from_snapshot_row(snapshot_permission_object&& row, permission_object& value, chainbase::database& db) {
            ...
            value.parent = 0;
            if (value.id == 0) {
               DOS_ASSERT(row.parent == permission_name(), snapshot_exception, "Unexpected parent name on reserved permission 0");
               DOS_ASSERT(row.name == permission_name(), snapshot_exception, "Unexpected permission name on reserved permission 0");
               DOS_ASSERT(row.owner == name(), snapshot_exception, "Unexpected owner name on reserved permission 0");
               DOS_ASSERT(row.auth.accounts.size() == 0,  snapshot_exception, "Unexpected auth accounts on reserved permission 0");
               DOS_ASSERT(row.auth.keys.size() == 0,  snapshot_exception, "Unexpected auth keys on reserved permission 0");
               DOS_ASSERT(row.auth.waits.size() == 0,  snapshot_exception, "Unexpected auth waits on reserved permission 0");
               DOS_ASSERT(row.auth.threshold == 0,  snapshot_exception, "Unexpected auth threshold on reserved permission 0");
               DOS_ASSERT(row.last_updated == time_point(),  snapshot_exception, "Unexpected auth last updated on reserved permission 0");
               value.parent = 0;
               ...
               DOS_ASSERT(parent.id != 0, snapshot_exception, "Unexpected mapping to reserved permission 0");
   ...
   void authorization_manager::remove_permission( const permission_object& permission ) {
      ...
      DOS_ASSERT( range.first == range.second, action_validate_exception,
   ...
   dp::time_point authorization_manager::get_permission_last_used( const permission_object& permission )const {
   ...
   { try {
      DOS_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.find<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } DOS_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                            ...
                                                                            )const
   {
      try {
       ...
       //  return optional<permission_name>();
      } DP_CAPTURE_AND_RETHROW((authorizer_account)(scope)(act_name))
   }
                                                                             ...
                                                                             )const
   {
      ...
      if( scope == config::system_account_name ) {
          DOS_ASSERT( act_name != updateauth::get_name() &&
      ...
      try {
         ...
         if( *linked_permission == config::dosio_any_name )
            return optional<permission_name>();
   ...
   {
      DOS_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      DOS_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      DOS_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      DOS_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      DOS_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      DOS_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link dosio::updateauth to a minimum permission" );
      DOS_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link dosio::deleteauth to a minimum permission" );
      DOS_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link dosio::linkauth to a minimum permission" );
      DOS_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link dosio::unlinkauth to a minimum permission" );
      DOS_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link dosio::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to dosio.any permission
         return;

      DOS_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      DOS_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      DOS_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::dosio_any_name )
         return;

      DOS_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
   ...
   dp::microseconds authorization_manager::check_canceldelay_authorization( const canceldelay& cancel,
                                                                            const vector<permission_level>& auths
                                                                          )const
   {
      DOS_ASSERT( auths.size() == 1, irrelevant_auth_exception,
      ...
      DOS_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      DOS_ASSERT( itr != generated_index.end() && itr->sender == account_name() && itr->trx_id == trx_id,
                  tx_not_found,
                 "cannot cancel trx_id=${tid}, there is no deferred transaction with that transaction id",
                 ("tid", trx_id) );

      auto trx = dp::raw::unpack<transaction>(itr->packed_trx.data(), itr->packed_trx.size());
      bool found = false;
      ...
      DOS_ASSERT( found, action_validate_exception,
   ...
   authorization_manager::check_authorization( const vector<action>&                actions,
                                               ...
                                               dp::microseconds                     provided_delay,
                                             ...
                                             )const
   {
      ...
      auto delay_max_limit = dp::seconds( _control.get_global_properties().configuration.max_transaction_delay );

      auto effective_provided_delay =  (provided_delay >= delay_max_limit) ? dp::microseconds::maximum() : provided_delay;
      ...
      map<permission_level, dp::microseconds> permissions_to_satisfy;

      for( const auto& act : actions ) {
         bool special_case = false;
         dp::microseconds delay = effective_provided_delay;
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is dosio.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  DOS_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         DOS_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         DOS_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      auto checker = make_auth_checker( [&](const permission_level& p){ return get_permission(p).auth; },
                                        ...
                                        provided_keys,
                                        provided_permissions,
                                        ( provided_delay >= delay_max_limit ) ? dp::microseconds::maximum() : provided_delay,
                                        checktime
                                      );

      DOS_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            DOS_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
