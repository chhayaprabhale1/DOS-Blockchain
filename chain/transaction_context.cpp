...
namespace bacc = boost::accumulators;
   ...
   void deadline_timer::start(dp::time_point tp) {
      if(tp == dp::time_point::maximum()) {
         expired = 0;
         return;
      }
      if(!deadline_timer_verification.use_deadline_timer) {
      ...
      microseconds x = tp.time_since_epoch() - dp::time_point::now().time_since_epoch();
   ...
   void transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
      ...
      for( const auto& a : trx.context_free_actions ) {
         ...
         DOS_ASSERT( code != nullptr, transaction_exception,
                     "action's code account '${account}' does not exist", ("account", a.account) );
         DOS_ASSERT( a.authorization.size() == 0, transaction_exception,
      ...
      for( const auto& a : trx.actions ) {
         ...
         for( const auto& auth : a.authorization ) {
            ...
            DOS_ASSERT( actor  != nullptr, transaction_exception,
                        "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
            DOS_ASSERT( auth_manager.find_permission(auth) != nullptr, transaction_exception,
      ...
      DOS_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
