...
namespace dosio { namespace chain {
   ...
   block_log::block_log(const dp::path& data_dir)
   ...
   void block_log::open(const dp::path& data_dir) {
      if (my->block_stream.is_open())
         my->block_stream.close();
      if (my->index_stream.is_open())
         my->index_stream.close();

      if (!dp::is_directory(data_dir))
         dp::create_directories(data_dir);
       ...
       */
      auto log_size = dp::file_size(my->block_file);
      auto index_size = dp::file_size(my->index_file);
      ...
      if (log_size) {
         ...
         DOS_ASSERT( my->version > 0, block_log_exception, "Block log was not setup properly" );
         DOS_ASSERT( my->version >= min_supported_version && my->version <= max_supported_version, block_log_unsupported_version,
         ...
         if (my->version > 1){
            ...
            DOS_ASSERT(my->first_block_num > 0, block_log_exception, "Block log is malformed, first recorded block number is 0 but must be greater than or equal to 1");
         } else {
   ...
   uint64_t block_log::append(const signed_block_ptr& b) {
      try {
         DOS_ASSERT( my->genesis_written_to_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );

         my->check_block_write();
         my->check_index_write();

         uint64_t pos = my->block_stream.tellp();
         DOS_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (b->block_num() - my->first_block_num),
   ...
   std::pair<signed_block_ptr, uint64_t> block_log::read_block(uint64_t pos)const {
      ...
      dp::raw::unpack(my->block_stream, *result.first);
   ...
   signed_block_ptr block_log::read_block_by_num(uint32_t block_num)const {
      try {
         ...
         if (pos != npos) {
            ...
            DOS_ASSERT(b->block_num() == block_num, reversible_blocks_exception,
   ...
   void block_log::construct_index() {
      ...
      genesis_state gs;
      dp::raw::unpack(my->block_stream, gs);
      ...
      while( pos < end_pos ) {
         dp::raw::unpack(my->block_stream, tmp);
         my->block_stream.read((char*)&pos, sizeof(pos));
         my->index_stream.write((char*)&pos, sizeof(pos));
      }
   } // construct_index

   dp::path block_log::repair_log( const dp::path& data_dir, uint32_t truncate_at_block ) {
      ilog("Recovering Block Log...");
      DOS_ASSERT( dp::is_directory(data_dir) && dp::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
                 "Block log not found in '${blocks_dir}'", ("blocks_dir", data_dir)          );

      auto now = dp::time_point::now();

      auto blocks_dir = dp::canonical( data_dir );
      ...
      DOS_ASSERT( version > 0, block_log_exception, "Block log was not setup properly" );
      DOS_ASSERT( version >= min_supported_version && version <= max_supported_version, block_log_unsupported_version,
                 "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
      ...
      genesis_state gs;
      dp::raw::unpack(old_block_stream, gs);
      ...
      if (version != 1) {
         ...
         DOS_ASSERT(actual_totem == expected_totem, block_log_exception,
                    "Expected separator between block log header and blocks was not found( expected: ${e}, actual: ${a} )",
                    ("e", dp::to_hex((char*)&expected_totem, sizeof(expected_totem) ))("a", dp::to_hex((char*)&actual_totem, sizeof(actual_totem) )));
      ...
      while( pos < end_pos ) {
         ...
         try {
            dp::raw::unpack(old_block_stream, tmp);
         } catch( ... ) {
      ...
      if( bad_block.valid() ) {
         ...
         if( !dp::exists(tail_path) && incomplete_block_data.size() > 0 ) {
   ...
   genesis_state block_log::extract_genesis_state( const dp::path& data_dir ) {
      DOS_ASSERT( dp::is_directory(data_dir) && dp::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
                 "Block log not found in '${blocks_dir}'", ("blocks_dir", data_dir)          );
