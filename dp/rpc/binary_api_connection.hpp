...
#include <functional>
#include <utility>
#include <dp/signals.hpp>
//#include <dp/rpc/json_connection.hpp>
...
namespace dp {
   ...
   namespace detail {
      ...
      class callback_functor
      {
         public:
            ...
            callback_functor( std::weak_ptr< dp::binary_api_connection > con, uint64_t id )
         ...
         private:
            ...
            std::weak_ptr< dp::binary_api_connection > _binary_api_connection;
      };
      ...
      std::function<variant(const dp::variants&)> to_generic( const std::function<R(Args...)>& f )
      {
      ...
      template<typename ... Args>
      std::function<variant(const dp::variants&)> to_generic( const std::function<void(Args...)>& f )
      {
         return [=]( const variants& args ) { 
      ...
      {
         public:
            any_api( api_id_type api_id, const std::shared_ptr<dp::binary_api_connection>& con )
            ...
            {  DP_ASSERT( false ); return api_id_type(); }
   ...
   class generic_api
   {
      public:
         ...
         generic_api( const Api& a, const std::shared_ptr<dp::binary_api_connection>& c );
         ...
         vector<char> call( const string& name, const vector<char>& args )
         {
            ...
            DP_ASSERT( itr != _by_name.end(), "no method with name '${name}'", ("name",name)("api",_by_name) );
            return call( itr->second, args );
         }

         vector<char> call( uint32_t method_id, const vector<char>& args )
         {
            DP_ASSERT( method_id < _methods.size() );
            return _methods[method_id](args);
         }

         std::weak_ptr< dp::binary_api_connection > get_connection()
         {
      ...
      private:
         ...
         template<typename R, typename Signature, typename ... Args>
         R call_generic( const std::function<R(const std::function<Signature>&,Args...)>& f, dp::datastream<const char*>& ds )
         {
            uint64_t callback_id = 0;
            dp::raw::unpack( ds, callback_id );
            detail::callback_functor<Signature> arg0( get_connection(), callback_id );
         ...
         template<typename R, typename Arg0, typename ... Args>
         R call_generic( const std::function<R(Arg0,Args...)>& f, dp::datastream<const char*>& ds )
         {
         ...
         struct api_visitor
         {
            api_visitor( generic_api& a, const std::weak_ptr<dp::binary_api_connection>& s ):api(a),_api_con(s){ }

            template<typename Interface, typename Adaptor, typename ... Args>
            std::function<variant(const dp::variants&)> to_generic( const std::function<api<Interface,Adaptor>(Args...)>& f )const;

            template<typename Interface, typename Adaptor, typename ... Args>
            std::function<variant(const dp::variants&)> to_generic( const std::function<dp::optional<api<Interface,Adaptor>>(Args...)>& f )const;

            template<typename ... Args>
            std::function<variant(const dp::variants&)> to_generic( const std::function<dp::api_ptr(Args...)>& f )const;

            template<typename R, typename ... Args>
            std::function<variant(const dp::variants&)> to_generic( const std::function<R(Args...)>& f )const;

            template<typename ... Args>
            std::function<variant(const dp::variants&)> to_generic( const std::function<void(Args...)>& f )const;
            ...
            generic_api&  api;
            const std::weak_ptr<dp::binary_api_connection>& _api_con;
         };
   ...
   class binary_api_connection : public std::enable_shared_from_this<dp::binary_api_connection>
   {
      public:
         ...
         {
            DP_ASSERT( _local_apis.size() > api_id );
         ...
         {
            DP_ASSERT( _local_callbacks.size() > callback_id );
      ...
      private:
         ...
         {
            uint32_t                            _api_id;
            std::shared_ptr<dp::binary_api_connection> _connection;

            api_visitor( uint32_t api_id, std::shared_ptr<dp::binary_api_connection> con )
            ...
            static Result from_vector( const vector<char>& v, Result*, const std::shared_ptr<dp::binary_api_connection>&  )
            {
               return dp::raw::unpack<Result>( v ); 
            }

            template<typename ResultInterface>
            static dp::api<ResultInterface> from_vector( const vector<char>& v, 
                                                          dp::api<ResultInterface>* /*used for template deduction*/,
                                                          const std::shared_ptr<dp::binary_api_connection>&  con 
                                                        )
            {
               return con->get_remote_api<ResultInterface>( dp::raw::unpack<uint64_t>( v ) );
            }

            static dp::api_ptr from_vector(
               const vector<char>& v,
               dp::api_ptr* /* used for template deduction */,
               const std::shared_ptr<dp::binary_api_connection>&  con
            )
            {
               return dp::api_ptr( new detail::any_api( dp::raw::unpack<uint64_t>(v), con ) );
            }

            template<typename T>
            static result_type convert_callbacks( const std::shared_ptr<dp::binary_api_connection>&, const T& v ) 
            { 
               return dp::raw::pack(v); 
            }

            template<typename Signature>
            static result_type convert_callbacks( const std::shared_ptr<dp::binary_api_connection>& con, const std::function<Signature>& v ) 
            { 
   ...
   class local_binary_api_connection : public binary_api_connection
   {
      public:
         ...
         void  set_remote_connection( const std::shared_ptr<dp::binary_api_connection>& rc )
         {
            DP_ASSERT( !_remote_connection );
            DP_ASSERT( rc != this->shared_from_this() );
            _remote_connection = rc;
         }
         const std::shared_ptr<dp::binary_api_connection>& remote_connection()const  { return _remote_connection; }

         std::shared_ptr<dp::binary_api_connection>    _remote_connection;
   };

   template<typename Api>
   generic_api::generic_api( const Api& a, const std::shared_ptr<dp::binary_api_connection>& c )
   ...
   std::function<result_type(const params_type&)> generic_api::api_visitor::to_generic( 
                                               const std::function<dp::api<Interface,Adaptor>(Args...)>& f )const
   {
      auto api_con = _api_con;
      auto gapi = &api;
      return [=]( const params_type& args ) { 
         auto con = api_con.lock();
         DP_ASSERT( con, "not connected" );

         dp::raw::datastream<const char*> ds( args.data(), args.size() );
   ...
   template<typename Interface, typename Adaptor, typename ... Args>
   std::function<result_type(const params_type&)> generic_api::api_visitor::to_generic( 
                                               const std::function<dp::optional<dp::api<Interface,Adaptor>>(Args...)>& f )const
   {
      auto api_con = _api_con;
      auto gapi = &api;
      return [=]( const params_type& args )-> dp::variant { 
         auto con = api_con.lock();
         DP_ASSERT( con, "not connected" );
   ...
   template<typename ... Args>
   std::function<result_type(const params_type&)> generic_api::api_visitor::to_generic(
                                               const std::function<dp::api_ptr(Args...)>& f )const
   {
      auto api_con = _api_con;
      auto gapi = &api;
      return [=]( const variants& args ) -> dp::variant {
         auto con = api_con.lock();
         DP_ASSERT( con, "not connected" );
   ...
   std::function<result_type(const params_type&)> generic_api::api_visitor::to_generic( const std::function<R(Args...)>& f )const
   {
      ...
      return [f,gapi]( const params_type& args ) { 
         dp::raw::datastream<const char*> ds( args.data(), args.size() );
         return  dp::raw::pack(gapi->call_generic( f, ds )); 
      };
   }
   ...
   namespace detail {
      ...
      {
         std::shared_ptr< dp::binary_api_connection > locked = _binary_api_connection.lock();
         // TODO:  make new exception type for this instead of recycling eof_exception
         if( !locked )
            throw dp::eof_exception();

         /// TODO------------->>> pack args... 
         locked->send_callback( _callback_id, dp::raw::pack( args... ) ).template as< result_type >();
      }
      ...
      class callback_functor<void(Args...)>
      {
         public:
          ...
          void operator()( Args... args )const
          {
             ...
             if( !locked )
                throw dp::eof_exception();
             locked->send_notice( _callback_id, dp::variants{ args... } );
          }
