...
namespace dp {
   ...
   namespace detail {
      ...
      class callback_functor
      {
         public:
            ...
            callback_functor( std::weak_ptr< dp::api_connection > con, uint64_t id )
         ...
         private:
            ...
            std::weak_ptr< dp::api_connection > _api_connection;
      };
      ...
      {
         public:
            any_api( api_id_type api_id, const std::shared_ptr<dp::api_connection>& con )
   ...
   class generic_api
   {
      public:
         ...
         generic_api( const Api& a, const std::shared_ptr<dp::api_connection>& c );
         ...
         std::weak_ptr< dp::api_connection > get_connection()
         {
      ...
      private:
         ...
         {
            DP_ASSERT( a0 != e, "too few arguments passed to method" );
         ...
         struct api_visitor
         {
            api_visitor( generic_api& a, const std::weak_ptr<dp::api_connection>& s ):_api(a),_api_con(s){ }
            ...
            generic_api& _api;
            const std::weak_ptr<dp::api_connection>& _api_con;
         };
   ...
   class api_connection : public std::enable_shared_from_this<dp::api_connection>
   {
      public:
         api_connection(){}
      ...
      private:
         ...
         {
            uint32_t                            _api_id;
            std::shared_ptr<dp::api_connection> _connection;

            api_visitor( uint32_t api_id, std::shared_ptr<dp::api_connection> con )
            ...
            static Result from_variant( const variant& v, Result*, const std::shared_ptr<dp::api_connection>&  )
            {
               return v.as<Result>();
            }

            template<typename ResultInterface>
            static dp::api<ResultInterface> from_variant( const variant& v, 
                                                          dp::api<ResultInterface>* /*used for template deduction*/,
                                                          const std::shared_ptr<dp::api_connection>&  con 
                                                        )
            {
               return con->get_remote_api<ResultInterface>( v.as_uint64() );
            }

            static dp::api_ptr from_variant(
               const variant& v,
               dp::api_ptr* /* used for template deduction */,
               const std::shared_ptr<dp::api_connection>&  con
            )
            {
               return dp::api_ptr( new detail::any_api( v.as_uint64(), con ) );
            }

            template<typename T>
            static dp::variant convert_callbacks( const std::shared_ptr<dp::api_connection>&, const T& v ) 
            { 
               return dp::variant(v); 
            }

            template<typename Signature>
            static dp::variant convert_callbacks( const std::shared_ptr<dp::api_connection>& con, const std::function<Signature>& v ) 
            { 
   ...
   class local_api_connection : public api_connection
   {
      public:
         ...
         void  set_remote_connection( const std::shared_ptr<dp::api_connection>& rc )
         {
            DP_ASSERT( !_remote_connection );
            DP_ASSERT( rc != this->shared_from_this() );
            _remote_connection = rc;
         }
         const std::shared_ptr<dp::api_connection>& remote_connection()const  { return _remote_connection; }

         std::shared_ptr<dp::api_connection>    _remote_connection;
   };

   template<typename Api>
   generic_api::generic_api( const Api& a, const std::shared_ptr<dp::api_connection>& c )
   ...
   template<typename Interface, typename Adaptor, typename ... Args>
   std::function<variant(const dp::variants&)> generic_api::api_visitor::to_generic( 
   ...
   template<typename R, typename ... Args>
   std::function<variant(const dp::variants&)> generic_api::api_visitor::to_generic( const std::function<R(Args...)>& f )const
   {
   ...
   template<typename ... Args>
   std::function<variant(const dp::variants&)> generic_api::api_visitor::to_generic( const std::function<void(Args...)>& f )const
   {
      generic_api* gapi = &_api;
      return [f,gapi]( const variants& args ) { 
   ...
   namespace detail {
      ...
      {
         std::shared_ptr< dp::api_connection > locked = _api_connection.lock();
         // TODO:  make new exception type for this instead of recycling eof_exception
         if( !locked )
            throw dp::eof_exception();
         locked->send_callback( _callback_id, dp::variants{ args... } ).template as< result_type >();
      }
