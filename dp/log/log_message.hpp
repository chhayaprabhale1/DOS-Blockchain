...
namespace dp
{
   ...
   class log_context 
   {
      public:
        ...
        void          append_context( const dp::string& c );
   ...
   class log_message
   {
      public:
         log_message();
         /**
          *  @param ctx - generally provided using the DP_LOG_CONTEXT(LEVEL) macro 
          */
...
DP_REFLECT_TYPENAME( dp::log_message );
...
/**
 * @def DP_LOG_CONTEXT(LOG_LEVEL)
 * @brief Automatically captures the File, Line, and Method names and passes them to
 *        the constructor of dp::log_context along with LOG_LEVEL
 * @param LOG_LEVEL - a valid log_level::Enum name.
 */
#define DP_LOG_CONTEXT(LOG_LEVEL) \
   dp::log_context( dp::log_level::LOG_LEVEL, __FILE__, __LINE__, __func__ )
   
/**
 * @def DP_LOG_MESSAGE(LOG_LEVEL,FORMAT,...)
 *
 ...
 */
#define DP_LOG_MESSAGE( LOG_LEVEL, FORMAT, ... ) \
   dp::log_message( DP_LOG_CONTEXT(LOG_LEVEL), FORMAT, dp::mutable_variant_object()__VA_ARGS__ )
