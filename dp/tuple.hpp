...
namespace dp {
   ...
   */
        ...
        template<typename T>
        struct is_tuple {
          typedef dp::false_type type;
        };

        #define RREF_PARAMS(z,n,data) BOOST_PP_CAT(AA,n)&& BOOST_PP_CAT(p,n)
        #define ILIST_PARAMS(z,n,data) BOOST_PP_CAT(a,n)( dp::forward<BOOST_PP_CAT(AA,n)>( BOOST_PP_CAT(p,n) ) )
        ...
        #define DEDUCE_MEMBERS(z,n,data) typename dp::deduce<BOOST_PP_CAT(AA,n)>::type
        #define FORWARD_PARAMS(z,n,data) dp::forward<BOOST_PP_CAT(AA,n)>(BOOST_PP_CAT(p,n))
        ...
        template<BOOST_PP_ENUM_PARAMS( n, typename AA)> \
        struct is_tuple<dp::tuple<BOOST_PP_ENUM_PARAMS(n,AA)> > { \
          typedef dp::true_type type; \
        }; \
        template<BOOST_PP_ENUM_PARAMS( n, typename AA)> \
        struct deduce<dp::tuple<BOOST_PP_ENUM_PARAMS(n,AA)> > { \
          typedef dp::tuple<BOOST_PP_ENUM( n, DEDUCE_MEMBERS,unused)>  type; \
        }; 
