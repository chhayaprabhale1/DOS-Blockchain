...
// it is not meant to be included except internally from json.cpp in dp
...
namespace dp { namespace json_relaxed
{
   ...
   std::string tokenFromStream( T& in )
   {
         ...
         while( true )
         {
            switch( c = in.peek() )
            {
               ...
               case '_': case '-': case '.': case '+': case '/':
                  ...
                  break;
               case EOF:
                  DP_THROW_EXCEPTION( eof_exception, "unexpected end of file" );
               default:
                  return token.str();
            }
         }
      ...
      catch( const dp::eof_exception& eof )
      {
         return token.str();
      }
      ...
      DP_RETHROW_EXCEPTIONS( warn, "while parsing token '${token}'",
                                          ("token", token.str() ) );
   }
   ...
   std::string quoteStringFromStream( T& in )
   {
       ...
       try
       {
           ...
           switch( q )
           {
               case '\'':
                   if( strict )
                       DP_THROW_EXCEPTION( parse_error_exception, "expected: '\"' at beginning of string, got '\''" );
                   // falls through
               case '"':
                   break;
               default:
                   if( strict )
                       DP_THROW_EXCEPTION( parse_error_exception, "expected: '\"' at beginning of string" );
                   else
                       DP_THROW_EXCEPTION( parse_error_exception, "expected: '\"' | '\\\'' at beginning of string" );
           }
           if( in.peek() == q )
           {
               ...
               if( strict )
                   DP_THROW_EXCEPTION( parse_error_exception, "triple quote unsupported in strict mode" );
               else
               {
                   in.get();
                   ...
                   while( true )
                   {
                       ...
                       if( c == q )
                       {
                           ...
                           DP_THROW_EXCEPTION( parse_error_exception, "unexpected EOF in string '${token}'",
                                      ("token", token.str() ) );
           ...
           while( true )
           {
               ...
               if( c == q )
               {
                   ...
                   DP_THROW_EXCEPTION( parse_error_exception, "unexpected EOL in string '${token}'",
                              ("token", token.str() ) );
               else
               {
                   in.get();
   ...
   std::string stringFromStream( T& in )
   {
      try
      {
         ...
         switch( c )
         {
             ...
             case 'r':
                 if( strict )
                     DP_THROW_EXCEPTION( parse_error_exception, "raw strings not supported in strict mode" );
             case 'R':
                 in.get();
                 c2 = in.peek();
                 switch( c2 )
                 {
                     ...
                     default:
                         if( strict )
                             DP_THROW_EXCEPTION( parse_error_exception, "unquoted strings not supported in strict mode" );
             ...
             case EOF:
                  DP_THROW_EXCEPTION( eof_exception, "unexpected end of file" );
             default:
                 DP_THROW_EXCEPTION( parse_error_exception, "expected: string" );
         }

       } DP_RETHROW_EXCEPTIONS( warn, "while parsing string" );
   }
   ...
   dp::variant parseInt( const std::string& token, size_t start )
   {
       ...
       if( i >= n )
           DP_THROW_EXCEPTION( parse_error_exception, "zero-length integer" );
       ...
       while(true)
       {
           ...
           uint8_t vc = ctbl[c];
           if( vc == 0xFF )
               DP_THROW_EXCEPTION( parse_error_exception, "illegal character {c} in integer of base {b}", ("c", c)("b", base) );
           if( val > maxb4mul )
               DP_THROW_EXCEPTION( parse_error_exception, "integer literal overflow" );
           val *= base;
       ...
       if( token[0] == '-' )
       {
           if( val > INT64_MAX_PLUS_ONE )
               DP_THROW_EXCEPTION( parse_error_exception, "negative integer literal overflow" );
           // special cased to avoid trying to compute -INT64_MIN which is probably undefined or something
           if( val == INT64_MAX_PLUS_ONE )
               return dp::variant( INT64_MIN );
           return dp::variant( -static_cast<int64_t>(val) );
       }
       return dp::variant( val );
   }

   template<bool strict, uint8_t base>
   dp::variant maybeParseInt( const std::string& token, size_t start )
   {
       try
       {
       ...
       catch( const parse_error_exception &e )
       {
           if( strict )
               ...
               return dp::variant( token );
       }
   }

   template<bool strict>
   dp::variant parseNumberOrStr( const std::string& token )
   { try {
       //ilog( (token) ); 
       size_t i = 0, n = token.length();
       if( n == 0 )
           DP_THROW_EXCEPTION( parse_error_exception, "expected: non-empty token, got: empty token" );
       switch( token[0] )
       {
           case '+':
               if( strict )
                   DP_THROW_EXCEPTION( parse_error_exception, "unary + not supported in strict mode" );
               i++;
               break;
           case '-':
               i++;
       ...
       switch( c )
       {
           case '0':
               if( i >= n )
                   return dp::variant( uint64_t( 0 ) );
               switch( token[i] )
               {
                   case 'b':
                   case 'B':
                       if( strict )
                           DP_THROW_EXCEPTION( parse_error_exception, "binary numeric literals not supported in strict mode" );
                       i++;
                       if( i >= n )
                           DP_THROW_EXCEPTION( parse_error_exception, "empty binary numeric literal" );
                       return maybeParseInt<strict, 2>( token, i+1 );
                   case 'o':
                   case 'O':
                       if( strict )
                           DP_THROW_EXCEPTION( parse_error_exception, "octal numeric literals not supported in strict mode" );
                       return maybeParseInt<strict, 8>( token, i+1 );
                   case 'x':
                   case 'X':
                       if( strict )
                           DP_THROW_EXCEPTION( parse_error_exception, "hex numeric literals not supported in strict mode" );
                   ...
                   default:
                       ...
                       if( strict )
                           DP_THROW_EXCEPTION( parse_error_exception, "expected '.'|'e'|'E' parsing number, got '{c}'",
                               ( "c", c ) );
               }
               break;
           ...
           case '_': case '-': case '.': case '+': case '/':
               if( strict )
                   DP_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' parsing number", ( "c", c ) );
               return dp::variant( token );
           default:
               DP_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in token", ( "c", c ) );
       }
       size_t start = i-1;
           ...
           switch( c )
           {
               ...
               case '.':
                   ...
                   if( dot_ok )
                   {
                       ...
                       {
                           if( strict )
                               DP_THROW_EXCEPTION( parse_error_exception, "number cannot end with '.' in strict mode" );
                           return dp::variant( dp::to_double(token.c_str()) );
                       }
                       ...
                       switch( c )
                       {
                           ...
                           case 'E':
                               if( strict )
                                   DP_THROW_EXCEPTION( parse_error_exception, "expected digit after '.'" );
                               break;
                           ...
                           case '_': case '-': case '.': case '+': case '/':
                               ...
                               return dp::variant( token );
                           default:
                               DP_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in token", ( "c", c )("i",int(c)) );
                       }
                   }
                   else
                   {
                       if( strict )
                           DP_THROW_EXCEPTION( parse_error_exception, "illegal multiple . in number" );
                       return dp::variant( token );
                   }
                   break;
               case 'e':
               case 'E':
                   ...
                   {
                       if( strict )
                           DP_THROW_EXCEPTION( parse_error_exception, "expected exponent after 'e'|'E' parsing number" );
                       return dp::variant( token );
                   }
                   c = token[i++];
                   switch( c )
                   {
                       case '+': case '-':
                           ...
                           {
                               if( strict )
                                   DP_THROW_EXCEPTION( parse_error_exception, "expected exponent" );
                               return dp::variant( token );
                           }
                           break;
                       case '0': case '1': case '2': case '3': case '4':
                       ...
                       case '_':           case '.':           case '/':
                           if( strict )
                               DP_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in number", ( "c", c ) );
                           return dp::variant( token );
                       default:
                           DP_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in token", ( "c", c ) );
                   }
   ...
   variant_object objectFromStream( T& in, uint32_t max_depth )
   {
      ...
      try
      {
         char c = in.peek();
         if( c != '{' )
            DP_THROW_EXCEPTION( parse_error_exception,
         ...
         while( in.peek() != '}' )
         {
            ...
            if( in.peek() != ':' )
            {
               DP_THROW_EXCEPTION( parse_error_exception, "Expected ':' after key \"${key}\"",
                                        ("key", key) );
            }
            in.get();
         ...
         DP_THROW_EXCEPTION( parse_error_exception, "Expected '}' after ${variant}", ("variant", obj ) );
      }
      catch( const dp::eof_exception& e )
      {
         DP_THROW_EXCEPTION( parse_error_exception, "Unexpected EOF: ${e}", ("e", e.to_detail_string() ) );
      }
      catch( const std::ios_base::failure& e )
      {
         DP_THROW_EXCEPTION( parse_error_exception, "Unexpected EOF: ${e}", ("e", e.what() ) );
      } DP_RETHROW_EXCEPTIONS( warn, "Error parsing object" );
   }
   ...
   variants arrayFromStream( T& in, uint32_t max_depth )
   {
      ...
      {
        if( in.peek() != '[' )
           DP_THROW_EXCEPTION( parse_error_exception, "Expected '['" );
        in.get();
        skip_white_space(in);
        ...
        if( in.peek() != ']' )
           DP_THROW_EXCEPTION( parse_error_exception, "Expected ']' after parsing ${variant}",
                                    ("variant", ar) );

        in.get();
      } DP_RETHROW_EXCEPTIONS( warn, "Attempting to parse array ${array}",
                                         ("array", ar ) );
      return ar;
   }
   ...
   { try {
       ...
       if( strict && !(result.is_int64() || result.is_uint64() || result.is_double()) )
           DP_THROW_EXCEPTION( parse_error_exception, "expected: number" );
       return result;
   ...
   variant wordFromStream( T& in )
   {
       ...
       DP_THROW_EXCEPTION( parse_error_exception, "expected: null|true|false" );
   }
      ...
      while( signed char c = in.peek() )
      {
         switch( c )
         {
            ...
            case EOF:
              DP_THROW_EXCEPTION( eof_exception, "unexpected end of file" );
            default:
              DP_THROW_EXCEPTION( parse_error_exception, "Unexpected char '${c}' in \"${s}\"",
