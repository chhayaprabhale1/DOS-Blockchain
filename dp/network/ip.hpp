...
namespace dp {
  ...
  namespace ip {
    class address {
      public:
        ...
        address( const dp::string& s );

        address& operator=( const dp::string& s );
  ...
  namespace raw 
  {
    ...
    {
       dp::raw::pack( s, v.get_address() );
       dp::raw::pack( s, v.port() );
    }
    template<typename Stream> 
...
DP_REFLECT_TYPENAME( dp::ip::address ) 
DP_REFLECT_TYPENAME( dp::ip::endpoint ) 
namespace std
{
    template<>
    struct hash<dp::ip::endpoint>
    {
       size_t operator()( const dp::ip::endpoint& e )const
       {
