...
namespace dp {
   ...
   *  It also supports use with the dp unpack() functionality via a datastream
   *  helper class.
   */
  template <uint32_t buffer_len>
  ...
  public:
    ...
    std::vector<boost::asio::mutable_buffer> get_buffer_sequence_for_boost_async_read() {
      ...
      DP_ASSERT(write_ind.first < buffers.size());
    ...
    bool read(void* s, uint32_t size) {
      if (bytes_to_read() < size) {
        DP_THROW_EXCEPTION(out_of_range_exception, "tried to read ${r} but only ${s} left", ("r", size)("s", bytes_to_read()));
      }
    ...
    bool peek(void* s, uint32_t size, index_t& index) {
      if (bytes_to_read_from_index(index) < size) {
        DP_THROW_EXCEPTION(out_of_range_exception, "tried to peek ${r} but only ${s} left", ("r", size)("s",bytes_to_read_from_index(index)));
      }
     ...
     *  dp library's unpack functionality.
     */
  ...
  /*
   *  @brief datastream adapter that adapts message_buffer for use with dp unpack
   *
   *  This class supports unpack functionality but not pack.
   */
  // Stream adapter for use with the dp unpack functionality
  template <uint32_t buffer_len>
  class mb_datastream {
     public:
        ...
        inline bool read( char* d, size_t s ) {
          ...
          dp::detail::throw_datastream_range_error( "read", mb.bytes_to_read(), s - mb.bytes_to_read());
        }
