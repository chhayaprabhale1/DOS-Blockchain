...
namespace dp {
   ...
   *  wrapped in an dp::error_report and the additional helper method dp::path::windows_string(),
   *  can be used to calculate paths intended for systems different than the host.
   *
   *  @note Serializes to a dp::value() as the result of generic_string()
   */
  class path {
    public:
      path();
      ...
      path& operator /=( const dp::path& );
      friend path operator /( const dp::path& p, const dp::path& );
      friend bool operator ==( const dp::path& p, const dp::path& );
      friend bool operator !=( const dp::path& p, const dp::path& );
      friend bool operator < ( const dp::path& p, const dp::path& );
      ...
      void         replace_extension( const dp::path& e );
      dp::path     stem()const;
      dp::path     extension()const;
      dp::path     filename()const;
      dp::path     parent_path()const;
      ...
      /** Retrieves native string path representation and next converts it into
          ...
          It is needed since not all parts of dp library accept unicode paths
          (dp::file_mapping).
      */
  ...
  class directory_iterator {
    public:
      directory_iterator( const dp::path& p );
      directory_iterator();
      ~directory_iterator();

      dp::path            operator*()const;
  ...
  class recursive_directory_iterator {
    public:
      recursive_directory_iterator( const dp::path& p );
  ...
  const dp::path& current_path();

  class variant;
  void to_variant( const dp::path&,  dp::variant&  );
  void from_variant( const dp::variant& , dp::path& );
  ...
  public:
     ...
     const dp::path& path() const;
     void remove();
     void release();
  protected:
     typedef dp::optional<dp::path> path_t;
  ...
  public:
     ...
     temp_file(const dp::path& tempFolder = dp::temp_directory_path(), bool create = false);
  };
  ...
  public:
     ...
     temp_directory(const dp::path& tempFolder = dp::temp_directory_path());
  };
...
# define DP_HAS_SIMPLE_FILE_LOCK
#endif
#ifdef DP_HAS_SIMPLE_FILE_LOCK  
  /** simple class which only allows one process to open any given file. 
   * approximate usage:
   * int main() {
   *   dp::simple_file_lock instance_lock("~/.my_app/.lock");
