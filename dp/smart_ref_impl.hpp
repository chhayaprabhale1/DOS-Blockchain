...
namespace dp {
    ...
    smart_ref<T>::smart_ref( U&& u ) {
      impl = new (this) T( dp::forward<U>(u) );
    }

    template<typename T>
    template<typename U,typename V>
    smart_ref<T>::smart_ref( U&& u, V&& v ) {
      impl = new T( dp::forward<U>(u), dp::forward<V>(v) );
    }
    template<typename T>
    template<typename U,typename V,typename X,typename Y>
    smart_ref<T>::smart_ref( U&& u, V&& v, X&& x, Y&&  y ) {
      impl = new T( dp::forward<U>(u), dp::forward<V>(v), dp::forward<X>(x), dp::forward<Y>(y) );
    }
