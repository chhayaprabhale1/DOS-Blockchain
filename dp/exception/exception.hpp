...
namespace dp
{
    ...
    *
    *  @see DP_THROW_EXCEPTION
    *  @see DP_RETHROW_EXCEPTION
    *  @see DP_RETHROW_EXCEPTIONS
    */
   class exception
   {
      public:
   ...
   template<typename T>
   dp::exception_ptr copy_exception( T&& e )
   {
#if defined(_MSC_VER) && (_MSC_VER < 1700)
     return std::make_shared<unhandled_exception>( log_message(),
                                                   std::copy_exception(dp::forward<T>(e)) );
#else
     return std::make_shared<unhandled_exception>( log_message(),
                                                   std::make_exception_ptr(dp::forward<T>(e)) );
#endif
   }
...
#define DP_REGISTER_EXCEPTION(r, unused, base) \
   dp::exception_factory::instance().register_exception<base>();

#define DP_REGISTER_EXCEPTIONS( SEQ )\
     \
   static bool exception_init = []()->bool{ \
    BOOST_PP_SEQ_FOR_EACH( DP_REGISTER_EXCEPTION, v, SEQ )  \
      return true; \
   }();  \


#define DP_DECLARE_DERIVED_EXCEPTION( TYPE, BASE, CODE, WHAT ) \
   ...
   { \
      public: \
       ...
       explicit TYPE( dp::log_message&& m, int64_t code, const std::string& name_value, const std::string& what_value ) \
       :BASE( std::move(m), code, name_value, what_value ){} \
       explicit TYPE( dp::log_messages&& m, int64_t code, const std::string& name_value, const std::string& what_value )\
       :BASE( std::move(m), code, name_value, what_value ){}\
       explicit TYPE( const dp::log_messages& m, int64_t code, const std::string& name_value, const std::string& what_value )\
       :BASE( m, code, name_value, what_value ){}\
       TYPE( const std::string& what_value, const dp::log_messages& m ) \
       :BASE( m, CODE, BOOST_PP_STRINGIZE(TYPE), what_value ){} \
       TYPE( dp::log_message&& m ) \
       :BASE( dp::move(m), CODE, BOOST_PP_STRINGIZE(TYPE), WHAT ){}\
       TYPE( dp::log_messages msgs ) \
       :BASE( dp::move( msgs ), CODE, BOOST_PP_STRINGIZE(TYPE), WHAT ) {} \
       TYPE( const TYPE& c ) \
       :BASE(c){} \
       TYPE( const BASE& c ) \
       :BASE(c){} \
       ...
       \
       virtual std::shared_ptr<dp::exception> dynamic_copy_exception()const\
       { return std::make_shared<TYPE>( *this ); } \
       virtual NO_RETURN void     dynamic_rethrow_exception()const \
       { if( code() == CODE ) throw *this;\
         else dp::exception::dynamic_rethrow_exception(); \
       } \
   };

  #define DP_DECLARE_EXCEPTION( TYPE, CODE, WHAT ) \
      DP_DECLARE_DERIVED_EXCEPTION( TYPE, dp::exception, CODE, WHAT )

  DP_DECLARE_EXCEPTION( timeout_exception, timeout_exception_code, "Timeout" );
  DP_DECLARE_EXCEPTION( file_not_found_exception, file_not_found_exception_code, "File Not Found" );
  /**
   * @brief report's parse errors
   */
  DP_DECLARE_EXCEPTION( parse_error_exception, parse_error_exception_code, "Parse Error" );
  DP_DECLARE_EXCEPTION( invalid_arg_exception, invalid_arg_exception_code, "Invalid Argument" );
  /**
   * @brief reports when a key, guid, or other item is not found.
   */
  DP_DECLARE_EXCEPTION( key_not_found_exception, key_not_found_exception_code, "Key Not Found" );
  DP_DECLARE_EXCEPTION( bad_cast_exception, bad_cast_exception_code, "Bad Cast" );
  DP_DECLARE_EXCEPTION( out_of_range_exception, out_of_range_exception_code, "Out of Range" );

  /** @brief if an operation is unsupported or not valid this may be thrown */
  DP_DECLARE_EXCEPTION( invalid_operation_exception,
                        invalid_operation_exception_code,
                        "Invalid Operation" );
  /** @brief if an host name can not be resolved this may be thrown */
  DP_DECLARE_EXCEPTION( unknown_host_exception,
   ...
   */
  DP_DECLARE_EXCEPTION( canceled_exception, canceled_exception_code, "Canceled" );
  /**
   *  @brief used inplace of assert() to report violations of pre conditions.
   */
  DP_DECLARE_EXCEPTION( assert_exception, assert_exception_code, "Assert Exception" );
  DP_DECLARE_EXCEPTION( eof_exception, eof_exception_code, "End Of File" );
  DP_DECLARE_EXCEPTION( null_optional, null_optional_code, "null optional" );
  DP_DECLARE_EXCEPTION( udt_exception, udt_error_code, "UDT error" );
  DP_DECLARE_EXCEPTION( aes_exception, aes_error_code, "AES error" );
  DP_DECLARE_EXCEPTION( overflow_exception, overflow_code, "Integer Overflow" );
  DP_DECLARE_EXCEPTION( underflow_exception, underflow_code, "Integer Underflow" );
  DP_DECLARE_EXCEPTION( divide_by_zero_exception, divide_by_zero_code, "Integer Divide By Zero" );
 ...
 */
#define DP_EXPAND_MACRO( x ) x
/**
 *  @brief Checks a condition and throws an assert_exception if the test is FALSE
 */
#define DP_ASSERT( TEST, ... ) \
  DP_EXPAND_MACRO( \
    DP_MULTILINE_MACRO_BEGIN \
      if( UNLIKELY(!(TEST)) ) \
      {                                                                      \
        if( dp::enable_record_assert_trip )                                  \
           dp::record_assert_trip( __FILE__, __LINE__, #TEST );              \
        DP_THROW_EXCEPTION( dp::assert_exception, #TEST ": "  __VA_ARGS__ ); \
      }                                                                      \
    DP_MULTILINE_MACRO_END \
  )

#define DP_CAPTURE_AND_THROW( EXCEPTION_TYPE, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
    throw EXCEPTION_TYPE( DP_LOG_MESSAGE( error, "", DP_FORMAT_ARG_PARAMS(__VA_ARGS__) ) ); \
  DP_MULTILINE_MACRO_END

//#define DP_THROW( FORMAT, ... )
// DP_INDIRECT_EXPAND workas around a bug in Visual C++ variadic macro processing that prevents it
// from separating __VA_ARGS__ into separate tokens
#define DP_INDIRECT_EXPAND(MACRO, ARGS) MACRO ARGS
#define DP_THROW(  ... ) \
  DP_MULTILINE_MACRO_BEGIN \
    throw dp::exception( DP_INDIRECT_EXPAND(DP_LOG_MESSAGE, ( error, __VA_ARGS__ )) );  \
  DP_MULTILINE_MACRO_END

#define DP_EXCEPTION( EXCEPTION_TYPE, FORMAT, ... ) \
    EXCEPTION_TYPE( DP_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) )
/**
 *  @def DP_THROW_EXCEPTION( EXCEPTION, FORMAT, ... )
 *  @param EXCEPTION a class in the Phoenix::Athena::API namespace that inherits
 *  @param format - a const char* string with "${keys}"
 */
#define DP_THROW_EXCEPTION( EXCEPTION, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
    throw EXCEPTION( DP_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) ); \
  DP_MULTILINE_MACRO_END


/**
 *  @def DP_RETHROW_EXCEPTION(ER,LOG_LEVEL,FORMAT,...)
 *  @brief Appends a log_message to the exception ER and rethrows it.
 */
#define DP_RETHROW_EXCEPTION( ER, LOG_LEVEL, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
    ER.append_log( DP_LOG_MESSAGE( LOG_LEVEL, FORMAT, __VA_ARGS__ ) ); \
    throw; \
  DP_MULTILINE_MACRO_END

#define DP_LOG_AND_RETHROW( )  \
   catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch( dp::exception& er ) { \
      wlog( "${details}", ("details",er.to_detail_string()) ); \
      DP_RETHROW_EXCEPTION( er, warn, "rethrow" ); \
      ...
      dp::exception dpe( \
                ...
                e.what() ) ; \
      wlog( "${details}", ("details",dpe.to_detail_string()) ); \
      throw dpe;\
   } catch( ... ) {  \
...
#define DP_CAPTURE_LOG_AND_RETHROW( ... )  \
   catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch( dp::exception& er ) { \
      wlog( "${details}", ("details",er.to_detail_string()) ); \
      wdump( __VA_ARGS__ ); \
      DP_RETHROW_EXCEPTION( er, warn, "rethrow", DP_FORMAT_ARG_PARAMS(__VA_ARGS__) ); \
   } catch( const std::exception& e ) {  \
      dp::exception dpe( \
                DP_LOG_MESSAGE( warn, "rethrow ${what}: ", DP_FORMAT_ARG_PARAMS( __VA_ARGS__ )("what",e.what())), \
                dp::std_exception_code,\
                BOOST_CORE_TYPEID(e).name(), \
                e.what() ) ; \
      wlog( "${details}", ("details",dpe.to_detail_string()) ); \
      ...
      dp::unhandled_exception e( \
                DP_LOG_MESSAGE( warn, "rethrow", DP_FORMAT_ARG_PARAMS( __VA_ARGS__) ), \
                std::current_exception() ); \
      wlog( "${details}", ("details",e.to_detail_string()) ); \
      wdump( __VA_ARGS__ ); \
      throw e; \
...
/**
 *  @def DP_RETHROW_EXCEPTIONS(LOG_LEVEL,FORMAT,...)
 *  @brief  Catchs all exception's, std::exceptions, and ... and rethrows them after
 *   appending the provided log message.
 */
#define DP_RETHROW_EXCEPTIONS( LOG_LEVEL, FORMAT, ... ) \
   catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch( dp::exception& er ) { \
      DP_RETHROW_EXCEPTION( er, LOG_LEVEL, FORMAT, __VA_ARGS__ ); \
   } catch( const std::exception& e ) {  \
      dp::exception dpe( \
                DP_LOG_MESSAGE( LOG_LEVEL, "${what}: " FORMAT,__VA_ARGS__("what",e.what())), \
                dp::std_exception_code,\
                BOOST_CORE_TYPEID(e).name(), \
                e.what() ) ; throw dpe;\
   } catch( ... ) {  \
      throw dp::unhandled_exception( \
                DP_LOG_MESSAGE( LOG_LEVEL, FORMAT,__VA_ARGS__), \
                std::current_exception() ); \
   }

#define DP_CAPTURE_AND_RETHROW( ... ) \
   catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch( dp::exception& er ) { \
      DP_RETHROW_EXCEPTION( er, warn, "", DP_FORMAT_ARG_PARAMS(__VA_ARGS__) ); \
   } catch( const std::exception& e ) {  \
      dp::exception dpe( \
