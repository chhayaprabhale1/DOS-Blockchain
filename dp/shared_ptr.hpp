...
namespace dp {
  ...
  dp::shared_ptr<T> dynamic_pointer_cast( const dp::shared_ptr<O>& t ) {
    return dp::shared_ptr<T>( dynamic_cast<T*>(t.get()), true );
  }
  template<typename T, typename O>
  dp::shared_ptr<T> static_pointer_cast( const dp::shared_ptr<O>& t ) {
    return dp::shared_ptr<T>( static_cast<T*>(t.get()), true );
  }
}
