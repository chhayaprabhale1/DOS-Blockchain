#!/bin/bash
#
# dosio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# dosio-tn_bounce.sh and dosio-tn_roll.sh scripts.

connected="0"

rundir=programs/noddos
prog=noddos
...
datadir=var/lib/node_$DOSIO_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/dosio/node_$DOSIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/dosio/node_$DOSIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
