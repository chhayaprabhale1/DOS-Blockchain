#!/bin/bash
#
# dosio-tn_roll is used to have all of the instances of the DOS daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: dosio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./dosio-tn_roll.sh is sufficient.
#

if [ -z "$DOSIO_HOME" ]; then
    echo DOSIO_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in dosd dosiod noddos; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for dosd or dosiod or noddos
    exit 1
fi
...
if [ $DD = "all" ]; then
    for DOSIO_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $DOSIO_HOME/scripts/dosio-tn_up.sh "$*"
    done
else
    bash $DOSIO_HOME/scripts/dosio-tn_up.sh "$*"
fi
unset DOSIO_RESTART_DATA_DIR
