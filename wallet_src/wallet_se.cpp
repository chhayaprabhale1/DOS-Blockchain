...
#include <dos/wallet_header/wallet_se.hpp>
#include <dos/wallet_plugin/user_auth.h>
...
#define STR(A) #A
   ...
   bool remove_key(string public_key) {
      ...
      macos_user_auth(auth_callback, &prom, CFSTR("remove a key from your DOS wallet"));
      if(!fut.get())
...
void se_wallet::unlock(string password) {
   promise<bool> prom;
   future<bool> fut = prom.get_future();
   macos_user_auth(detail::auth_callback, &prom, CFSTR("unlock your DOS wallet"));
   if(!fut.get())
      DP_THROW_EXCEPTION(chain::wallet_invalid_password_exception, "Local user authentication failed");
   my->locked = false;
}
