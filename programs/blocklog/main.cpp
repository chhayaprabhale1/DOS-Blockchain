...
void blocklog::read_log() {
   ...
   DOS_ASSERT( end, block_log_exception, "No blocks found in block log" );
   DOS_ASSERT( end->block_num() > 1, block_log_exception, "Only one block found in block log" );
   ...
   const dp::microseconds deadline = dp::seconds(10);
   auto print_block = [&](signed_block_ptr& next) {
      ...
      const auto enhanced_object = dp::mutable_variant_object
                 ("block_num",next->block_num())
                 ("id", block_id)
                 ("ref_block_prefix", ref_block_prefix)
                 (pretty_output.get_object());
      dp::variant v(std::move(enhanced_object));
       if (no_pretty_print)
          dp::json::to_stream(*out, v, dp::json::stringify_large_ints_and_doubles);
       else
...
int main(int argc, char** argv)
{
   ...
   options_description cli ("dosio-blocklog command line options");
   try {
      blocklog blog;
