/**
  @defgroup DOSclienttool

  @section intro Introduction to clDOS
...
Options:
  ...
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where node is running
...
Subcommands:
  ...
  transfer                    Transfer DOS from account to account
...
Usage: ./cldos create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./cldos create account
Create a new account on the blockchain
Usage: ./cldos create account [OPTIONS] creator name OwnerKey ActiveKey
...
DOSio::client::http::http_context context;
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      DOSio::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = DOSio::chain::authority{1, {{owner, 1}}, {}},
         .active       = DOSio::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } DOS_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "DOS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } DOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      DOS_ASSERT( DOSio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( const string& code, const string& s ) {
   static map<DOSio::chain::symbol_code, DOSio::chain::symbol> cache;
   auto a = asset::from_string( s );
   DOSio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<DOSio::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.insert(make_pair( sym, result.max_supply.get_symbol() ));
         it = p.first;
      } else {
         DOS_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      DOS_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( "DOSio.token", s );
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } DOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of DOS delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of DOS delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_DOS,
                                   (localized("The amount of RAM bytes to purchase for the new account in DOS")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake DOS to receiver")));
      }
      ...
      createAccount->set_callback([this] {
            ...
            } DOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
               active_key = public_key_type(active_key_str);
            } DOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str));
            auto create = create_newaccount(creator, account_name, owner_key, active_key);
            if (!simple) {
               if ( buy_ram_DOS.empty() && buy_ram_bytes_in_kbytes == 0) {
                  std::cerr << "ERROR: Either --buy-ram or --buy-ram-kbytes with non-zero value is required" << std::endl;
                  return;
               }
               action buyram = !buy_ram_DOS.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_DOS))
...
struct vote_producers_subcommand {
   string voter_str;
   vector<DOSio::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<DOSio::chain_apis::read_only::get_table_rows_result>();
            ...
            DOS_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<DOSio::name> prods;
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->set_callback([&] {
         ...
         } DOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of DOS to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of DOS to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of DOS to buyram"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake DOS to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of DOS to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of DOS to undelegate for CPU bandwidth"))->required();
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of DOS to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive DOS for sold RAM"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   if (!json_format) {
      ...
      unordered_map<name, DOSio::chain_apis::permission> cache;
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodDOS responding to the request is different than core symbol built into clDOS
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'DOSio::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = DOSio::client::http::create_http_context();

   CLI::App app{"Command Line Interface to DOSIO Client"};
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", DOSio::client::config::version_str)) << std::endl;
   });
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older node which returns malformed binary wasm") << std::endl;
         }
         else
   ...
   // Transfer subcommand
   string con = "DOSio.token";
   string sender;
   string recipient;
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer DOS from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending DOS"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving DOS"))->required();
   transfer->add_option("amount", amount, localized("The amount of DOS to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         DOS_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   propose_action->set_callback([&] {
      ...
      } DOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } DOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            DOS_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "DOSio.msig", "propose", variant_to_bin( N(DOSio.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, dp::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", DOSio::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << dp::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "DOSio.msig", action, variant_to_bin( N(DOSio.msig), action, args ) }});
   };
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            DOS_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "DOSio.msig", "cancel", variant_to_bin( N(DOSio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            DOS_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "DOSio.msig", "exec", variant_to_bin( N(DOSio.msig), N(exec), args ) }});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "DOSio.sudo", "exec", variant_to_bin( N(DOSio.sudo), N(exec), args ) }});
   });

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send DOSio.system contract action to the blockchain."), false);
