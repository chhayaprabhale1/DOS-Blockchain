...
void
CUBd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string CUBio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), CUBio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (CUBd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (CUBd_def &node, host_def *host);
  void launch (CUBd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, CUBd_def> find_node(uint16_t node_num);
  vector<pair<host_def, CUBd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodCUB does not require transaction signatures.")
    ("nodCUB", bpo::value<string>(&CUBd_extra_args), "forward nodCUB command line argument(s) to each instance of nodCUB, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodCUB instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
    ...
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &CUBd : binding.instances) {
          ...
          CUBd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(CUBd.p2p_port);

          aliases.push_back (CUBd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("CUBIO_HOME");
...
void
launcher_def::assign_name (CUBd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.CUBio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      CUBd_def CUBd;
      assign_name(CUBd, i == 0);
      aliases.push_back(CUBd.name);
      CUBd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(CUBd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->CUBio_home =
          (local_id.contains (lhost->host_name) || servers.default_CUBio_home.empty()) ?
          erd : servers.default_CUBio_home;
        host_ndx++;
      } // ph_count == 0

      CUBd_def CUBd;
      assign_name(CUBd, do_bios);
      CUBd.has_db = false;
      ...
      aliases.push_back(CUBd.name);
      CUBd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(CUBd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  CUBd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->CUBio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->CUBio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->CUBio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->CUBio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->CUBio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = CUBio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = CUBio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = CUBio::net_plugin\n";
  } else {
    cfg << "plugin = CUBio::bnet_plugin\n";
  }
  cfg << "plugin = CUBio::chain_api_plugin\n"
      << "plugin = CUBio::history_api_plugin\n";
  cfg.close();
}
...
void
launcher_def::prep_remote_config_dir (CUBd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->CUBio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->CUBio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->CUBio_home;

  cmd = "cd " + host->CUBio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->CUBio_home
...
void
launcher_def::launch (CUBd_def &instance, string &gts) {
  ...
  string CUBdcmd = "programs/nodCUB/nodCUB ";
  if (skip_transaction_signatures) {
    CUBdcmd += "--skip-transaction-signatures ";
  }
  if (!CUBd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +CUBio::mongo_db_plugin");
       string args = std::regex_replace (CUBd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       CUBdcmd += args + " ";
    }
    else {
       CUBdcmd += CUBd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    CUBdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  CUBdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  CUBdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    CUBdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->CUBio_home + "; nohup " + CUBdcmd + " > "
    ...
    string cmd = "cd " + host->CUBio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << CUBdcmd << endl;

    bp::child c(CUBdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << CUBdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(CUBd_def, string sig_opt) {
}
#endif
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const CUBd_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.CUBio_home + "; "
                 + "export CUBIO_HOME=" + host.CUBio_home + string("; ")
                 + "export CUBIO_NODE=" + node_num + "; "
                 + "./scripts/CUBio-tn_bounce.sh " + CUBd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.CUBio_home + "; "
                 + "export CUBIO_HOME=" + host.CUBio_home + "; "
                 + "export CUBIO_NODE=" + node_num + "; "
         + "export CUBIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/CUBio-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the CUBio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the CUBio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the CUBio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << CUBio::launcher::config::version_str << endl;
      return 0;
    }
