/**
 *  @file
 *  @copyright defined in dosio/LICENSE.txt
 */
...
void configure_logging(const bfs::path& config_path)
{
   try {
      try {
         dp::configure_logging(config_path);
      } catch (...) {
...
void logging_conf_loop()
{
   ...
   sighup_set->async_wait([sighup_set](const boost::system::error_code& err, int /*num*/) {
      if(!err)
      {
         ...
         if(dp::exists(config_path))
            ::detail::configure_logging(config_path);
         for(auto iter : dp::get_appender_map())
...
void initialize_logging()
{
   auto config_path = app().get_logging_conf();
   if(dp::exists(config_path))
     dp::configure_logging(config_path); // intentionally allowing exceptions to escape
   for(auto iter : dp::get_appender_map())
     iter.second->initialize(app().get_io_service());
...
{
   try {
      app().set_version(dosio::noddos::config::version);

      auto root = dp::app_path();
      app().set_default_data_dir(root / "dosio/noddos/data" );
      app().set_default_config_dir(root / "dosio/noddos/config" );
      ...
      initialize_logging();
      ilog("noddos version ${ver}", ("ver", app().version_string()));
      ilog("dosio root is ${root}", ("root", root.string()));
      ilog("noddos using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("noddos data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
      ...
      if( e.code() == dp::std_exception_code ) {
