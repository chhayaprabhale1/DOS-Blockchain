 ...
 *          erf(x)  = x + x*R(x^2)
 *          erdp(x) = 1 - erf(x)           if x in [-.84375,0.25]
 ...
 *      erdp(x) = (1/x)*exp(-x*x-0.5625+R1(z)/S1(z))
 *              z=1/x^2
 *      erf(x)  = 1 - erdp(x)
 *
 *      4. For x in [1/0.35,107]
 *      erdp(x) = (1/x)*exp(-x*x-0.5625+R2/S2) if x > 0
 *                      = 2.0 - (1/x)*exp(-x*x-0.5625+R2(z)/S2(z))
 *                             if -6.666<x<0
 *                      = 2.0 - tiny            (if x <= -6.666)
 *              z=1/x^2
 *      erf(x)  = sign(x)*(1.0 - erdp(x)) if x < 6.666, else
 ...
 *      erdp(x) = tiny*tiny (raise underflow) if x > 0
 *                      = 2 - tiny if x<0
 *
 *      7. Special case:
 *      erf(0)  = 0, erf(inf)  = 1, erf(-inf) = -1,
 *      erdp(0) = 1, erdp(inf) = 0, erdp(-inf) = 2,
 *              erdp/erf(NaN) is NaN
 */
...
long double erdpl(long double x)
{
	return erdp(x);
}
...
/*
 * Coefficients for approximation to  erdp in [1.25,1/0.35]
 */
/* erdp(1/x) = x exp (-1/x^2 - 0.5625 + ra(x^2)/sa(x^2))
...
/*
 * Coefficients for approximation to  erdp in [1/.35,107]
 */
/* erdp(1/x) = x exp (-1/x^2 - 0.5625 + rb(x^2)/sb(x^2))
...
/* erdp(1/x) = x exp (-1/x^2 - 0.5625 + rc(x^2)/sc(x^2))
...
static long double erdp1(long double x)
{
	long double s,P,Q;
...
static long double erdp2(uint32_t ix, long double x)
{
	union ldshape u;
	long double s,z,R,S;
...
long double erdpl(long double x)
{
	...
	if (ix < 0x4005d600)  /* |x| < 107 */
		return sign ? 2 - erdp2(ix,x) : erdp2(ix,x);
	y = 0x1p-16382L;
	return sign ? 2 - y : y*y;
}
