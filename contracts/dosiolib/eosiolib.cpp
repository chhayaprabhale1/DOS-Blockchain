...
namespace dosio {
   ...
   void set_blockchain_parameters(const dosio::blockchain_parameters& params) {
      char buf[sizeof(dosio::blockchain_parameters)];
      dosio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(dosio::blockchain_parameters& params) {
      char buf[sizeof(dosio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      dosio_assert( size <= sizeof(buf), "buffer is too small" );
      dosio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            dosio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return dosio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = dosio::memory_heap.malloc(count*size);
...
{
   return dosio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return dosio::memory_heap.free(ptr);
}
