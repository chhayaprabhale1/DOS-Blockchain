...
#define N(X) ::dosio::string_to_name(#X)
namespace dosio {
    ...
    * ```
    *
    * For this to work the Actions must be derived from dosio::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return dosio::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from dosio::contract
...
// Helper macro for DOSIO_API
#define DOSIO_API_CALL( r, OP, elem ) \
   case ::dosio::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      dosio::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for DOSIO_ABI
#define DOSIO_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( DOSIO_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from dosio::contract
 * 
 ...
 * Example:
 * @code
 * DOSIO_ABI( dosio::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define DOSIO_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "dosio" code account and authorized by "dosio"'s "active permission */ \
         dosio_assert(code == N(dosio), "onerror action's are only valid from the \"dosio\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            DOSIO_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: dosio_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher
