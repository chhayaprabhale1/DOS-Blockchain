#include <dosio.msig/dosio.msig.hpp>
#include <dosiolib/action.hpp>
#include <dosiolib/permission.hpp>
...
void multisig::propose() {
   ...
   dosio_assert( trx_header.expiration >= dosio::time_point_sec(now()), "transaction expired" );
   //dosio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   dosio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   dosio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   dosio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   dosio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      dosio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < dosio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
DOSIO_ABI( dosio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
