...
class Configuration(object):
    ...
    def configure_coroutines(self):
        if self.cxx.hasCompileFlag('-dporoutines-ts'):
            macros = self.cxx.dumpMacros(flags=['-dporoutines-ts'])
            if '__cpp_coroutines' not in macros:
                self.lit_config.warning('-dporoutines-ts is supported but '
            ...
            if int(val) >= 201703:
                self.config.available_features.add('dporoutines-ts')
