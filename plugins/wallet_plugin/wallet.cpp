 ...
 */
#include <dosio/wallet_plugin/wallet.hpp>
...
#include <dp/container/deque.hpp>
#include <dp/crypto/elliptic.hpp>
#include <dp/git_revision.hpp>
#include <dp/io/json.hpp>
#include <dp/crypto/aes.hpp>
...
namespace dosio { namespace wallet {
                                         ...
                                         int sequence_number )
{
   ...
   dp::sha512 h = dp::sha512::hash(prefix_string + " " + sequence_string);
   return private_key_type::regenerate<dp::ecc::private_key_shim>(dp::sha256::hash(h));
}
...
public:
   ...
   void encrypt_keys()
   {
      if( !is_locked() )
      {
         ...
         auto plain_txt = dp::raw::pack(data);
         _wallet.cipher_keys = dp::aes_encrypt( data.checksum, plain_txt );
      }
   }

   bool copy_wallet_file( string destination_filename )
   {
      dp::path src_path = get_wallet_filename();
      if( !dp::exists( src_path ) )
         return false;
      dp::path dest_path = destination_filename + _wallet_filename_extension;
      int suffix = 0;
      while( dp::exists(dest_path) )
      {
         ++suffix;
      ...
      dp::path dest_parent = dp::absolute(dest_path).parent_path();
      try
      {
         enable_umask_protection();
         if( !dp::exists( dest_parent ) )
            dp::create_directories( dest_parent );
         dp::copy( src_path, dest_path );
   ...
   private_key_type get_private_key(const public_key_type& id)const
   {
      ...
      DOS_ASSERT( has_key, chain::key_nonexistent_exception, "Key doesn't exist!" );
      return *has_key;
   }
   ...
   bool import_key(string wif_key)
   {
      ...
      dosio::chain::public_key_type wif_pub_key = priv.get_public_key();
      ...
      DOS_THROW( chain::key_exist_exception, "Key already in wallet" );
   }
   ...
   bool remove_key(string key)
   {
      ...
      DOS_THROW( chain::key_nonexistent_exception, "Key not in wallet" );
   }
   ...
   string create_key(string key_type)
   {
      ...
      if(key_type == "K1")
         priv_key = dp::crypto::private_key::generate<dp::ecc::private_key_shim>();
      else if(key_type == "R1")
         priv_key = dp::crypto::private_key::generate<dp::crypto::r1::private_key_shim>();
      else
         DOS_THROW(chain::unsupported_key_type_exception, "Key type \"${kt}\" not supported by software wallet", ("kt", key_type));
   ...
   bool load_wallet_file(string wallet_filename = "")
   {
      ...
      if( ! dp::exists( wallet_filename ) )
         return false;

      _wallet = dp::json::from_file( wallet_filename ).as< wallet_data >();
   ...
   void save_wallet_file(string wallet_filename = "")
   {
      ...
      string data = dp::json::to_pretty_string( _wallet );
      try
      {
         enable_umask_protection();
         //
         ...
         if (!outfile) {
            ...
            DOS_THROW(wallet_exception, "Unable to open file: ${fn}", ("fn", wallet_filename));
         }
...
{
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "Unable to import key on a locked wallet");
...
{
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "Unable to remove key from a locked wallet");
...
{
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "Unable to create key on a locked wallet");
...
{ try {
   DOS_ASSERT( !is_locked(), wallet_locked_exception, "Unable to lock a locked wallet" );
   encrypt_keys();
   for( auto key : my->_keys )
      key.second = private_key_type();

   my->_keys.clear();
   my->_checksum = dp::sha512();
} DP_CAPTURE_AND_RETHROW() }

void soft_wallet::unlock(string password)
{ try {
   DP_ASSERT(password.size() > 0);
   auto pw = dp::sha512::hash(password.c_str(), password.size());
   vector<char> decrypted = dp::aes_decrypt(pw, my->_wallet.cipher_keys);
   auto pk = dp::raw::unpack<plain_keys>(decrypted);
   DP_ASSERT(pk.checksum == pw);
   my->_keys = std::move(pk.keys);
   my->_checksum = pk.checksum;
} DOS_RETHROW_EXCEPTIONS(chain::wallet_invalid_password_exception,
...
{
   if( !is_new() )
      DOS_ASSERT( !is_locked(), wallet_locked_exception, "The wallet must be unlocked before the password can be set" );
   my->_checksum = dp::sha512::hash( password.c_str(), password.size() );
   lock();
}

map<public_key_type, private_key_type> soft_wallet::list_keys()
{
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list public keys of a locked wallet");
   return my->_keys;
}

flat_set<public_key_type> soft_wallet::list_public_keys() {
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list private keys of a locked wallet");
...
pair<public_key_type,private_key_type> soft_wallet::get_private_key_from_password( string account, string role, string password )const {
   ...
   DOS_ASSERT( seed.size(), wallet_exception, "seed should not be empty" );
   auto secret = dp::sha256::hash( seed.c_str(), seed.size() );
   auto priv = private_key_type::regenerate<dp::ecc::private_key_shim>( secret );
