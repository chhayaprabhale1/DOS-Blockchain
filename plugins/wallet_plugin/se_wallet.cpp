 ...
 */
#include <dosio/wallet_plugin/se_wallet.hpp>
#include <dosio/wallet_plugin/macos_user_auth.h>
...
struct se_wallet_impl {
   ...
   static public_key_data get_public_key_data(SecKeyRef key) {
      ...
      if(error) {
         ...
         CFRelease(error);
         DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to get public key from Secure Enclave: ${m}", ("m", error_string));
      }
...
#define STR(A) #A
   ...
   public_key_type create() {
      ...
      if(error_string.size())
         DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to create key in Secure Enclave: ${m}", ("m", error_string));
   ...
   optional<signature_type> try_sign_digest(const digest_type d, const public_key_type public_key) {
      ...
      if(error) {
         ...
         DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to sign digest in Secure Enclave: ${m}", ("m", error_string));
      }
   ...
   bool remove_key(string public_key) {
      ...
      if(it == _keys.end())
         DP_THROW_EXCEPTION(chain::wallet_exception, "Given key to delete not found in Secure Enclave wallet");

      promise<bool> prom;
      future<bool> fut = prom.get_future();
      macos_user_auth(auth_callback, &prom, CFSTR("remove a key from your DOSIO wallet"));
      if(!fut.get())
         DP_THROW_EXCEPTION(chain::wallet_invalid_password_exception, "Local user authentication failed");
      ...
      if(ret)
         DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to getremove key from Secure Enclave");
...
static void check_signed() {
   ...
   if(is_valid != errSecSuccess) {
      ...
      DOS_THROW(secure_enclave_exception, "");
   }
}
...
se_wallet::se_wallet() : my(new detail::se_wallet_impl()) {
   ...
   DOS_THROW(secure_enclave_exception, "Secure Enclave not supported on this hardware");
}
...
private_key_type se_wallet::get_private_key(public_key_type pubkey) const {
   DP_THROW_EXCEPTION(chain::wallet_exception, "Obtaining private key for a key stored in Secure Enclave is impossible");
}
...
void se_wallet::lock() {
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "You can not lock an already locked wallet");
   my->locked = true;
}
...
void se_wallet::unlock(string password) {
   promise<bool> prom;
   future<bool> fut = prom.get_future();
   macos_user_auth(detail::auth_callback, &prom, CFSTR("unlock your DOSIO wallet"));
   if(!fut.get())
      DP_THROW_EXCEPTION(chain::wallet_invalid_password_exception, "Local user authentication failed");
   my->locked = false;
}
...
void se_wallet::set_password(string password) {
   DP_THROW_EXCEPTION(chain::wallet_exception, "Secure Enclave wallet cannot have a password set");
}

map<public_key_type, private_key_type> se_wallet::list_keys() {
   DP_THROW_EXCEPTION(chain::wallet_exception, "Getting the private keys from the Secure Enclave wallet is impossible");
}
...
bool se_wallet::import_key(string wif_key) {
   DP_THROW_EXCEPTION(chain::wallet_exception, "It is not possible to import a key in to the Secure Enclave wallet");
}
...
bool se_wallet::remove_key(string key) {
   DOS_ASSERT(!is_locked(), wallet_locked_exception, "You can not remove a key from a locked wallet");
