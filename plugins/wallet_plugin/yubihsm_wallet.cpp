...
#include <dosio/wallet_plugin/yubihsm_wallet.hpp>
#include <dosio/chain/exceptions.hpp>
#include <dosio/wallet_plugin/yubihsm.h>
...
struct yubihsm_api {
   ...
   struct yubihsm_shlib {
      yubihsm_shlib() {
         ...
         if(!_handle)
            DP_THROW("Failed to load libyubihsm: ${m}", ("m", dlerror()));
      }
      ~yubihsm_shlib() {
         dlclose(_handle);
      }
      ...
      func_ptr operator[](const char* import_name) const {
         ...
         char* error;
         if((error = dlerror()))
            DP_THROW("Failed to import ${i} from libyubihsm: ${m}", ("i", import_name)("m", error));
         return func_ptr(ret);
      }
...
struct yubihsm_wallet_impl {
   ...
   yubihsm_wallet_impl(const string& ep, const uint16_t ak) : endpoint(ep), authkey(ak) {
      yh_rc rc;
      if((rc = api.init()))
         DP_THROW("yubihsm init failure: ${c}", ("c", api.strerror(rc)));
   }
   ...
   key_map_type::iterator populate_key_map_with_keyid(const uint16_t key_id) {
      ...
      if((rc = api.util_get_pubkey(session, key_id, blob, &blob_sz, nullptr)))
         DP_THROW_EXCEPTION(chain::wallet_exception, "yh_util_get_pubkey failed: ${m}", ("m", api.strerror(rc)));
      if(blob_sz != 64)
         DP_THROW_EXCEPTION(chain::wallet_exception, "unexpected pubkey size from yh_util_get_pubkey");
      ...
      dp::datastream<const char *> ds(serialized_pub_key, sizeof(serialized_pub_key));
      dp::raw::unpack(ds, pub_key);
   ...
   void unlock(const string& password) {
      ...
      try {
         if((rc = api.init_connector(endpoint.c_str(), &connector)))
            DP_THROW_EXCEPTION(chain::wallet_exception, "Failled to initialize yubihsm connector URL: ${c}", ("c", api.strerror(rc)));
         if((rc = api.connect_best(&connector, 1, NULL)))
            DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to connect to YubiHSM connector: ${m}", ("m", api.strerror(rc)));
         if((rc = api.create_session_derived(connector, authkey, (const uint8_t *)password.data(), password.size(), false, context, sizeof(context), &session)))
            DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to create YubiHSM session: ${m}", ("m", api.strerror(rc)));
         if((rc = api.authenticate_session(session, context, sizeof(context))))
            DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to authenticate YubiHSM session: ${m}", ("m", api.strerror(rc)));

         yh_object_descriptor authkey_desc;
         if((rc = api.util_get_object_info(session, authkey, YH_AUTHKEY, &authkey_desc)))
            DP_THROW_EXCEPTION(chain::wallet_exception, "Failed to get authkey info: ${m}", ("m", api.strerror(rc)));
         ...
         if(!api.check_capability(&authkey_caps, "asymmetric_sign_ecdsa"))
            DP_THROW_EXCEPTION(chain::wallet_exception, "Given authkey cannot perform signing");
         ...
         if((rc = api.util_list_objects(session, 0, YH_ASYMMETRIC, 0, &find_caps, YH_ALGO_EC_P256, nullptr, found_objs, &found_objects_n)))
            DP_THROW_EXCEPTION(chain::wallet_exception, "yh_util_list_objects failed: ${m}", ("m", api.strerror(rc)));
   ...
   optional<signature_type> try_sign_digest(const digest_type d, const public_key_type public_key) {
      ...
      if((rc = api.util_sign_ecdsa(session, it->second, (uint8_t*)d.data(), d.data_size(), der_sig, &der_sig_sz))) {
         lock();
         DP_THROW_EXCEPTION(chain::wallet_exception, "yh_util_sign_ecdsa failed: ${m}", ("m", api.strerror(rc)));
      }

      ///XXX a lot of this below is similar to SE wallet; commonize it in non-junky way
      dp::ecdsa_sig sig = ECDSA_SIG_new();
      ...
      dp::datastream<char *> eds(pub_key_shim_data, sizeof(pub_key_shim_data));
      dp::raw::pack(eds, it->first);
      ...
      dp::datastream<const char *> ds(serialized_signature, sizeof(serialized_signature));
      dp::raw::unpack(ds, final_signature);
      return final_signature;
   }

   public_key_type create() {
      if(!api.check_capability(&authkey_caps, "asymmetric_gen"))
         DP_THROW_EXCEPTION(chain::wallet_exception, "Given authkey cannot create keys");
      ...
      if(api.capabilities_to_num("asymmetric_sign_ecdsa:export_under_wrap", &creation_caps))
         DP_THROW_EXCEPTION(chain::wallet_exception, "Cannot create caps mask");

      try {
         if((rc = api.util_generate_key_ec(session, &new_key_id, "kdosd created key", authkey_domains, &creation_caps, YH_ALGO_EC_P256)))
            DP_THROW_EXCEPTION(chain::wallet_exception, "yh_util_generate_key_ec failed: ${m}", ("m", api.strerror(rc)));
   ...
   dp::ec_key key = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1);
...
private_key_type yubihsm_wallet::get_private_key(public_key_type pubkey) const {
   DP_THROW_EXCEPTION(chain::wallet_exception, "Obtaining private key for a key stored in YubiHSM is impossible");
}
...
void yubihsm_wallet::set_password(string password) {
   DP_THROW_EXCEPTION(chain::wallet_exception, "YubiHSM wallet cannot have a password set");
}

map<public_key_type, private_key_type> yubihsm_wallet::list_keys() {
   DP_THROW_EXCEPTION(chain::wallet_exception, "Getting the private keys from the YubiHSM wallet is impossible");
}
...
bool yubihsm_wallet::import_key(string wif_key) {
   DP_THROW_EXCEPTION(chain::wallet_exception, "It is not possible to import a key in to the YubiHSM wallet");
}
