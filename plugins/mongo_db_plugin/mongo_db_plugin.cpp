 ...
 */
#include <dosio/mongo_db_plugin/mongo_db_plugin.hpp>
...
public:
   ...
   dp::optional<boost::signals2::scoped_connection> accepted_block_connection;
   dp::optional<boost::signals2::scoped_connection> irreversible_block_connection;
   dp::optional<boost::signals2::scoped_connection> accepted_transaction_connection;
   dp::optional<boost::signals2::scoped_connection> applied_transaction_connection;
   ...
   template<typename T> dp::variant to_variant_with_abi( const T& obj );
   ...
   dp::optional<mongocxx::pool> mongo_pool;
   ...
   dp::optional<chain::chain_id_type> chain_id;
   dp::microseconds abi_serializer_max_time;
   ...
   struct abi_cache {
      ...
      dp::time_point                   last_accessed;
      dp::optional<abi_serializer>     serializer;
   };
   ...
   typedef boost::multi_index_container<abi_cache,
         indexed_by<
               ...
               ordered_non_unique< tag<by_last_access>,  member<abi_cache,dp::time_point,&abi_cache::last_accessed> >
         >
   > abi_cache_index_t;
...
void mongo_db_plugin_impl::accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   try {
      ...
      elog("DP Exception while accepted_transaction ${e}", ("e", e.to_string()));
...
void mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
      ...
      //
      // Allow logging traces if node is a producer for testing purposes, so a single noddos can do both for testing.
      //
      ...
      elog("DP Exception while applied_transaction ${e}", ("e", e.to_string()));
   } catch (std::exception& e) {
...
void mongo_db_plugin_impl::applied_irreversible_block( const chain::block_state_ptr& bs ) {
   try {
      ...
      elog("DP Exception while applied_irreversible_block ${e}", ("e", e.to_string()));
   } catch (std::exception& e) {
...
void mongo_db_plugin_impl::accepted_block( const chain::block_state_ptr& bs ) {
   try {
      ...
      elog("DP Exception while accepted_block ${e}", ("e", e.to_string()));
   } catch (std::exception& e) {
...
void mongo_db_plugin_impl::consume_blocks() {
   try {
      ...
      while (true) {
         ...
         auto start_time = dp::time_point::now();
         ...
         auto time = dp::time_point::now() - start_time;
         auto per = size > 0 ? time.count()/size : 0;
         if( time > dp::microseconds(500000) ) // reduce logging, .5 secs
            ilog( "process_applied_transaction,  time per: ${p}, size: ${s}, time: ${t}", ("s", size)("t", time)("p", per) );

         start_time = dp::time_point::now();
         ...
         time = dp::time_point::now() - start_time;
      ...
      elog("DP Exception while consuming block ${e}", ("e", e.to_string()));
   } catch (std::exception& e) {
...
void handle_mongo_exception( const std::string& desc, int line_num ) {
   ...
   try {
      try {
         ...
         elog( "mongo dp exception, ${desc}, line ${line}, ${details}",
...
optional<abi_serializer> mongo_db_plugin_impl::get_abi_serializer( account_name n ) {
   ...
   if( n.good()) {
      try {
         ...
         if( itr != abi_cache_index.end() ) {
            abi_cache_index.modify( itr, []( auto& entry ) {
               entry.last_accessed = dp::time_point::now();
            });
         ...
         if(account) {
            ...
            if( view.find( "abi" ) != view.end()) {
               try {
                  abi = dp::json::from_string( bsoncxx::to_json( view["abi"].get_document())).as<abi_def>();
               } catch (...) {
               ...
               if( n == chain::config::system_account_name ) {
                  // redefine dosio setabi.abi from bytes to abi_def
                           ...
                           abis.add_specialized_unpack_pack( "abi_def",
                                 std::make_pair<abi_serializer::unpack_function, abi_serializer::pack_function>(
                                       []( dp::datastream<const char*>& stream, bool is_array, bool is_optional ) -> dp::variant {
                                          DOS_ASSERT( !is_array && !is_optional, chain::mongo_db_exception, "unexpected abi_def");
                                          chain::bytes temp;
                                          dp::raw::unpack( stream, temp );
                                          return dp::variant( dp::raw::unpack<abi_def>( temp ) );
                                       },
                                       []( const dp::variant& var, dp::datastream<char*>& ds, bool is_array, bool is_optional ) {
                                          DOS_ASSERT( false, chain::mongo_db_exception, "never called" );
                                       }
                                 ) );
                        }
                     }
      ...
      } DP_CAPTURE_AND_LOG((n))
   }
   return optional<abi_serializer>();
}

template<typename T>
dp::variant mongo_db_plugin_impl::to_variant_with_abi( const T& obj ) {
...
void mongo_db_plugin_impl::process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   try {
      ...
      elog("DP Exception while processing accepted transaction metadata: ${e}", ("e", e.to_detail_string()));
   } catch (std::exception& e) {
...
void mongo_db_plugin_impl::process_applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
      ...
      elog("DP Exception while processing applied transaction trace: ${e}", ("e", e.to_detail_string()));
   } catch (std::exception& e) {
...
void mongo_db_plugin_impl::process_irreversible_block(const chain::block_state_ptr& bs) {
  try {
     ...
     elog("DP Exception while processing irreversible block: ${e}", ("e", e.to_detail_string()));
  } catch (std::exception& e) {
...
void mongo_db_plugin_impl::process_accepted_block( const chain::block_state_ptr& bs ) {
   try {
      ...
      elog("DP Exception while processing accepted block trace ${e}", ("e", e.to_string()));
   } catch (std::exception& e) {
...
void mongo_db_plugin_impl::_process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   ...
   auto now = std::chrono::duration_cast<std::chrono::milliseconds>(
         std::chrono::microseconds{dp::time_point::now().time_since_epoch().count()} );
   ...
   string trx_json = dp::json::to_string( v );
   ...
   try {
      ...
      try {
         trx_json = dp::prune_invalid_utf8( trx_json );
         const auto& trx_value = bsoncxx::from_json( trx_json );
         trans_doc.append( bsoncxx::builder::concatenate_doc{trx_value.view()} );
   ...
   if( t->signing_keys.valid() ) {
      signing_keys_json = dp::json::to_string( t->signing_keys->second );
   } else {
      auto signing_keys = trx.get_signature_keys( *chain_id, false, false );
      if( !signing_keys.empty() ) {
         signing_keys_json = dp::json::to_string( signing_keys );
      }
   }
   ...
   try {
      ...
      if( !_trans.update_one( make_document( kvp( "trx_id", trx_id_str ) ),
         ...
         DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", trx_id) );
      }
   } catch( ... ) {
      ...
      try {
         ...
         try {
            if( !_trans_traces.insert_one( trans_traces_doc.view() ) ) {
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", t->id) );
            }
         } catch( ... ) {
   ...
   if( write_atraces ) {
      try {
         if( !bulk_action_traces.execute() ) {
            DOS_ASSERT( false, chain::mongo_db_insert_fail,
...
void mongo_db_plugin_impl::_process_accepted_block( const chain::block_state_ptr& bs ) {
   ...
   if( store_block_states ) {
      ...
      auto json = dp::json::to_string( bhs );
      try {
         const auto& value = bsoncxx::from_json( json );
      ...
      try {
         if( update_blocks_via_block_num ) {
            if( !_block_states.update_one( make_document( kvp( "block_num", b_int32{static_cast<int32_t>(block_num)} ) ),
               ...
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${num}", ("num", block_num) );
            }
         } else {
            if( !_block_states.update_one( make_document( kvp( "block_id", block_id_str ) ),
                                           make_document( kvp( "$set", block_state_doc.view() ) ), update_opts ) ) {
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${bid}", ("bid", block_id) );
            }
         }
      } catch( ... ) {
      ...
      try {
         if( update_blocks_via_block_num ) {
            if( !_blocks.update_one( make_document( kvp( "block_num", b_int32{static_cast<int32_t>(block_num)} ) ),
               ...
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${num}", ("num", block_num) );
            }
         } else {
            if( !_blocks.update_one( make_document( kvp( "block_id", block_id_str ) ),
                                     make_document( kvp( "$set", block_doc.view() ) ), update_opts ) ) {
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${bid}", ("bid", block_id) );
            }
         }
      } catch( ... ) {
   ...
   if( store_transactions ) {
      ...
      for( const auto& receipt : bs->block->transactions ) {
         ...
         if( receipt.trx.contains<packed_transaction>() ) {
            ...
            const auto& trx = dp::raw::unpack<transaction>( raw );
      ...
      if( transactions_in_block ) {
         try {
            if( !bulk.execute() ) {
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Bulk transaction insert failed for block: ${bid}", ("bid", block_id) );
            }
         } catch( ... ) {
...
void mongo_db_plugin_impl::remove_pub_keys( const account_name& name, const permission_name& permission )
{
   ...
   try {
      auto result = _pub_keys.delete_many( make_document( kvp( "account", name.to_string()),
                                                         kvp( "permission", permission.to_string())));
      if( !result ) {
         DOS_ASSERT( false, chain::mongo_db_update_fail,
...
void create_account( mongocxx::collection& accounts, const name& name, std::chrono::milliseconds& now ) {
   ...
   try {
      if( !accounts.update_one( make_document( kvp( "name", name_str )), update.view(), update_opts )) {
         DOS_ASSERT( false, chain::mongo_db_update_fail, "Failed to insert account ${n}", ("n", name));
      }
   } catch (...) {
...
void mongo_db_plugin_impl::update_account(const chain::action& act)
{
   ...
   try {
      if( act.name == newaccount ) {
         ...
         if( account ) {
            abi_def abi_def = dp::raw::unpack<chain::abi_def>( setabi.abi );
            const string json_str = dp::json::to_string( abi_def );
            ...
            try{
               ...
               try {
                  if( !_accounts.update_one( make_document( kvp( "_id", account->view()["_id"].get_oid())),
                                             update_from.view())) {
                     DOS_ASSERT( false, chain::mongo_db_update_fail, "Failed to udpdate account ${n}", ("n", setabi.account));
                  }
               } catch( ... ) {
      ...
      if( accounts.count( make_document()) == 0 ) {
         ...
         try {
            if( !accounts.insert_one( doc.view())) {
               DOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert account ${n}",
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between noddos and MongoDB plugin thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'DOS' is used if not specified in URI."
               " Example: mongodb://127.0.0.1:27017/DOS")
         ...
         ("mongodb-filter-on", bpo::value<vector<string>>()->composing(),
          "Track actions which match receiver:action:actor. Receiver, Action, & Actor may be blank to include all. i.e. dosio:: or :transfer:  Use * or leave unspecified to include all.")
      ...
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               DOS_ASSERT( false, chain::plugin_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
                                 " --mongodb-wipe will remove all DOS collections from mongodb." );
            }
         }

         if( options.count( "abi-serializer-max-time-ms") == 0 ) {
            DOS_ASSERT(false, chain::plugin_config_exception, "--abi-serializer-max-time-ms required as default value not appropriate for parsing full blocks");
         }
         ...
         if( options.count( "mongodb-abi-cache-size" )) {
            ...
            DOS_ASSERT( my->abi_cache_size > 0, chain::plugin_config_exception, "mongodb-abi-cache-size > 0 required" );
         }
         ...
         if( options.count( "mongodb-filter-on" )) {
            ...
            for( auto& s : fo ) {
               ...
               DOS_ASSERT( v.size() == 3, dp::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-on", ("s", s));
         ...
         if( options.count( "mongodb-filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               DOS_ASSERT( v.size() == 3, dp::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
         ...
         wlog( "dosio::mongo_db_plugin configured, but no --mongodb-uri specified." );
