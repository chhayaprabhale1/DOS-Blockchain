...
#include <dosio/login_plugin/login_plugin.hpp>
   ...
   {                                                                                                                   \
      std::string("/v1/login/" #call_name), [this](string, string body, url_response_callback cb) mutable {            \
         try {                                                                                                         \
            if (body.empty())                                                                                          \
               body = "{}";                                                                                            \
            auto result = call_name(dp::json::from_string(body).as<login_plugin::call_name##_params>());               \
...
login_plugin::start_login_request(const login_plugin::start_login_request_params& params) {
   my->expire_requests();
   DOS_ASSERT(params.expiration_time > dp::time_point::now(), dp::timeout_exception,
              "Requested expiration time ${expiration_time} is in the past",
              ("expiration_time", params.expiration_time));
   DOS_ASSERT(my->requests.size() < my->max_login_requests, dp::timeout_exception, "Too many pending login requests");
   ...
   request.expiration_time =
       std::min(params.expiration_time, dp::time_point_sec{dp::time_point::now()} + my->max_login_timeout);
...
login_plugin::finalize_login_request(const login_plugin::finalize_login_request_params& params) {
   ...
   dp::raw::pack(sig_data_ds, params.permission);
   dp::raw::pack(sig_data_ds, shared_secret);
   dp::raw::pack(sig_data_ds, params.data);
   ...
   try {
      ...
      chain.get_authorization_manager().check_authorization( //
          params.permission.actor, params.permission.permission, result.recovered_keys, {}, dp::microseconds(0),
          noop_checktime, true);
