...
#include <dosio/state_history_plugin/state_history_log.hpp>
#include <dosio/state_history_plugin/state_history_serialization.hpp>
...
struct state_history_plugin_impl : std::enable_shared_from_this<state_history_plugin_impl> {
   ...
   dp::optional<state_history_log>                      trace_log;
   dp::optional<state_history_log>                      chain_state_log;
   bool                                                 stopping = false;
   ...
   void get_log_entry(state_history_log& log, uint32_t block_num, dp::optional<bytes>& result) {
   ...
   void get_block(uint32_t block_num, dp::optional<bytes>& result) {
      ...
      result = dp::raw::pack(*p);
   }

   dp::optional<chain::block_id_type> get_block_id(uint32_t block_num) {
   ...
   struct session : std::enable_shared_from_this<session> {
      ...
      dp::optional<get_blocks_request_v0>        current_request;
      ...
      template <typename T>
      void send(T obj) {
         send_queue.push_back(dp::raw::pack(state_result{std::move(obj)}));
         send();
      }
   ...
   void listen() {
      ...
      auto check_ec = [&](const char* what) {
         if (!ec)
            return;
         elog("${w}: ${m}", ("w", what)("m", ec.message()));
         DOS_ASSERT(false, plugin_exception, "unable to open listen socket");
      };
   ...
   static bool is_onblock(const transaction_trace_ptr& p) {
      ...
      if (act.account != dosio::chain::config::system_account_name || act.name != N(onblock) ||
          act.authorization.size() != 1)
         return false;
      auto& auth = act.authorization[0];
      return auth.actor == dosio::chain::config::system_account_name &&
             auth.permission == dosio::chain::config::active_name;
   }
   ...
   void store_traces(const block_state_ptr& block_state) {
      ...
      for (auto& r : block_state->block->transactions) {
         ...
         DOS_ASSERT(it != cached_traces.end() && it->second->receipt, plugin_exception,
      ...
      auto  traces_bin = zlib_compress_bytes(dp::raw::pack(make_history_serial_wrapper(db, traces)));
      DOS_ASSERT(traces_bin.size() == (uint32_t)traces_bin.size(), plugin_exception, "traces is too big");
   ...
   void store_chain_state(const block_state_ptr& block_state) {
      ...
      auto get_table_id = [&](uint64_t tid) -> const table_id_object& {
         ...
         DOS_ASSERT(it != removed_table_id.end(), chain::plugin_exception, "can not found table id ${tid}",
                    ("tid", tid));
         return *it->second;
      };

      auto pack_row          = [&](auto& row) { return dp::raw::pack(make_history_serial_wrapper(db, row)); };
      auto pack_contract_row = [&](auto& row) {
         return dp::raw::pack(make_history_context_wrapper(db, get_table_id(row.t_id._id), row));
      };
      ...
      auto deltas_bin = zlib_compress_bytes(dp::raw::pack(deltas));
      DOS_ASSERT(deltas_bin.size() == (uint32_t)deltas_bin.size(), plugin_exception, "deltas is too big");
      state_history_log_header header{.block_num    = block_state->block->block_num(),
                                      .block_id     = block_state->block->id(),
...
void state_history_plugin::plugin_initialize(const variables_map& options) {
   try {
      DOS_ASSERT(options.at("disable-replay-opts").as<bool>(), plugin_exception,
