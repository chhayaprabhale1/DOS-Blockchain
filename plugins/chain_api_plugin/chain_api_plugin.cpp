 ...
 */
#include <dosio/chain_api_plugin/chain_api_plugin.hpp>
...
struct async_result_visitor : public dp::visitor<std::string> {
   template<typename T>
   std::string operator()(const T& v) const {
      return dp::json::to_string(v);
   }
};
...
{std::string("/v1/" #api_name "/" #call_name), \
   [api_handle](string, string body, url_response_callback cb) mutable { \
          ...
          try { \
             if (body.empty()) body = "{}"; \
             auto result = api_handle.call_name(dp::json::from_string(body).as<api_namespace::call_name ## _params>()); \
...
{std::string("/v1/" #api_name "/" #call_name), \
   [api_handle](string, string body, url_response_callback cb) mutable { \
      if (body.empty()) body = "{}"; \
      api_handle.validate(); \
      api_handle.call_name(dp::json::from_string(body).as<api_namespace::call_name ## _params>(),\
         [cb, body](const dp::static_variant<dp::exception_ptr, call_result>& result){\
            if (result.contains<dp::exception_ptr>()) {\
               try {\
                  result.get<dp::exception_ptr>()->dynamic_rethrow_exception();\
               } catch (...) {\
                  http_plugin::handle_exception(#api_name, #call_name, body, cb);\
               }\
            } else {\
