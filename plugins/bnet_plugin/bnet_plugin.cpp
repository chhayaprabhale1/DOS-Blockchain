...
#include <dosio/bnet_plugin/bnet_plugin.hpp>
...
const dp::string logger_name("bnet_plugin");
dp::logger plugin_logger;
std::string peer_log_format;

#define peer_dlog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( plugin_logger.is_enabled( dp::log_level::debug ) ) \
      plugin_logger.log( DP_LOG_MESSAGE( debug, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  DP_MULTILINE_MACRO_END

#define peer_ilog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( plugin_logger.is_enabled( dp::log_level::info ) ) \
      plugin_logger.log( DP_LOG_MESSAGE( info, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  DP_MULTILINE_MACRO_END

#define peer_wlog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( plugin_logger.is_enabled( dp::log_level::warn ) ) \
      plugin_logger.log( DP_LOG_MESSAGE( warn, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  DP_MULTILINE_MACRO_END

#define peer_elog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( plugin_logger.is_enabled( dp::log_level::error ) ) \
      plugin_logger.log( DP_LOG_MESSAGE( error, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant())) ); \
  DP_MULTILINE_MACRO_END


using dosio::public_key_type;
using dosio::chain_id_type;
using dosio::block_id_type;
using dosio::block_timestamp_type;
using std::string;
using dosio::sha256;
using dosio::signed_block_ptr;
using dosio::packed_transaction_ptr;
using std::vector;
...
DP_REFLECT( hello, (peer_id)(network_version)(user)(password)(agent)(protocol_version)(chain_id)(request_transactions)(last_irr_block_num)(pending_block_ids) )

struct hello_extension_irreversible_only {};

DP_REFLECT( hello_extension_irreversible_only, BOOST_PP_SEQ_NIL )

using hello_extension = dp::static_variant<hello_extension_irreversible_only>;
...
DP_REFLECT( trx_notice, (signed_trx_id) )
...
DP_REFLECT( block_notice, (block_ids) );
...
DP_REFLECT( ping, (sent)(code)(lib) )

struct pong {
   dp::time_point sent;
   dp::sha256     code;
};
DP_REFLECT( pong, (sent)(code) )

using bnet_message = dp::static_variant<hello,
                                        trx_notice,
                                        block_notice,
                                        signed_block_ptr,
...
namespace dosio {
     ...
     public:
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        struct transaction_status {
           ...
           void mark_known_by_peer() { received = dp::time_point::maximum(); trx.reset();  }
           bool known_by_peer()const { return received == dp::time_point::maximum(); }
        };
        ...
        dp::sha256         _current_code;
        dp::time_point     _last_recv_ping_time = dp::time_point::now();
        ...
        void on_accepted_transaction( transaction_metadata_ptr t ) {
           ...
           if( itr != _transaction_status.end() ) {
              if( !itr->known_by_peer() ) {
                 _transaction_status.modify( itr, [&]( auto& stat ) {
                    stat.expired = std::min<dp::time_point>( dp::time_point::now() + dp::seconds(5), t->trx.expiration );
                 });
              }
              return;
           }
           ...
           stat.received = dp::time_point::now();
           stat.expired  = stat.received + dp::seconds(5);
           stat.id       = t->id;
           stat.trx      = t;
        ...
        void on_accepted_block_header( const block_state_ptr& s ) {
          ...
          // ilog( "accepted block header ${n}", ("n",s->block_num) );
           ...
           if( dp::time_point::now() - s->block->timestamp  < dp::seconds(6) ) {
        ...
        void send( const bnet_message& msg ) { try {
           auto ps = dp::raw::pack_size(msg);
           _out_buffer.resize(ps);
           dp::datastream<char*> ds(_out_buffer.data(), ps);
           dp::raw::pack(ds, msg);
           send();
        } DP_LOG_AND_RETHROW() }
        ...
        void send( const bnet_message& msg, const T& ex ) { try {
           auto ex_size = dp::raw::pack_size(ex);
           auto ps = dp::raw::pack_size(msg) + dp::raw::pack_size(unsigned_int(ex_size)) + ex_size;
           _out_buffer.resize(ps);
           dp::datastream<char*> ds(_out_buffer.data(), ps);
           dp::raw::pack( ds, msg );
           dp::raw::pack( ds, unsigned_int(ex_size) );
           dp::raw::pack( ds, ex );
           send();
        } DP_LOG_AND_RETHROW() }
        ...
        bool send_pong() {
           if( _last_recv_ping.code == dp::sha256() )
              return false;

           send( pong{ dp::time_point::now(), _last_recv_ping.code } );
           _last_recv_ping.code = dp::sha256();
           return true;
        }

        bool send_ping() {
           auto delta_t = dp::time_point::now() - _last_sent_ping.sent;
           if( delta_t < dp::seconds(3) ) return false;

           if( _last_sent_ping.code == dp::sha256() ) {
              _last_sent_ping.sent = dp::time_point::now();
              _last_sent_ping.code = dp::sha256::hash(_last_sent_ping.sent); /// TODO: make this more random
           ...
           // empty blocks...
           //
           //if( dp::time_point::now() - _last_recv_ping_time > dp::seconds(6) ) {
        ...
        void clear_expired_trx() {
           auto& idx = _transaction_status.get<by_expired>();
           auto itr = idx.begin();
           while( itr != idx.end() && itr->expired < dp::time_point::now() ) {
              idx.erase(itr);
              itr = idx.begin();
           }
        }
        ...
        void on_async_get_block( const signed_block_ptr& nextblock ) {
           verify_strand_in_this_thread(_strand, __func__, __LINE__);
            ...
            if( nextblock->timestamp > (dp::time_point::now() - dp::seconds(5)) ) {
        ...
        void on_read( boost::system::error_code ec, std::size_t bytes_transferred ) {
           ...
           try {
              ...
              dp::datastream<const char*> ds(d,s);
        ...
        void on_message( const bnet_message& msg, dp::datastream<const char*>& ds ) {
           try {
              switch( msg.which() ) {
        ...
        void on( const hello& hi, dp::datastream<const char*>& ds );

        void on( const ping& p ) {
           peer_ilog(this, "received ping");
           _last_recv_ping = p;
           _remote_lib     = p.lib;
           _last_recv_ping_time = dp::time_point::now();
        }
        ...
        void on( const pong& p ) {
           ...
           _last_sent_ping.code = dp::sha256();
        }
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              DOS_THROW(block_validate_exception, "bad block" );
           }
        ...
        bool mark_transaction_known_by_peer( const transaction_id_type& id ) {
           ...
           if( itr != _transaction_status.end() ) {
              ...
              stat.id = id;
              stat.mark_known_by_peer();
              stat.expired = dp::time_point::now()+dp::seconds(5);
              _transaction_status.insert(stat);
           }
           return false;
        }
        ...
        //   ilog( "${remote_peer}: ${msg}", ("remote_peer",dp::variant(_remote_peer_id).as_string().substr(3,5) )("msg",msg) );
        }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           DOS_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   class bnet_plugin_impl : public std::enable_shared_from_this<bnet_plugin_impl> {
      public:
         ...
         const private_key_type  _peer_pk = dp::crypto::private_key::generate(); /// one time random key to identify this process
   ...
   void bnet_plugin::plugin_startup() {
      if(dp::get_logger_map().find(logger_name) != dp::get_logger_map().end())
         plugin_logger = dp::get_logger_map()[logger_name];

      wlog( "bnet startup " );

      auto& chain = app().get_plugin<chain_plugin>().chain();
      DP_ASSERT ( chain.get_read_mode() != chain::db_read_mode::IRREVERSIBLE, "bnet is not compatible with \"irreversible\" read_mode");
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         DOS_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
   ...
   void session::on( const hello& hi, dp::datastream<const char*>& ds ) {
      ...
      if ( hi.protocol_version >= "1.0.1" ) {
         ...
         while ( 0 < ds.remaining() ) {
            unsigned_int size;
            dp::raw::unpack( ds, size ); // next extension size
            auto ex_start = ds.pos();
            dp::datastream<const char*> dsw( ex_start, size );
            unsigned_int wich;
            dp::raw::unpack( dsw, wich );
            hello_extension ex;
            if ( wich < ex.count() ) { //know extension
               dp::datastream<const char*> dsx( ex_start, size ); //unpack needs to read static_variant _tag again
               dp::raw::unpack( dsx, ex );
   ...
   void session::on( const packed_transaction_ptr& p ) {
      ...
      if (!p) {
        ...
        DOS_THROW(transaction_exception, "bad transaction");
      }
      if( !_net_plugin->_request_trx )
        return;

      // ilog( "recv trx ${n}", ("n", id) );
      if( p->expiration() < dp::time_point::now() ) return;
