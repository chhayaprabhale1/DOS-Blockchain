...
#include <dosio/http_plugin/local_endpoint.hpp>
      ...
      public:
         ...
         bool host_is_valid( const std::string& host, const string& endpoint_local_host_port, bool secure) {
            ...
            if (std::regex_search(host, has_port_expr)) {
               ...
               // according to RDP 2732 ipv6 addresses should always be enclosed with brackets so we shouldn't need to special case here
         ...
         ssl_context_ptr on_tls_init(websocketpp::connection_hdl hdl) {
            ...
            try {
               ...
               dp::ec_key ecdh = EC_KEY_new_by_curve_name(NID_secp384r1);
               if (!ecdh)
                  DOS_THROW(chain::http_exception, "Failed to set NID_secp384r1");
               if(SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY*)ecdh) != 1)
                  DOS_THROW(chain::http_exception, "Failed to set ECDH PFS");
               ...
               if(SSL_CTX_set_cipher_list(ctx->native_handle(), \
                  ...
                  DOS_THROW(chain::http_exception, "Failed to set HTTPS cipher list");
         ...
         static void handle_exception(typename websocketpp::server<T>::connection_ptr con) {
            ...
            try {
               ...
               try {
                  ...
                  con->set_body( dp::json::to_string( results ));
               } catch (const std::exception& e) {
                  err += e.what();
                  elog( "${e}", ("e", err));
                  error_results results{websocketpp::http::status_code::internal_server_error,
                                        "Internal Service Error", error_results::error_info(dp::exception( DP_LOG_MESSAGE( error, e.what())), verbose_http_errors )};
                  con->set_body( dp::json::to_string( results ));
               } catch (...) {
                  err += "Unknown Exception";
                  error_results results{websocketpp::http::status_code::internal_server_error,
                                        "Internal Service Error",
                                        error_results::error_info(dp::exception( DP_LOG_MESSAGE( error, "Unknown Exception" )), verbose_http_errors )};
                  con->set_body( dp::json::to_string( results ));
               }
            } catch (...) {
            ...
            try {
               ...
               if( handler_itr != url_handlers.end()) {
                  ...
                  error_results results{websocketpp::http::status_code::not_found,
                                        "Not Found", error_results::error_info(dp::exception( DP_LOG_MESSAGE( error, "Unknown Endpoint" )), verbose_http_errors )};
                  con->set_body( dp::json::to_string( results ));
   ...
   void http_plugin::handle_exception( const char *api_name, const char *call_name, const string& body, url_response_callback cb ) {
      try {
         try {
            ...
            cb( 500, dp::json::to_string( results ));
            if (e.code() != chain::greylist_net_usage_exceeded::code_value && e.code() != chain::greylist_cpu_usage_exceeded::code_value) {
               elog( "DP Exception encountered while processing ${api}.${call}",
                     ("api", api_name)( "call", call_name ));
               dlog( "Exception Details: ${e}", ("e", e.to_detail_string()));
            }
         } catch (std::exception& e) {
            error_results results{500, "Internal Service Error", error_results::error_info(dp::exception( DP_LOG_MESSAGE( error, e.what())), verbose_http_errors)};
            cb( 500, dp::json::to_string( results ));
