...
namespace dosio {
   ...
   struct error_results {
      ...
      struct error_info {
         ...
         error_info(const dp::exception& exc, bool include_log) {
            code = exc.code();
            name = exc.name();
            what = exc.what();
            if (include_log) {
...
DP_REFLECT(dosio::error_results::error_info::error_detail, (message)(file)(line_number)(method))
DP_REFLECT(dosio::error_results::error_info, (code)(name)(what)(details))
DP_REFLECT(dosio::error_results, (code)(message)(error))
