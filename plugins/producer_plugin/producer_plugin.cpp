 ...
 */
#include <dosio/producer_plugin/producer_plugin.hpp>
#include <dosio/chain/producer_object.hpp>
#include <dosio/chain/plugin_interface.hpp>
#include <dosio/chain/global_property_object.hpp>
#include <dosio/chain/transaction_object.hpp>
#include <dosio/chain/snapshot.hpp>

#include <dp/io/json.hpp>
#include <dp/smart_ref_impl.hpp>
#include <dp/scoped_exit.hpp>
...
const dp::string logger_name("producer_plugin");
dp::logger _log;

const dp::string trx_trace_logger_name("transaction_tracing");
dp::logger _trx_trace_log;
...
namespace {
   bool failure_is_subjective(const dp::exception& e, bool deadline_is_subjective) {
      auto code = e.code();
...
using transaction_id_with_expiry_index = multi_index_container<
   ...
   indexed_by<
      ...
      ordered_non_unique<tag<by_expiry>, BOOST_MULTI_INDEX_MEMBER(transaction_id_with_expiry, dp::time_point, expiry)>
   >
>;
...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      optional<dp::time_point> calculate_next_block_time(const account_name& producer_name, const block_timestamp_type& current_block_time) const;
      ...
      uint32_t _production_skip_flags              = 0; //dosio::chain::skip_nothing;
      ...
      dp::microseconds                                          _max_irreversible_block_age_us;
      int32_t                                                   _produce_time_offset_us = 0;
      int32_t                                                   _last_block_time_offset_us = 0;
      dp::time_point                                            _irreversible_block_time;
      dp::microseconds                                          _kdosd_provider_timeout_us;

      time_point _last_signed_block_time;
      time_point _start_time = dp::time_point::now();
      ...
      dp::optional<scoped_connection>                          _accepted_block_connection;
      dp::optional<scoped_connection>                          _irreversible_block_connection;
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
      ...
      auto publish_results_of(const Type &data, Channel& channel, F f) {
         auto publish_success = dp::make_scoped_exit([&, this](){
            channel.publish(std::pair<dp::exception_ptr, Type>(nullptr, data));
         });
         ...
         try {
            ...
            if (trace->except) {
               ...
               channel.publish(std::pair<dp::exception_ptr, Type>(trace->except->dynamic_copy_exception(), data));
            }
            return trace;
         } catch (const dp::exception& e) {
            publish_success.cancel();
            channel.publish(std::pair<dp::exception_ptr, Type>(e.dynamic_copy_exception(), data));
            throw e;
         } catch( const std::exception& e ) {
            publish_success.cancel();
            auto dpe = dp::exception(
               DP_LOG_MESSAGE( info, "Caught std::exception: ${what}", ("what",e.what())),
               dp::std_exception_code,
               BOOST_CORE_TYPEID(e).name(),
               e.what()
            );
            channel.publish(std::pair<dp::exception_ptr, Type>(dpe.dynamic_copy_exception(),data));
            throw dpe;
         } catch( ... ) {
            publish_success.cancel();
            auto dpe = dp::unhandled_exception(
               DP_LOG_MESSAGE( info, "Caught unknown exception"),
               std::current_exception()
            );
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         ...
         dp_dlog(_log, "received incoming block ${id}", ("id", id));

         DOS_ASSERT( block->timestamp < (dp::time_point::now() + dp::seconds( 7 )), block_from_the_future,
         ...
         auto ensure = dp::make_scoped_exit([this](){
         ...
         if( chain.head_block_state()->header.timestamp.next().to_time_point() >= dp::time_point::now() ) {
            _production_enabled = true;
         }


         if( dp::time_point::now() - block->timestamp < dp::minutes(5) || (block->block_num() % 1000 == 0) ) {
            ilog("Received block ${id}... #${n} @ ${t} signed by ${p} [trxs: ${count}, lib: ${lib}, conf: ${confs}, latency: ${latency} ms]",
                 ("p",block->producer)("id",dp::variant(block->id()).as_string().substr(8,16))
                 ("n",block_header::num_from_id(block->id()))("t",block->timestamp)
                 ("count",block->transactions.size())("lib",chain.last_irreversible_block_num())("confs", block->confirmed)("latency", (dp::time_point::now() - block->timestamp).count()/1000 ) );
         }
      }
      ...
      void on_incoming_transaction_async(const packed_transaction_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         ...
         auto send_response = [this, &trx, &chain, &next](const dp::static_variant<dp::exception_ptr, transaction_trace_ptr>& response) {
            next(response);
            if (response.contains<dp::exception_ptr>()) {
               _transaction_ack_channel.publish(std::pair<dp::exception_ptr, packed_transaction_ptr>(response.get<dp::exception_ptr>(), trx));
               if (_pending_block_mode == pending_block_mode::producing) {
                  dp_dlog(_trx_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${txid} : ${why} ",
                        ("block_num", chain.head_block_num() + 1)
                        ("prod", chain.pending_block_state()->header.producer)
                        ("txid", trx->id())
                        ("why",response.get<dp::exception_ptr>()->what()));
               } else {
                  dp_dlog(_trx_trace_log, "[TRX_TRACE] Speculative execution is REJECTING tx: ${txid} : ${why} ",
                          ("txid", trx->id())
                          ("why",response.get<dp::exception_ptr>()->what()));
               }
            } else {
               _transaction_ack_channel.publish(std::pair<dp::exception_ptr, packed_transaction_ptr>(nullptr, trx));
               if (_pending_block_mode == pending_block_mode::producing) {
                  dp_dlog(_trx_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is ACCEPTING tx: ${txid}",
                          ("block_num", chain.head_block_num() + 1)
                          ("prod", chain.pending_block_state()->header.producer)
                          ("txid", trx->id()));
               } else {
                  dp_dlog(_trx_trace_log, "[TRX_TRACE] Speculative execution is ACCEPTING tx: ${txid}",
                          ("txid", trx->id()));
               }
            }
         };
         ...
         auto id = trx->id();
         if( dp::time_point(trx->expiration()) < block_time ) {
            send_response(std::static_pointer_cast<dp::exception>(std::make_shared<expired_tx_exception>(DP_LOG_MESSAGE(error, "expired transaction ${id}", ("id", id)) )));
            return;
         }

         if( chain.is_known_unexpired_transaction(id) ) {
            send_response(std::static_pointer_cast<dp::exception>(std::make_shared<tx_duplicate>(DP_LOG_MESSAGE(error, "duplicate transaction ${id}", ("id", id)) )));
            return;
         }

         auto deadline = dp::time_point::now() + dp::milliseconds(_max_transaction_time_ms);
         ...
         try {
            ...
            if (trace->except) {
               if (failure_is_subjective(*trace->except, deadline_is_subjective)) {
                  _pending_incoming_transactions.emplace_back(trx, persist_until_expired, next);
                  if (_pending_block_mode == pending_block_mode::producing) {
                     dp_dlog(_trx_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} COULD NOT FIT, tx: ${txid} RETRYING ",
                             ("block_num", chain.head_block_num() + 1)
                             ("prod", chain.pending_block_state()->header.producer)
                             ("txid", trx->id()));
                  } else {
                     dp_dlog(_trx_trace_log, "[TRX_TRACE] Speculative execution COULD NOT FIT tx: ${txid} RETRYING",
                             ("txid", trx->id()));
                  }
               } else {
      ...
      dp::microseconds get_irreversible_block_age() {
         auto now = dp::time_point::now();
         if (now < _irreversible_block_time) {
            return dp::microseconds(0);
         } else {
      ...
      dp::time_point calculate_pending_block_time() const;
...
void new_chain_banner(const dosio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   if( db.head_block_state()->header.timestamp.to_time_point() < (dp::time_point::now() - dp::milliseconds(200 * config::block_interval_ms)))
   {
...
{
   auto default_priv_key = private_key_type::regenerate<dp::ecc::private_key_shim>(dp::sha256::hash(std::string("nathan")));
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "Where:\n"
          "   <public-key>    \tis a string form of a vaild DOSIO public key\n\n"
          "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
          "   <provider-type> \tis KEY, or KDOSD\n\n"
          "   KEY:<data>      \tis a string form of a valid DOSIO private key which maps to the provided public key\n\n"
          "   KDOSD:<data>    \tis the URL where kdosd is available and the approptiate wallet(s) are unlocked")
         ("kdosd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowd for sending blocks to a kdosd provider for signing")
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const dp::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    DOS_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
T dejsonify(const string& s) {
   return dp::json::from_string(s).as<T>();
}
...
static producer_plugin_impl::signature_provider_type
make_kdosd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   dp::url kdosd_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kdosd_url = dp::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), dp::optional<uint16_t>());
   else
      kdosd_url = dp::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kdosd_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         dp::variant params;
         dp::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kdosd_provider_timeout_us.count() >= 0 ? dp::time_point::now() + impl->_kdosd_provider_timeout_us : dp::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kdosd_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            DOS_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            DOS_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
            ...
            } else if (spec_type_str == "KDOSD") {
               my->_signature_providers[pubkey] = make_kdosd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kdosd_provider_timeout_us = dp::milliseconds(options.at("kdosd-provider-timeout").as<int32_t>());
   ...
   my->_max_irreversible_block_age_us = dp::seconds(options.at("max-irreversible-block-age").as<int32_t>());
   ...
   if( options.count( "snapshots-dir" )) {
      ...
      if( sd.is_relative()) {
         ...
         if (!dp::exists(my->_snapshots_dir)) {
            dp::create_directories(my->_snapshots_dir);
         }
      } else {
         my->_snapshots_dir = sd;
      }

      DOS_ASSERT( dp::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
...
{ try {
   auto& logger_map = dp::get_logger_map();
   ...
   chain::controller& chain = app().get_plugin<chain_plugin>().chain();
   DOS_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   DOS_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
   ...
   if (lib) {
      ...
      my->_irreversible_block_time = dp::time_point::maximum();
   }

   if (!my->_producers.empty()) {
      ilog("Launching block production for ${n} producers at ${time}.", ("n", my->_producers.size())("time",dp::time_point::now()));

      if (my->_production_enabled) {
         if (chain.head_block_num() == 0) {
            new_chain_banner(chain);
         }
         //_production_skip_flags |= dosio::chain::skip_undo_history_check;
      }
   }
...
void producer_plugin::update_runtime_options(const runtime_options& options) {
   ...
   if (options.max_irreversible_block_age) {
      my->_max_irreversible_block_age_us =  dp::seconds(*options.max_irreversible_block_age);
   ...
   if (options.subjective_cpu_leeway_us) {
      chain::controller& chain = app().get_plugin<chain_plugin>().chain();
      chain.set_subjective_cpu_leeway(dp::microseconds(*options.subjective_cpu_leeway_us));
   }
}
...
producer_plugin::integrity_hash_information producer_plugin::get_integrity_hash() const {
   ...
   auto reschedule = dp::make_scoped_exit([this](){
      my->schedule_production_loop();
   });
...
producer_plugin::snapshot_information producer_plugin::create_snapshot() const {
   ...
   std::string snapshot_path = (my->_snapshots_dir / dp::format_string("snapshot-${id}.bin", dp::mutable_variant_object()("id", head_id))).generic_string();

   DOS_ASSERT( !dp::is_regular_file(snapshot_path), snapshot_exists_exception,
...
optional<dp::time_point> producer_plugin_impl::calculate_next_block_time(const account_name& producer_name, const block_timestamp_type& current_block_time) const {
   chain::controller& chain = app().get_plugin<chain_plugin>().chain();
...
dp::time_point producer_plugin_impl::calculate_pending_block_time() const {
   const chain::controller& chain = app().get_plugin<chain_plugin>().chain();
   const dp::time_point now = dp::time_point::now();
   const dp::time_point base = std::max<dp::time_point>(now, chain.head_block_time());
   const int64_t min_time_to_next_block = (config::block_interval_us) - (base.time_since_epoch().count() % (config::block_interval_us) );
   dp::time_point block_time = base + dp::microseconds(min_time_to_next_block);


   if((block_time - now) < dp::microseconds(config::block_interval_us/10) ) {     // we must sleep for at least 50ms
      block_time += dp::microseconds(config::block_interval_us);
   }
   return block_time;
}
...
producer_plugin_impl::start_block_result producer_plugin_impl::start_block(bool &last_block) {
   ...
   const dp::time_point now = dp::time_point::now();
   const dp::time_point block_time = calculate_pending_block_time();
   ...
   if (_pending_block_mode == pending_block_mode::speculating) {
      ...
      if (head_block_age > dp::seconds(5))
         return start_block_result::waiting;
   }
   ...
   if (pbs) {
      ...
      if (!persisted_by_expiry.empty()) {
         ...
         while(!persisted_by_expiry.empty() && persisted_by_expiry.begin()->expiry <= pbs->header.timestamp.to_time_point()) {
            ...
            if (_pending_block_mode == pending_block_mode::producing) {
               dp_dlog(_trx_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is EXPIRING PERSISTED tx: ${txid}",
                       ("block_num", chain.head_block_num() + 1)
                       ("prod", chain.pending_block_state()->header.producer)
                       ("txid", txid));
            } else {
               dp_dlog(_trx_trace_log, "[TRX_TRACE] Speculative execution is EXPIRING PERSISTED tx: ${txid}",
                       ("txid", txid));
            }
         ...
         dp_dlog(_log, "Processed ${n} persisted transactions, Expired ${expired}",
                ("n", orig_count)
      ...
      try {
         ...
         if (_producers.empty() && persisted_by_id.empty()) {
            ...
            { // derive appliable transactions from unapplied_transactions and drop droppable transactions
               ...
               for (auto& trx: unapplied_trxs) {
                  ...
                  if (category == tx_category::EXPIRED || (category == tx_category::UNEXPIRED_UNPERSISTED && _producers.empty())) {
                     if (!_producers.empty()) {
                        dp_dlog(_trx_trace_log, "[TRX_TRACE] Node with producers configured is dropping an EXPIRED transaction that was PREVIOUSLY ACCEPTED : ${txid}",
                               ("txid", trx->id));
                     }
            ...
            if (!apply_trxs.empty()) {
               ...
               for (const auto& trx: apply_trxs) {
                  if (block_time <= dp::time_point::now()) exhausted = true;
                  if (exhausted) {
                     break;
                  }
               ...
               dp_dlog(_log, "Processed ${m} of ${n} previously applied transactions, Applied ${applied}, Failed/Dropped ${failed}",
                      ("m", num_processed)
                      ("n", apply_trxs.size())
                      ("applied", num_applied)
                      ("failed", num_failed));
         ...
         if (_pending_block_mode == pending_block_mode::producing) {
            ...
            if(!blacklist_by_expiry.empty()) {
               ...
               dp_dlog(_log, "Processed ${n} blacklisted transactions, Expired ${expired}",
                      ("n", orig_count)
                      ("expired", num_expired));
            }
            ...
            if (!scheduled_trxs.empty()) {
               ...
               for (const auto& trx : scheduled_trxs) {
                  ...
                  if (block_time <= dp::time_point::now()) {
                     exhausted = true;
                     break;
                  }
                  ...
                  try {
                     ...
                     if (trace->except) {
                        if (failure_is_subjective(*trace->except, deadline_is_subjective)) {
                           ...
                           auto expiration = dp::time_point::now() + dp::seconds(chain.get_global_properties().configuration.deferred_trx_expiration_window);
               ...
               dp_dlog(_log, "Processed ${m} of ${n} scheduled transactions, Applied ${applied}, Failed/Dropped ${failed}",
                      ("m", num_processed)
         ...
         if (exhausted || block_time <= dp::time_point::now()) {
            ...
            if (!_pending_incoming_transactions.empty()) {
               dp_dlog(_log, "Processing ${n} pending transactions");
               while (orig_pending_txn_size && _pending_incoming_transactions.size()) {
                  auto e = _pending_incoming_transactions.front();
                  _pending_incoming_transactions.pop_front();
                  --orig_pending_txn_size;
                  on_incoming_transaction_async(std::get<0>(e), std::get<1>(e), std::get<2>(e));
                  if (block_time <= dp::time_point::now()) return start_block_result::exhausted;
               }
            }
...
void producer_plugin_impl::schedule_production_loop() {
   ...
   if (result == start_block_result::failed) {
      ...
      if (!_producers.empty() && !production_disabled_by_policy()) {
         dp_dlog(_log, "Waiting till another block is received and scheduling Speculative/Production Change");
         schedule_delayed_production_loop(weak_this, calculate_pending_block_time());
      } else {
         dp_dlog(_log, "Waiting till another block is received");
      ...
      if (result == start_block_result::succeeded) {
         ...
         DOS_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state, start_block succeeded" );
         ...
         dp_dlog(_log, "Scheduling Block Production on Normal Block #${num} for ${time}", ("num", chain.pending_block_state()->block_num)("time",deadline));
      } else {
         DOS_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state" );
         auto expect_time = chain.pending_block_time() - dp::microseconds(config::block_interval_us);
         // ship this block off up to 1 block time earlier or immediately
         if (dp::time_point::now() >= expect_time) {
            _timer.expires_from_now( boost::posix_time::microseconds( 0 ));
            dp_dlog(_log, "Scheduling Block Production on Exhausted Block #${num} immediately", ("num", chain.pending_block_state()->block_num));
         } else {
            _timer.expires_at(epoch + boost::posix_time::microseconds(expect_time.time_since_epoch().count()));
            dp_dlog(_log, "Scheduling Block Production on Exhausted Block #${num} at ${time}", ("num", chain.pending_block_state()->block_num)("time",expect_time));
         }
      }
      ...
      _timer.async_wait([&chain,weak_this,cid=++_timer_corelation_id](const boost::system::error_code& ec) {
         ...
         if (self && ec != boost::asio::error::operation_aborted && cid == self->_timer_corelation_id) {
            ...
            dp_dlog(_log, "Producing Block #${num} returned: ${res}", ("num", block_num)("res", res));
         }
      });
   } else if (_pending_block_mode == pending_block_mode::speculating && !_producers.empty() && !production_disabled_by_policy()){
      dp_dlog(_log, "Specualtive Block Created; Scheduling Speculative/Production Change");
      DOS_ASSERT( chain.pending_block_state(), missing_pending_block_state, "speculating without pending_block_state" );
      const auto& pbs = chain.pending_block_state();
      schedule_delayed_production_loop(weak_this, pbs->header.timestamp);
   } else {
      dp_dlog(_log, "Speculative Block Created");
   }
}
...
void producer_plugin_impl::schedule_delayed_production_loop(const std::weak_ptr<producer_plugin_impl>& weak_this, const block_timestamp_type& current_block_time) {
   ...
   optional<dp::time_point> wake_up_time;
   for (const auto&p: _producers) {
      auto next_producer_block_time = calculate_next_block_time(p, current_block_time);
      if (next_producer_block_time) {
         auto producer_wake_up_time = *next_producer_block_time - dp::microseconds(config::block_interval_us);
         if (wake_up_time) {
            // wake up with a full block interval to the deadline
            wake_up_time = std::min<dp::time_point>(*wake_up_time, producer_wake_up_time);
         } else {
   ...
   if (wake_up_time) {
      dp_dlog(_log, "Scheduling Speculative/Production Change at ${time}", ("time", wake_up_time));
      static const boost::posix_time::ptime epoch(boost::gregorian::date(1970, 1, 1));
      ...
      dp_dlog(_log, "Not Scheduling Speculative/Production, no local producers had valid wake up times");
   }
}
...
bool producer_plugin_impl::maybe_produce_block() {
   ...
   dp_dlog(_log, "Aborting block due to produce_block error");
   chain::controller& chain = app().get_plugin<chain_plugin>().chain();
   chain.abort_block();
   return false;
}
...
static auto make_debug_time_logger() {
   auto start = dp::time_point::now();
   return dp::make_scoped_exit([=](){
      dp_dlog(_log, "Signing took ${ms}us", ("ms", dp::time_point::now() - start) );
   });
}

static auto maybe_make_debug_time_logger() -> dp::optional<decltype(make_debug_time_logger())> {
   if (_log.is_enabled( dp::log_level::debug ) ){
...
void producer_plugin_impl::produce_block() {
   //ilog("produce_block ${t}", ("t", dp::time_point::now())); // for testing _produce_time_offset_us
   DOS_ASSERT(_pending_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");
   chain::controller& chain = app().get_plugin<chain_plugin>().chain();
   const auto& pbs = chain.pending_block_state();
   const auto& hbs = chain.head_block_state();
   DOS_ASSERT(pbs, missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
   auto signature_provider_itr = _signature_providers.find( pbs->block_signing_key );

   DOS_ASSERT(signature_provider_itr != _signature_providers.end(), producer_priv_key_not_found, "Attempting to produce a block for which we don't have the private key");

   //idump( (dp::time_point::now() - chain.pending_block_time()) );
   ...
   //idump((dp::time_point::now() - hbt));

   block_state_ptr new_bs = chain.head_block_state();
   _producer_watermarks[new_bs->header.producer] = chain.head_block_num();

   ilog("Produced block ${id}... #${n} @ ${t} signed by ${p} [trxs: ${count}, lib: ${lib}, confirmed: ${confs}]",
        ("p",new_bs->header.producer)("id",dp::variant(new_bs->id).as_string().substr(0,16))
