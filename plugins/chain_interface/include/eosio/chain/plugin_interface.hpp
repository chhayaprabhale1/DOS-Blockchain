...
#include <dosio/chain/block_state.hpp>
#include <dosio/chain/transaction_metadata.hpp>
#include <dosio/chain/trace.hpp>

namespace dosio { namespace chain { namespace plugin_interface {
   using namespace dosio::chain;
   using namespace appbase;

   template<typename T>
   using next_function = std::function<void(const dp::static_variant<dp::exception_ptr, T>&)>;
   ...
   namespace compat {
      namespace channels {
         using transaction_ack       = channel_decl<struct accepted_transaction_tag, std::pair<dp::exception_ptr, packed_transaction_ptr>>;
      }
   }
