...
#include <dosio/history_plugin/account_control_history_object.hpp>
#include <dosio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(dosio::account_history_object, dosio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(dosio::action_history_object, dosio::action_history_index)
...
namespace dosio {
   ...
   class history_plugin_impl {
      public:
         ...
         void on_action_trace( const action_trace& at ) {
            if( filter( at ) ) {
               //idump((dp::json::to_pretty_string(at)));
               auto& chain = chain_plug->chain();
               chainbase::database& db = const_cast<chainbase::database&>( chain.db() ); // Override read-only access to state DB (highly unrecommended practice!)

               db.create<action_history_object>( [&]( auto& aho ) {
                  auto ps = dp::raw::pack_size( at );
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing noddos to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               DOS_ASSERT( v.size() == 3, dp::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               DOS_ASSERT( fe.receiver.value, dp::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               DOS_ASSERT( v.size() == 3, dp::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               DOS_ASSERT( fe.receiver.value, dp::invalid_arg_exception,
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        DOS_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
        ...
        while( start_itr != end_itr ) {
           ...
           dp::datastream<const char*> ds( a.packed_action_trace.data(), a.packed_action_trace.size() );
           action_trace t;
           dp::raw::unpack( ds, t );
           ...
           end_time = dp::time_point::now();
           if( end_time - start_time > dp::microseconds(100000) ) {
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         try {
            DP_ASSERT( input_id_length <= 64, "hex string is too long to represent an actual transaction id" );
            DP_ASSERT( input_id_length >= 8,  "hex string representing transaction id should be at least 8 characters long to avoid excessive collisions" );
            input_id = transaction_id_type(p.id);
         } DOS_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            DOS_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            while( itr != idx.end() && itr->trx_id == result.id ) {
              ...
              dp::datastream<const char*> ds( itr->packed_action_trace.data(), itr->packed_action_trace.size() );
              action_trace t;
              dp::raw::unpack( ds, t );
                ...
                for (const auto &receipt: blk->transactions) {
                    if (receipt.trx.contains<packed_transaction>()) {
                        ...
                        if (mtrx.id == result.id) {
                            dp::mutable_variant_object r("receipt", receipt);
            ...
            if (!found) {
               DOS_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
