...
#include <dosio/net_plugin/net_plugin.hpp>
#include <dosio/net_plugin/protocol.hpp>
...
#include <dp/network/message_buffer.hpp>
...
using namespace dosio::chain::plugin_interface::compat;
...
namespace dosio {
   ...
   using dosio::chain::transaction_id_type;
   ...
   struct update_in_flight {
      ...
      void operator() (node_transaction_state& nts) {
         ...
         nts.expires = dp::time_point_sec (exp + incr * 60);
         if( nts.requests == 0 ) {
   ...
   public:
      ...
      void transaction_ack(const std::pair<dp::exception_ptr, packed_transaction_ptr>&);
       ...
       */
      chain::signature_type sign_compact(const chain::public_key_type& signer, const dp::sha256& digest) const;

      uint16_t to_protocol_version(uint16_t v);
   };

   const dp::string logger_name("net_plugin_impl");
   dp::logger logger;
...
#define peer_dlog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( logger.is_enabled( dp::log_level::debug ) ) \
      logger.log( DP_LOG_MESSAGE( debug, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  DP_MULTILINE_MACRO_END

#define peer_ilog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( logger.is_enabled( dp::log_level::info ) ) \
      logger.log( DP_LOG_MESSAGE( info, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  DP_MULTILINE_MACRO_END

#define peer_wlog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( logger.is_enabled( dp::log_level::warn ) ) \
      logger.log( DP_LOG_MESSAGE( warn, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant()) ) ); \
  DP_MULTILINE_MACRO_END

#define peer_elog( PEER, FORMAT, ... ) \
  DP_MULTILINE_MACRO_BEGIN \
   if( logger.is_enabled( dp::log_level::error ) ) \
      logger.log( DP_LOG_MESSAGE( error, peer_log_format + FORMAT, __VA_ARGS__ (PEER->get_logger_variant())) ); \
  DP_MULTILINE_MACRO_END
   ...
   typedef multi_index_container<
      ...
      indexed_by<
         ...
         ordered_non_unique< tag< by_expiry >, member< transaction_state,dp::time_point_sec,&transaction_state::expires >>,
         ordered_non_unique<
            tag<by_block_num>,
   ...
   struct update_request_time {
      ...
      void operator () (struct dosio::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      dosio::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<dosio::peer_block_state, block_id_type, &dosio::peer_block_state::id > >,
         ordered_unique< tag<by_block_num>, member<dosio::peer_block_state, uint32_t, &dosio::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (dosio::peer_block_state& bs) {
         bs.is_known = true;
      }
      void operator() (transaction_state& ts) {
   ...
   public:
      ...
      dp::message_buffer<1024*1024>    pending_message_buffer;
      dp::optional<std::size_t>        outstanding_read_bytes;
      ...
      dp::optional<dp::variant_object> _logger_variant;
      const dp::variant_object& get_logger_variant()  {
         if (!_logger_variant) {
            ...
            _logger_variant.emplace(dp::mutable_variant_object()
               ("_name", peer_name())
               ("_id", node_id)
   ...
   struct msgHandler : public dp::visitor<void> {
   ...
   void connection::close() {
      ...
      dp_dlog(logger, "canceling wait on ${p}", ("p",peer_name()));
      cancel_wait();
               ...
               queue_write(std::make_shared<vector<char>>(tx->serialized_txn),
                           ...
                           [tx_id=tx->id](boost::system::error_code ec, std::size_t ) {
                              ...
                              if (tx != local_txns.end()) {
                                 ...
                                 dp_wlog(logger, "Local pending TX erased before queued_write called callback");
                              }
                           });
            }
         }
            ...
            queue_write(std::make_shared<vector<char>>(tx->serialized_txn),
                        ...
                        [t](boost::system::error_code ec, std::size_t ) {
                           ...
                           if (tx != local_txns.end()) {
                              ...
                              dp_wlog(logger, "Local TX erased before queued_write called callback");
                           }
                        });
         }
      }
   ...
   void connection::blk_send_branch() {
      ...
      dp_dlog(logger, "head_num = ${h}",("h",head_num));
      if(head_num == 0) {
         enqueue(note);
         return;
      }
      ...
      try {
         if (last_handshake_recv.generation >= 1) {
            ...
            dp_dlog(logger, "maybe truncating branch at  = ${h}:${id}",("h",remote_head_num)("id",remote_head_id));
         }
      ...
      if (!bstack.empty()) {
         ...
         dp_ilog(logger, "Sent ${n} blocks on my fork",("n",count));
      } else {
         dp_ilog(logger, "Nothing to send on fork request");
      }
   ...
   void connection::blk_send(const vector<block_id_type> &ids) {
      ...
      for(auto &blkid : ids) {
         ...
         try {
            ...
            if(b) {
               dp_dlog(logger,"found block for id at num ${n}",("n",b->block_num()));
   ...
   void connection::send_handshake( ) {
      ...
      dp_dlog(logger, "Sending handshake generation ${g} to ${ep}",
   ...
   void connection::do_queue_write() {
      ...
      if(!socket->is_open()) {
         dp_elog(logger,"socket not open to ${p}",("p",peer_name()));
   ...
   void connection::cancel_sync(go_away_reason reason) {
      dp_dlog(logger,"cancel sync reason = ${m}, write queue size ${o} peer ${p}",
      ...
      default:
         dp_dlog(logger, "sending empty request but not calling sync wait on ${p}", ("p",peer_name()));
   ...
   void connection::enqueue( const net_message &m, bool trigger_send ) {
      ...
      uint32_t payload_size = dp::raw::pack_size( m );
      ...
      dp::datastream<char*> ds( send_buffer->data(), buffer_size);
      ...
      queue_write(send_buffer,trigger_send,
                  [weak_this, close_after_send](boost::system::error_code ec, std::size_t ) {
                     ...
                     if (conn) {
                        ...
                        dp_wlog(logger, "connection expired before enqueued net_message called callback!");
                     }
                  });
   }
   ...
   void connection::fetch_timeout( boost::system::error_code ec ) {
      if( !ec ) {
         ...
         if( !connected( ) ) {
            dp_dlog(logger, "fetch timeout was cancelled due to dead connection");
         }
      }
      else {
   ...
   bool connection::process_next_message(net_plugin_impl& impl, uint32_t message_length) {
      try {
         ...
         // This code is copied from dp::io::unpack(..., unsigned_int)
      ...
      ,state(in_sync)
   {
      ...
      DOS_ASSERT( chain_plug, chain::missing_chain_plugin_exception, ""  );
   }
   ...
   void sync_manager::set_state(stages newstate) {
      ...
      dp_dlog(logger, "old state ${os} becoming ${ns}",("os",stage_str (state))("ns",stage_str (newstate)));
      state = newstate;
   }
   ...
   bool sync_manager::is_active(connection_ptr c) {
      if (state == head_catchup && c) {
         ...
         dp_dlog(logger, "fork_head_num = ${fn} fork_head set = ${s}",
   ...
   bool sync_manager::sync_required( ) {
      dp_dlog(logger, "last req = ${req}, last recv = ${recv} known = ${known} our head = ${head}",
   ...
   void sync_manager::request_next_chunk( connection_ptr conn ) {
      ...
      if (head_block < sync_last_requested_num && source && source->current()) {
         dp_ilog (logger, "ignoring request, head is ${h} last req = ${r} source is ${p}",
      ...
      if( sync_last_requested_num != sync_known_lib_num ) {
         ...
         if( end > 0 && end >= start ) {
            dp_ilog(logger, "requesting range ${s} to ${e}, from ${n}",
   ...
   void sync_manager::start_sync( connection_ptr c, uint32_t target) {
      ...
      if (!sync_required()) {
         ...
         dp_dlog( logger, "We are already caught up, my irr = ${b}, head = ${h}, target = ${t}",
      ...
      dp_ilog(logger, "Catching up with chain, our last req is ${cc}, theirs is ${t} peer ${p}",
   ...
   void sync_manager::reassign_fetch(connection_ptr c, go_away_reason reason) {
      dp_ilog(logger, "reassign_fetch, our last req is ${cc}, next expected is ${ne} peer ${p}",
   ...
   void sync_manager::recv_handshake (connection_ptr c, const handshake_message &msg) {
      ...
      if (head_id == msg.head_id) {
         dp_dlog(logger, "sync check state 0");
      ...
      if (head < peer_lib) {
         dp_dlog(logger, "sync check state 1");
      ...
      if (lib_num > msg.head_num ) {
         dp_dlog(logger, "sync check state 2");
      ...
      if (head <= msg.head_num ) {
         dp_dlog(logger, "sync check state 3");
         verify_catchup (c, msg.head_num, msg.head_id);
         return;
      }
      else {
         dp_dlog(logger, "sync check state 4");
         if (msg.generation > 1 ||  c->protocol_version > proto_base) {
            notice_message note;
   ...
   void sync_manager::recv_notice (connection_ptr c, const notice_message &msg) {
      dp_ilog (logger, "sync_manager got ${m} block notice",("m",modes_str(msg.known_blocks.mode)));
   ...
   void sync_manager::rejected_block (connection_ptr c, uint32_t blk_num) {
      if (state != in_sync ) {
         dp_ilog (logger, "block ${bn} not accepted from ${p}",("bn",blk_num)("p",c->peer_name()));
         sync_last_requested_num = 0;
         source.reset();
         my_impl->close(c);
         set_state(in_sync);
   ...
   void sync_manager::recv_block (connection_ptr c, const block_id_type &blk_id, uint32_t blk_num) {
      dp_dlog(logger," got block ${bn} from ${p}",("bn",blk_num)("p",c->peer_name()));
      if (state == lib_catchup) {
         if (blk_num != sync_next_expected_num) {
            dp_ilog (logger, "expected block ${ne} but got ${bn}",("ne",sync_next_expected_num)("bn",blk_num));
            my_impl->close(c);
            return;
         }
      ...
      if (state == head_catchup) {
         dp_dlog (logger, "sync_manager in head_catchup state");
         set_state(in_sync);
         source.reset();
         ...
         if( blk_num == sync_known_lib_num ) {
            dp_dlog( logger, "All caught up with last known last irreversible block resending handshake");
            set_state(in_sync);
            send_handshakes();
         }
            ...
            dp_dlog(logger,"calling sync_wait on connection ${p}",("p",c->peer_name()));
            c->sync_wait();
         }
      }
   }
   ...
   void dispatch_manager::bcast_block (const signed_block &bsum) {
      ...
      uint32_t packsiz = dp::raw::pack_size(msg);
      ...
      if (( large_msg_notify && msgsiz > just_send_it_max) && !skips.empty()) {
         dp_ilog(logger, "block size is ${ms}, sending notify",("ms", msgsiz));
   ...
   void dispatch_manager::recv_block (connection_ptr c, const block_id_type& id, uint32_t bnum) {
      ...
      dp_dlog(logger, "canceling wait on ${p}", ("p",c->peer_name()));
      c->cancel_wait();
   }

   void dispatch_manager::rejected_block (const block_id_type& id) {
      dp_dlog(logger,"not sending rejected transaction ${tid}",("tid",id));
   ...
   void dispatch_manager::bcast_transaction (const packed_transaction& trx) {
      ...
      if( my_impl->local_txns.get<by_id>().find( id ) != my_impl->local_txns.end( ) ) { //found
         dp_dlog(logger, "found trxid in local_trxs" );
         return;
      }
      uint32_t packsiz = 0;
      uint32_t bufsiz = 0;
      ...
      net_message msg(trx);
      packsiz = dp::raw::pack_size(msg);
      bufsiz = packsiz + sizeof(packsiz);
      vector<char> buff(bufsiz);
      dp::datastream<char*> ds( buff.data(), bufsiz);
      ...
      if( !large_msg_notify || bufsiz <= just_send_it_max) {
         my_impl->send_all( trx, [id, &skips, trx_expiration](connection_ptr c) -> bool {
               ...
               if( unknown) {
                  ...
                  dp_dlog(logger, "sending whole trx to ${n}", ("n",c->peer_name() ) );
               } else {
         ...
         my_impl->send_all(pending_notify, [id, &skips, trx_expiration](connection_ptr c) -> bool {
               ...
               bool unknown = bs == c->trx_state.end();
               if( unknown) {
                  dp_dlog(logger, "sending notice to ${n}", ("n",c->peer_name() ) );
   ...
   void dispatch_manager::recv_notice (connection_ptr c, const notice_message& msg, bool generated) {
      ...
      if (msg.known_trx.mode == normal) {
         ...
         for( const auto& t : msg.known_trx.ids ) {
            ...
            if( tx == my_impl->local_txns.end( ) ) {
               dp_dlog(logger,"did not find ${id}",("id",t));
               ...
               dp_dlog(logger,"big msg manager found txn id in table, ${id}",("id", t));
            }
         }
         send_req = !req.req_trx.ids.empty();
         dp_dlog(logger,"big msg manager send_req ids list has ${ids} entries", ("ids", req.req_trx.ids.size()));
      }
      ...
      if (msg.known_blocks.mode == normal) {
         ...
         for( const auto& blkid : msg.known_blocks.ids) {
            signed_block_ptr b;
            peer_block_state entry = {blkid,0,true,true,dp::time_point()};
            try {
            ...
            if (!b) {
               ...
               entry.requested_time = dp::time_point::now();
            }
      ...
      dp_dlog( logger, "send req = ${sr}", ("sr",send_req));
      if( send_req) {
         c->enqueue(req);
         c->fetch_wait();
   ...
   void dispatch_manager::retry_fetch( connection_ptr c ) {
      ...
      dp_wlog( logger, "failed to fetch from ${p}",("p",c->peer_name()));
      ...
      if( c->last_req->req_trx.mode == normal && !c->last_req->req_trx.ids.empty() ) {
         ...
         dp_wlog( logger,"no retry, block mpde = ${b} trx mode = ${t}",
   ...
   void net_plugin_impl::connect( connection_ptr c ) {
      if( c->no_retry != go_away_reason::no_reason) {
         dp_dlog( logger, "Skipping connect due to go_away reason ${r}",("r", reason_str( c->no_retry )));
         return;
      }
   ...
   void net_plugin_impl::start_listen_loop( ) {
      ...
      acceptor->async_accept( *socket, [socket,this]( boost::system::error_code ec ) {
            if( !ec ) {
               ...
               if (ec) {
                  dp_elog(logger,"Error getting remote endpoint: ${m}",("m", ec.message()));
               }
               else {
                  ...
                  if( from_addr < max_nodes_per_host && (max_client_count == 0 || num_clients < max_client_count )) {
                     ...
                     if (from_addr >= max_nodes_per_host) {
                        dp_elog(logger, "Number of connections (${n}) from ${ra} exceeds limit",
                                ("n", from_addr+1)("ra",paddr.to_string()));
                     }
                     else {
                        dp_elog(logger, "Error max_client_count ${m} exceeded",
                                ( "m", max_client_count) );
                     }
                     socket->close( );
                  }
            ...
            [this,weak_conn]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     DOS_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   bool net_plugin_impl::is_valid( const handshake_message &msg) {
      ...
      if ((msg.sig != chain::signature_type() || msg.token != sha256()) && (msg.token != dp::sha256::hash(msg.time))) {
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const handshake_message &msg) {
      ...
      if (msg.generation == 1) {
         ...
         if( c->peer_addr.empty() || c->last_handshake_recv.node_id == dp::sha256()) {
            dp_dlog(logger, "checking for duplicate" );
            for(const auto &check : connections) {
               ...
               if(check->connected() && check->peer_name() == msg.p2p_address) {
                  ...
                  dp_dlog( logger, "sending go_away duplicate to ${ep}", ("ep",msg.p2p_address) );
            ...
            dp_dlog(logger, "skipping duplicate check, addr == ${pa}, id = ${ni}",("pa",c->peer_addr)("ni",c->last_handshake_recv.node_id));
         }
         ...
         bool on_fork = false;
         dp_dlog(logger, "lib_num = ${ln} peer_lib = ${pl}",("ln",lib_num)("pl",peer_lib));
   ...
   void net_plugin_impl::handle_message(connection_ptr c, const time_message &msg) {
      ...
      if(logger.is_enabled(dp::log_level::all))
         logger.log(DP_LOG_MESSAGE(all, "Clock offset is ${o}ns (${us}us)", ("o", c->offset)("us", c->offset/NsecPerUsec)));
      c->org = 0;
      c->rec = 0;
   }
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const notice_message &msg) {
      ...
      if (msg.known_trx.mode != none) {
         dp_dlog(logger,"this is a ${m} notice with ${n} transactions", ("m",modes_str(msg.known_trx.mode))("n",msg.known_trx.pending));
      }
      ...
      if (msg.known_blocks.mode != none) {
         dp_dlog(logger,"this is a ${m} notice with ${n} blocks", ("m",modes_str(msg.known_blocks.mode))("n",msg.known_blocks.pending));
      }
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const packed_transaction &msg) {
      dp_dlog(logger, "got a packed transaction, cancel wait");
      peer_ilog(c, "received packed_transaction");
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == dosio::db_read_mode::READ_ONLY ) {
         dp_dlog(logger, "got a txn in read-only mode - dropping");
         return;
      }
      if( sync_master->is_active(c) ) {
         dp_dlog(logger, "got a txn during sync - dropping");
         return;
      }
      transaction_id_type tid = msg.id();
      c->cancel_wait();
      if(local_txns.get<by_id>().find(tid) != local_txns.end()) {
         dp_dlog(logger, "got a duplicate transaction - dropping");
         return;
      }
      dispatcher->recv_transaction(c, tid);
      chain_plug->accept_transaction(msg, [=](const static_variant<dp::exception_ptr, transaction_trace_ptr>& result) {
         if (result.contains<dp::exception_ptr>()) {
            peer_dlog(c, "bad packed_transaction : ${m}", ("m",result.get<dp::exception_ptr>()->what()));
         } else {
            auto trace = result.get<transaction_trace_ptr>();
            if (!trace->except) {
               dp_dlog(logger, "chain accepted transaction");
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const signed_block &msg) {
      ...
      dp::microseconds age( dp::time_point::now() - msg.timestamp);
   ...
   void net_plugin_impl::expire_txns() {
      ...
      auto ex_lo = old.lower_bound( dp::time_point_sec( 0));
   ...
   void net_plugin_impl::connection_monitor(std::weak_ptr<connection> from_connection) {
      auto max_time = dp::time_point::now();
      max_time += dp::milliseconds(max_cleanup_time_ms);
      auto from = from_connection.lock();
      auto it = (from ? connections.find(from) : connections.begin());
      if (it == connections.end()) it = connections.begin();
      while (it != connections.end()) {
         if (dp::time_point::now() >= max_time) {
   ...
   void net_plugin_impl::close( connection_ptr c ) {
      if( c->peer_addr.empty( ) && c->socket->is_open() ) {
         if (num_clients == 0) {
            dp_wlog( logger, "num_clients already at 0");
         }
         else {
            --num_clients;
         }
   ...
   void net_plugin_impl::accepted_block_header(const block_state_ptr& block) {
      dp_dlog(logger,"signaled, id = ${id}",("id", block->id));
   }
   ...
   void net_plugin_impl::accepted_transaction(const transaction_metadata_ptr& md) {
      dp_dlog(logger,"signaled, id = ${id}",("id", md->id));
//      dispatcher->bcast_transaction(md->packed_trx);
   }

   void net_plugin_impl::applied_transaction(const transaction_trace_ptr& txn) {
      dp_dlog(logger,"signaled, id = ${id}",("id", txn->id));
   }

   void net_plugin_impl::accepted_confirmation(const header_confirmation& head) {
      dp_dlog(logger,"signaled, id = ${id}",("id", head.block_id));
   }

   void net_plugin_impl::transaction_ack(const std::pair<dp::exception_ptr, packed_transaction_ptr>& results) {
      transaction_id_type id = results.second->id();
      if (results.first) {
         dp_ilog(logger,"signaled NACK, trx-id = ${id} : ${why}",("id", id)("why", results.first->to_detail_string()));
         dispatcher->rejected_transaction(id);
      } else {
         dp_ilog(logger,"signaled ACK, trx-id = ${id}",("id", id));
   ...
   bool net_plugin_impl::authenticate_peer(const handshake_message& msg) const {
      ...
      if(msg.sig != chain::signature_type() && msg.token != sha256()) {
         sha256 hash = dp::sha256::hash(msg.time);
         if(hash != msg.token) {
   ...
   chain::signature_type net_plugin_impl::sign_compact(const chain::public_key_type& signer, const dp::sha256& digest) const
   {
   ...
   handshake_initializer::populate( handshake_message &hello) {
      ...
      hello.token = dp::sha256::hash(hello.time);
      ...
      controller& cc = my_impl->chain_plug->chain();
      hello.head_id = dp::sha256();
      hello.last_irreversible_block_id = dp::sha256();
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"DOS Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( options.count( "p2p-server-address" )) {
            ...
            if( my->listen_endpoint.address().to_v4() == address_v4::any()) {
               ...
               if( ec.value() != boost::system::errc::success ) {
                  ...
                  DP_THROW_EXCEPTION( dp::invalid_arg_exception,
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            DOS_ASSERT( options.count( "peer-key" ),
         ...
         if( options.count( "peer-private-key" )) {
            ...
            for( const std::string& key_id_to_wif_pair_string : key_id_to_wif_pair_strings ) {
               ...
               my->private_keys[key_id_to_wif_pair.first] = dp::crypto::private_key( key_id_to_wif_pair.second );
            }
         }

         my->chain_plug = app().find_plugin<chain_plugin>();
         DOS_ASSERT( my->chain_plug, chain::missing_chain_plugin_exception, ""  );
         my->chain_id = app().get_plugin<chain_plugin>().get_chain_id();
         dp::rand_pseudo_bytes( my->node_id.data(), my->node_id.data_size());
   ...
   void net_plugin::plugin_startup() {
      ...
      if(dp::get_logger_map().find(logger_name) != dp::get_logger_map().end())
         logger = dp::get_logger_map()[logger_name];
   }
   ...
   string net_plugin::connect( const string& host ) {
      ...
      dp_dlog(logger,"adding new connection to the list");
      my->connections.insert( c );
      dp_dlog(logger,"calling active connector");
      my->connect( c );
