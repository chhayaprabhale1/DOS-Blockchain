 ...
 */
#include <dosio/faucet_testnet_plugin/faucet_testnet_plugin.hpp>
...
DP_REFLECT(dosio::detail::faucet_testnet_empty, );
DP_REFLECT(dosio::detail::faucet_testnet_keys, (owner)(active));
DP_REFLECT(dosio::detail::faucet_testnet_create_account_params, (account)(keys));
DP_REFLECT(dosio::detail::faucet_testnet_create_account_alternates_response, (alternates)(message));
DP_REFLECT(dosio::detail::faucet_testnet_create_account_rate_limited_response, (message));
...
using results_pair = std::pair<uint32_t,dp::variant>;
...
{std::string("/v1/" #api_name "/" #call_name), \
   [this](string, string body, url_response_callback response_cb) mutable { \
          try { \
             ...
             response_cb(result.first, dp::json::to_string(result.second)); \
          } catch (...) { \
...
struct faucet_testnet_plugin_impl {
   ...
   results_pair find_alternates(const std::string& new_account_name) {
      ...
      while (names.size() < _default_create_alternates_to_return &&
         ...
         if (extension_capacity > 0) {
            ...
            while(names.size() < _default_create_alternates_to_return && ext.increment()) {
               ...
               } catch (const dp::assert_exception& ) {
                  continue;
               }
      ...
      const dosio::detail::faucet_testnet_create_account_alternates_response response{
         names, "Account name is already in use."};
      return { conflict_with_alternates, dp::variant(response) };
   }

   results_pair create_account(const std::string& new_account_name, const dp::crypto::public_key& owner_pub_key, const dp::crypto::public_key& active_pub_key) {

      auto creating_account = database().find<account_object, by_name>(_create_account_name);
      DOS_ASSERT(creating_account != nullptr, transaction_exception,
      ...
      {
         dosio::detail::faucet_testnet_create_account_rate_limited_response response{
            "Rate limit exceeded, the max is 1 request per " + dp::to_string(_create_interval_msec) +
            " milliseconds. Come back later."};
         return std::make_pair(too_many_requests, dp::variant(response));
      }
      ...
      auto memo = dp::variant(dp::time_point::now()).as_string() + " " + dp::variant(dp::time_point::now().time_since_epoch()).as_string();
      ...
      return std::make_pair(account_created, dp::variant(dosio::detail::faucet_testnet_empty()));
   }

   results_pair create_faucet_account(const std::string& body) {
      const dosio::detail::faucet_testnet_create_account_params params = dp::json::from_string(body).as<dosio::detail::faucet_testnet_create_account_params>();
      return create_account(params.account, dp::crypto::public_key(params.keys.owner), dp::crypto::public_key(params.keys.active));
   }
   ...
   dp::crypto::private_key _create_account_private_key;
...
const key_pair faucet_testnet_plugin_impl::_default_key_pair = {"DOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"};
...
void faucet_testnet_plugin::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options()
         ...
         ("faucet-private-key", boost::program_options::value<std::string>()->default_value(dp::json::to_string(faucet_testnet_plugin_impl::_default_key_pair)),
...
void faucet_testnet_plugin::plugin_initialize(const variables_map& options) {
   try {
      ...
      auto faucet_key_pair = dp::json::from_string( options.at( "faucet-private-key" ).as<std::string>()).as<key_pair>();
      ...
      dp::crypto::private_key private_key( faucet_key_pair.second );
