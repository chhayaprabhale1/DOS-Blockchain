 ...
 */
#include <dosio/test_control_plugin/test_control_plugin.hpp>
...
private:
   ...
   dp::optional<boost::signals2::scoped_connection> _accepted_block_connection;
   dp::optional<boost::signals2::scoped_connection> _irreversible_block_connection;
...
void test_control_plugin_impl::process_next_block_state(const chain::block_header_state& bhs) {
   const auto block_time = _chain.head_block_time() + dp::microseconds(chain::config::block_interval_us);
