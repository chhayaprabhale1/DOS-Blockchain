...
#include <dosio/chain/fork_database.hpp>
#include <dosio/chain/block_log.hpp>
#include <dosio/chain/exceptions.hpp>
#include <dosio/chain/authorization_manager.hpp>
#include <dosio/chain/producer_object.hpp>
#include <dosio/chain/config.hpp>
#include <dosio/chain/wasm_interface.hpp>
#include <dosio/chain/resource_limits.hpp>
#include <dosio/chain/reversible_block_object.hpp>
#include <dosio/chain/controller.hpp>
#include <dosio/chain/generated_transaction_object.hpp>
#include <dosio/chain/snapshot.hpp>

#include <dosio/chain/dosio_contract.hpp>
...
std::ostream& operator<<(std::ostream& osm, dosio::chain::db_read_mode m) {
   if ( m == dosio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == dosio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == dosio::chain::db_read_mode::READ_ONLY ) {
      osm << "read-only";
   } else if ( m == dosio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              dosio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(dosio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(dosio::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(dosio::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(dosio::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, dosio::chain::validation_mode m) {
   if ( m == dosio::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == dosio::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              dosio::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(dosio::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(dosio::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace dosio::chain::config;
using namespace dosio::chain::plugin_interface;
...
#define CATCH_AND_CALL(NEXT)\
   catch ( const dp::exception& err ) {\
      NEXT(err.dynamic_copy_exception());\
   } catch ( const std::exception& e ) {\
      dp::exception dpe( \
         DP_LOG_MESSAGE( warn, "rethrow ${what}: ", ("what",e.what())),\
         dp::std_exception_code,\
         BOOST_CORE_TYPEID(e).name(),\
         e.what() ) ;\
      NEXT(dpe.dynamic_copy_exception());\
   } catch( ... ) {\
      dp::unhandled_exception e(\
         DP_LOG_MESSAGE(warn, "rethrow"),\
...
public:
   ...
   dp::optional<fork_database>      fork_db;
   dp::optional<block_log>          block_logger;
   dp::optional<controller::config> chain_config;
   dp::optional<controller>         chain;
   dp::optional<chain_id_type>      chain_id;
   //txn_msg_rate_limits              rate_limits;
   dp::optional<vm_type>            wasm_runtime;
   dp::microseconds                 abi_serializer_max_time_ms;
   dp::optional<bfs::path>          snapshot_path;
   ...
   dp::optional<scoped_connection>                                   pre_accepted_block_connection;
   dp::optional<scoped_connection>                                   accepted_block_header_connection;
   dp::optional<scoped_connection>                                   accepted_block_connection;
   dp::optional<scoped_connection>                                   irreversible_block_connection;
   dp::optional<scoped_connection>                                   accepted_transaction_connection;
   dp::optional<scoped_connection>                                   applied_transaction_connection;
   dp::optional<scoped_connection>                                   accepted_confirmation_connection;
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<dosio::chain::wasm_interface::vm_type>()->value_name("wavm/wabt"), "Override default WASM runtime")
         ...
         ("read-mode", boost::program_options::value<dosio::chain::db_read_mode>()->default_value(dosio::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<dosio::chain::validation_mode>()->default_value(dosio::chain::validation_mode::FULL),
...
dp::time_point calculate_genesis_timestamp( string tstr ) {
   dp::time_point genesis_timestamp;
   if( strcasecmp (tstr.c_str(), "now") == 0 ) {
      genesis_timestamp = dp::time_point::now();
   } else {
   ...
   if (diff_us > 0) {
      ...
      genesis_timestamp += dp::microseconds(delay_us);
...
void clear_directory_contents( const dp::path& p ) {
   using boost::filesystem::directory_iterator;

   if( !dp::is_directory( p ) )
      return;

   for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
      dp::remove_all( itr->path() );
   }
}
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if DOSIO_ROOT_KEY is bad
      } catch ( const dp::exception& ) {
         elog( "DOSIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::dosio_root_key));
         throw;
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            DOS_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            auto item = dp::json::from_string(cp).as<std::pair<uint32_t,block_id_type>>();
            auto itr = my->loaded_checkpoints.find(item.first);
            if( itr != my->loaded_checkpoints.end() ) {
               DOS_ASSERT( itr->second == item.second,
      ...
      if(options.count("abi-serializer-max-time-ms"))
         my->abi_serializer_max_time_ms = dp::microseconds(options.at("abi-serializer-max-time-ms").as<uint32_t>() * 1000);
      ...
      if( options.count( "chain-threads" )) {
         ...
         DOS_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( dp::exists( my->blocks_dir / "blocks.log" )) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            dp::json::save_to_file( gs, p, true );
            ilog( "Saved genesis JSON to '${path}'", ("path", p.generic_string()));
         }

         DOS_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         DOS_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         if( dp::exists( backup_dir / config::reversible_blocks_dir_name ) ||
            ...
            if( !recover_reversible_blocks( backup_dir / config::reversible_blocks_dir_name,
               ...
               dp::copy( backup_dir / config::reversible_blocks_dir_name,
                         my->chain_config->blocks_dir / config::reversible_blocks_dir_name );
               dp::copy( backup_dir / config::reversible_blocks_dir_name / "shared_memory.bin",
                         my->chain_config->blocks_dir / config::reversible_blocks_dir_name / "shared_memory.bin" );
               dp::copy( backup_dir / config::reversible_blocks_dir_name / "shared_memory.meta",
         ...
         DOS_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         dp::remove_all( my->chain_config->blocks_dir/config::reversible_blocks_dir_name );
         ...
         DOS_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         DOS_ASSERT( dp::exists(*my->snapshot_path), plugin_config_exception,
         ...
         DOS_ASSERT( options.count( "genesis-json" ) == 0 &&  options.count( "genesis-timestamp" ) == 0,
                 plugin_config_exception,
         ...
         DOS_ASSERT( !dp::exists(shared_mem_path),
                 plugin_config_exception,
                 "Snapshot can only be used to initialize an empty database." );

         if( dp::is_regular_file( my->blocks_dir / "blocks.log" )) {
            auto log_genesis = block_log::extract_genesis_state(my->blocks_dir);
            DOS_ASSERT( log_genesis.compute_chain_id() == my->chain_config->genesis.compute_chain_id(),
                    plugin_config_exception,
         ...
         dp::optional<genesis_state> existing_genesis;
         ...
         if( options.count( "genesis-json" )) {
            ...
            DOS_ASSERT( dp::is_regular_file( genesis_file ),
                        plugin_config_exception,
                       "Specified genesis file '${genesis}' does not exist.",
                       ("genesis", genesis_file.generic_string()));

            my->chain_config->genesis = dp::json::from_file( genesis_file ).as<genesis_state>();
         }
         ...
         if( !existing_genesis ) {
            ...
            DOS_ASSERT( my->chain_config->genesis == *existing_genesis, plugin_config_exception,
      ...
      if ( options.count("read-mode") ) {
         ...
         DOS_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, plugin_config_exception, "irreversible mode not currently supported." );
      }
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            DOS_ASSERT( itr->second == id, checkpoint_exception,
...
chain_apis::read_write::read_write(controller& db, const dp::microseconds& abi_serializer_max_time)
: db(db)
...
void chain_apis::read_write::validate() const {
   DOS_ASSERT( db.get_read_mode() != chain::db_read_mode::READ_ONLY, missing_chain_api_plugin_exception, "Not allowed, node in read-only mode" );
}
...
bool chain_plugin::recover_reversible_blocks( const dp::path& db_dir, uint32_t cache_size,
                                              optional<dp::path> new_db_dir, uint32_t truncate_at_block ) {
   try {
   ...
   auto reversible_dir = dp::canonical( db_dir );
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         DOS_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
...
bool chain_plugin::import_reversible_blocks( const dp::path& reversible_dir,
                                             uint32_t cache_size,
                                             const dp::path& reversible_blocks_file ) {
   std::fstream         reversible_blocks;
   chainbase::database  new_reversible( reversible_dir, database::read_write, cache_size );
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         signed_block tmp;
         dp::raw::unpack(reversible_blocks, tmp);
         num = tmp.block_num();

         if( start == 0 ) {
            start = num;
         } else {
            DOS_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
      ...
      wlog( "${details}", ("details", e.to_detail_string()) );
      DP_RETHROW_EXCEPTION( e, warn, "rethrow" );
   } catch( ... ) {}
...
bool chain_plugin::export_reversible_blocks( const dp::path& reversible_dir,
                                             const dp::path& reversible_blocks_file ) {
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         ...
         signed_block tmp;
         dp::datastream<const char *> ds( itr->packedblock.data(), itr->packedblock.size() );
         dp::raw::unpack(ds, tmp); // Verify that packed block has not been corrupted.
         reversible_blocks.write( itr->packedblock.data(), itr->packedblock.size() );
         end = itr->blocknum;
         ++num;
      }
...
chain::chain_id_type chain_plugin::get_chain_id()const {
   DOS_ASSERT( my->chain_id.valid(), chain_id_type_exception, "chain ID has not been initialized yet" );
   return *my->chain_id;
}

dp::microseconds chain_plugin::get_abi_serializer_max_time() const {
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/noddos/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   DOS_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         pos = dp::to_uint64( p.index_position );
      } catch(...) {
         DOS_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,DOS
      try {
         auto symb = dosio::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }
   
   try {
      return ( dosio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      DOS_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   double val{};
   try {
      val = dp::variant(str).as<double>();
   } DP_RETHROW_EXCEPTIONS(warn, "Could not convert ${desc} string '${str}' to key type.", ("desc", desc)("str",str) )

   DOS_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   DOS_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   DOS_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = dosio::chain_apis::get_abi( db, p.code );

   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      DOS_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      DOS_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      DOS_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      DOS_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
}
...
read_only::get_table_by_scope_result read_only::get_table_by_scope( const read_only::get_table_by_scope_params& p )const {
   ...
   auto walk_table_range = [&]( auto itr, auto end_itr ) {
      auto cur_time = dp::time_point::now();
      auto end_time = cur_time + dp::microseconds(1000 * 10); /// 10ms max time
      for( unsigned int count = 0; cur_time <= end_time && count < p.limit && itr != end_itr; ++itr, cur_time = dp::time_point::now() ) {
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      DOS_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      dp::datastream<const char *> ds(obj.value.data(), obj.value.size());
      dp::raw::unpack(ds, cursor);

      DOS_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
dp::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   dp::mutable_variant_object results;

   const abi_def abi = dosio::chain_apis::get_abi( db, p.code );
   (void)get_table_type( abi, "stat" );

   uint64_t scope = ( dosio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, scope, N(stat), [&](const key_value_object& obj){
      DOS_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");

      dp::datastream<const char *> ds(obj.value.data(), obj.value.size());
      read_only::get_currency_stats_result result;

      dp::raw::unpack(ds, result.supply);
      dp::raw::unpack(ds, result.max_supply);
      dp::raw::unpack(ds, result.issuer);
...
dp::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const dp::microseconds& abi_serializer_max_time_ms, bool shorten_abi_errors ) {
   const auto table_type = get_table_type(abi, N(global));
   DOS_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   DOS_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   DOS_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   const abi_def abi = dosio::chain_apis::get_abi(db, config::system_account_name);
   const auto table_type = get_table_type(abi, N(producers));
   const abi_serializer abis{ abi, abi_serializer_max_time };
   DOS_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   DOS_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
   ...
   const auto stopTime = dp::time_point::now() + dp::microseconds(1000 * 10); // 10ms
   vector<char> data;
   ...
   for( ; it != secondary_index_by_secondary.end() && it->t_id == secondary_table_id->id; ++it ) {
      if (result.rows.size() >= p.limit || dp::time_point::now() > stopTime) {
      ...
      if (p.json)
         ...
         result.rows.emplace_back(dp::variant(data));
   }
...
struct resolver_factory {
   static auto make(const Api* api, const dp::microseconds& max_serialization_time) {
...
template<typename Api>
auto make_resolver(const Api* api, const dp::microseconds& max_serialization_time) {
...
read_only::get_scheduled_transactions( const read_only::get_scheduled_transactions_params& p ) const {
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  DOS_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
   ...
   auto time_limit = dp::time_point::now() + dp::microseconds(1000 * 10); /// 10ms max time
   while (itr != idx_by_delay.end() && remaining > 0 && time_limit > dp::time_point::now()) {
      auto row = dp::mutable_variant_object()
              ("trx_id", itr->trx_id)
              ("sender", itr->sender)
      ...
      if (p.json) {
         dp::variant pretty_transaction;

         transaction trx;
         dp::datastream<const char*> ds( itr->packed_trx.data(), itr->packed_trx.size() );
         dp::raw::unpack(ds,trx);
...
dp::variant read_only::get_block(const read_only::get_block_params& params) const {
   signed_block_ptr block;
   DOS_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64, chain::block_id_type_exception, "Invalid Block number or ID, must be greater than 0 and less than 64 characters" );
   try {
      block = db.fetch_block_by_id(dp::variant(params.block_num_or_id).as<block_id_type>());
      if (!block) {
         block = db.fetch_block_by_number(dp::to_uint64(params.block_num_or_id));
      }

   } DOS_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   DOS_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));

   dp::variant pretty_output;
   abi_serializer::to_variant(*block, pretty_output, make_resolver(this, abi_serializer_max_time), abi_serializer_max_time);

   uint32_t ref_block_prefix = block->id()._hash[1];

   return dp::mutable_variant_object(pretty_output.get_object())
           ("id", block->id())
           ("block_num",block->block_num())
           ("ref_block_prefix", ref_block_prefix);
}

dp::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   block_state_ptr b;
   optional<uint64_t> block_num;
   std::exception_ptr e;
   try {
      block_num = dp::to_uint64(params.block_num_or_id);
   } catch( ... ) {}
   ...
   if( block_num.valid() ) {
      ...
      try {
         b = db.fetch_block_state_by_id(dp::variant(params.block_num_or_id).as<block_id_type>());
      } DOS_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   DOS_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));

   dp::variant vo;
   dp::to_variant( static_cast<const block_header_state&>(*b), vo );
   return vo;
}
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   ...
   try {
      ...
      } DOS_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")

      app().get_method<incoming::methods::transaction_async>()(pretty_input, true, [this, next](const dp::static_variant<dp::exception_ptr, transaction_trace_ptr>& result) -> void{
...
static void push_recurse(read_write* rw, int index, const std::shared_ptr<read_write::push_transactions_params>& params, const std::shared_ptr<read_write::push_transactions_results>& results, const next_function<read_write::push_transactions_results>& next) {
   auto wrapped_next = [=](const dp::static_variant<dp::exception_ptr, read_write::push_transaction_results>& result) {
      if (result.contains<dp::exception_ptr>()) {
         const auto& e = result.get<dp::exception_ptr>();
         results->emplace_back( read_write::push_transaction_results{ transaction_id_type(), dp::mutable_variant_object( "error", e->to_detail_string() ) } );
      } else {
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      DOS_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   DOS_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );

   if( accnt.code.size() ) {
      result.wasm = string(accnt.code.begin(), accnt.code.end());
      result.code_hash = dp::sha256::hash( accnt.code.data(), accnt.code.size() );
   }
...
read_only::get_raw_abi_results read_only::get_raw_abi( const get_raw_abi_params& params )const {
   ...
   const auto& d = db.db();
   const auto& accnt = d.get<account_object,by_name>(params.account_name);
   result.abi_hash = dp::sha256::hash( accnt.abi.data(), accnt.abi.size() );
   result.code_hash = dp::sha256::hash( accnt.code.data(), accnt.code.size() );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            DOS_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(dosio.token);
      ...
      if( t_id != nullptr ) {
         ...
         if( it != idx.end() && it->value.size() >= sizeof(asset) ) {
            asset bal;
            dp::datastream<const char *> ds(it->value.data(), it->value.size());
            dp::raw::unpack(ds, bal);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   DOS_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      auto action_type = abis.get_action_type(params.action);
      DOS_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary( action_type, params.args, abi_serializer_max_time, shorten_abi_errors );
      } DOS_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      DOS_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
} DP_RETHROW_EXCEPTIONS( warn, "code: ${code}, action: ${action}, args: ${args}",
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } DOS_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")

   auto required_keys_set = db.get_authorization_manager().get_required_keys( pretty_input, params.available_keys, dp::seconds( pretty_input.delay_sec ));
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on dosio account (i.e. the system contract) and how it stores its data.
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(dosio), N(dosio), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
      auto it = idx.find(boost::make_tuple( t_id->id, dosio::chain::string_to_symbol_c(4,"RAMCORE") ));
      if( it != idx.end() ) {
         ...
         try {
            dp::raw::unpack(ds, ram_market_exchange_state);
         } catch( ... ) {
            return core_symbol;
         }
...
DP_REFLECT( dosio::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
