#!/bin/bash
   ...
   if [ "$ARCH" == "Linux" ]; then
      ...
      if [ ! -e /etc/os-release ]; then
         printf "\\n\\tDOSIO currently supports Amazon, Centos, Fedora, Mint & Ubuntu Linux only.\\n"
      ...
      case "$OS_NAME" in
         "Amazon Linux AMI"|"Amazon Linux")
            FILE="${SOURCE_DIR}/scripts/dosio_build_amazon.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
         ...
         ;;
         "CentOS Linux")
            FILE="${SOURCE_DIR}/scripts/dosio_build_centos.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
            MONGOD_CONF=${HOME}/opt/mongodb/mongod.conf
            export LLVM_DIR=${HOME}/opt/wasm/lib/cmake/llvm
         ...
         ;;
         "elementary OS")
            FILE="${SOURCE_DIR}/scripts/dosio_build_ubuntu.sh"
         ...
         ;;
         "Fedora")
            FILE="${SOURCE_DIR}/scripts/dosio_build_fedora.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
   ...
   if [ "$ARCH" == "Darwin" ]; then
      FILE="${SOURCE_DIR}/scripts/dosio_build_darwin.sh"
      CXX_COMPILER=clang++
      C_COMPILER=clang
   ...
   printf "\\n\\n>>>>>>>> ALL dependencies successfully found or installed . Installing DOSIO\\n\\n"
   ...
   if ! "${CMAKE}" -DCMAKE_BUILD_TYPE="${CMAKE_BUILD_TYPE}" -DCMAKE_CXX_COMPILER="${CXX_COMPILER}" \
      ...
      -DCMAKE_INSTALL_PREFIX="/usr/local/dosio" ${LOCAL_CMAKE_FLAGS} "${SOURCE_DIR}"
   then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> CMAKE building DOSIO has exited with the above error.\\n\\n"
      exit -1
   fi

   if [ "${START_MAKE}" == "false" ]; then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> DOSIO has been successfully configured but not yet built.\\n\\n"
      exit 0
   fi

   if [ -z ${JOBS} ]; then JOBS=$CPU_CORE; fi # Future proofing: Ensure $JOBS is set (usually set in scripts/dosio_build_*.sh scripts)
   if ! make -j"${JOBS}"
   then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> MAKE building DOSIO has exited with the above error.\\n\\n"
      exit -1
   fi
   ...
   printf "\\n\\tDOSIO has been successfully built. %02d:%02d:%02d\\n\\n" $(($TIME_END/3600)) $(($TIME_END%3600/60)) $(($TIME_END%60))
   printf "\\tTo verify your installation run the following commands:\\n"

   print_instructions

   printf "\\tFor more information:\\n"
   printf "\\tDOSIO website: https://eos.io\\n"
   printf "\\tDOSIO Telegram channel @ https://t.me/DOSProject\\n"
   printf "\\tDOSIO resources: https://eos.io/resources/\\n"
   printf "\\tDOSIO Stack Exchange: https://dosio.stackexchange.com\\n"
   printf "\\tDOSIO wiki: https://github.com/DOSIO/eos/wiki\\n\\n\\n"
